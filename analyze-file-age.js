...
const jsonReport = JSON.parse(fs.readFileSync('dead-code-analysis.json', 'utf-8'));
...
results.forEach((result, index) => {
  const date = result.lastModified === 'Never committed'
    ? 'Never committed'
...
oldFiles.forEach((result, index) => {
  ...
  console.log(`${index + 1}. ${result.file}`);
  console.log(`   Unused: ${result.unusedExports} | Last: ${date} | Commits: ${result.commitCount}`);
  console.log('');
});
