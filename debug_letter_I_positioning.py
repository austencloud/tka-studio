...
def debug_letter_I_positioning():
    ...
    try:
        ...
        print(f"  Red motion: {red_motion.motion_type.value} from {red_motion.start_loc.value} to {red_motion.end_loc.value}")
        print(f"  Blue motion: {blue_motion.motion_type.value} from {blue_motion.start_loc.value} to {blue_motion.end_loc.value}")
        ...
        blue_result, red_result = prop_service._calculate_letter_I_directions(blue_motion, red_motion)
        
        print(f"Blue {blue_motion.motion_type.value.upper()} direction: {blue_result.value}")
        print(f"Red {red_motion.motion_type.value.upper()} direction: {red_result.value}")
        ...
        is_opposite_first = (blue_result.value, red_result.value) in opposite_pairs or (red_result.value, blue_result.value) in opposite_pairs
        ...
        if len(letter_I_beats) > 1:
            ...
            if second_beat.has_pictograph:
                ...
                if red_motion_2 and blue_motion_2:
                    print(f"  Red motion: {red_motion_2.motion_type.value} from {red_motion_2.start_loc.value} to {red_motion_2.end_loc.value}")
                    print(f"  Blue motion: {blue_motion_2.motion_type.value} from {blue_motion_2.start_loc.value} to {blue_motion_2.end_loc.value}")
                    
                    print(f"\n🔍 Testing _calculate_letter_I_directions method (second):")
                    blue_result_2, red_result_2 = prop_service._calculate_letter_I_directions(blue_motion_2, red_motion_2)
                    
                    print(f"Blue {blue_motion_2.motion_type.value.upper()} direction: {blue_result_2.value}")
                    print(f"Red {red_motion_2.motion_type.value.upper()} direction: {red_result_2.value}")
                    
                    is_opposite_second = (blue_result_2.value, red_result_2.value) in opposite_pairs or (red_result_2.value, blue_result_2.value) in opposite_pairs
                    print(f"  Are opposite: {is_opposite_second}")
                    
                    if is_opposite_first and is_opposite_second:
                        print("✅ SUCCESS: Letter I positioning is working correctly for both cases!")
                        return True
