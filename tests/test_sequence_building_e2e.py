...
from PyQt6.QtCore import QTimer, QEventLoop, pyqtSignal, QObject
...
class SequenceBuildingE2ETest(QObject):
    ...
    def navigate_to_construct_tab(self) -> bool:
        ...
        try:
            ...
            for i in range(tab_widget.count()):
                ...
                if "construct" in tab_text.lower() or i == 0:  # Construct is usually first tab
    ...
    def _find_tab_widget(self):
        ...
        try:
            ...
            logger.info(f"SEARCH: Found {len(all_children)} total children in main window")
    ...
    def _find_start_position_picker(self, all_children):
        ...
        strategies = [
            ...
            lambda w: "start" in w.__class__.__name__.lower() and "position" in w.__class__.__name__.lower(),
            lambda w: hasattr(w, 'position_selected'),
            lambda w: "picker" in w.__class__.__name__.lower() and "start" in w.__class__.__name__.lower()
        ]
    ...
    def _find_option_picker(self, all_children):
        ...
        strategies = [
            ...
            lambda w: "option" in w.__class__.__name__.lower() and "picker" in w.__class__.__name__.lower(),
            lambda w: hasattr(w, 'option_selected'),
            lambda w: "picker" in w.__class__.__name__.lower() and "option" in w.__class__.__name__.lower()
        ]
            ...
            for widget in all_children:
                try:
                    if strategy(widget):
                        ...
                        logger.info(f"🎛️ Found option picker: {widget.__class__.__name__}")
                        return
                except:
                    continue
    ...
    def _find_workbench(self, all_children):
        ...
        strategies = [
            ...
            lambda w: "sequence" in w.__class__.__name__.lower() and "view" in w.__class__.__name__.lower(),
            lambda w: hasattr(w, 'get_sequence_length'),
            lambda w: hasattr(w, 'add_beat'),
            lambda w: "beat" in w.__class__.__name__.lower() and "view" in w.__class__.__name__.lower()
        ]
    ...
    def test_start_position_selection_flow(self) -> bool:
        ...
        try:
            ...
            if not options_populated:
                logger.error("❌ Option picker was not populated after start position selection")
                return False
    ...
    def test_sequence_building_flow(self) -> bool:
        ...
        try:
            ...
            target_sequence_length = min(3, len(available_options))  # Build sequence of 3 or less
            
            for i in range(target_sequence_length):
                option = available_options[i]
                logger.info(f"🎯 Selecting option {i+1}/{target_sequence_length}: {option}")
                ...
                if new_length != expected_length:
                    logger.error(f"❌ Sequence length mismatch. Expected: {expected_length}, Got: {new_length}")
                    return False
    ...
    def test_sequence_management(self) -> bool:
        ...
        try:
            ...
            if cleared_length != 0:
                logger.error(f"❌ Sequence not properly cleared. Length: {cleared_length}")
                return False
    ...
    def _click_start_position(self, position: str) -> bool:
        ...
        try:
            ...
            if hasattr(self.start_position_picker, 'findChildren'):
                ...
                for widget in position_widgets:
                    ...
                    if position in str(widget_text) or position in str(widget_name):
                        logger.info(f"🎯 Found position widget: {widget.__class__.__name__}")
            ...
            try:
                from desktop.modern.application.services.start_position.start_position_orchestrator import StartPositionOrchestrator
    ...
    def _get_current_sequence_data(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "beats": [{"beat": i, "valid": True} for i in range(self.current_sequence_length)]
            }
