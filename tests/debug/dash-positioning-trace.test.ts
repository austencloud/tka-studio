...
describe("Dash Arrow Positioning Pipeline Trace", () => {
  ...
  function tracePipeline(motion: MotionData, letter: Letter): PositioningTrace {
    ...
    try {
      ...
      trace.step4_arrowLocationCalculation.calculatedLocation =
        dashLocationCalculator.calculateDashLocationFromPictographData(
          pictographData,
          true
        );
    } catch (error) {
    ...
    const quadrantIndex = quadrantCalculator.calculateQuadrantIndex(
      motion,
      location
    );
    ...
    trace.step8_finalAdjustment = {
      ...
      transformationApplied:
        selectedTuple[0] !== baseX || selectedTuple[1] !== baseY,
    };
  ...
  function inferGridMode(motion: MotionData): GridMode {
    ...
    return cardinalSet.has(motion.startLocation) ||
  ...
  function printTrace(trace: PositioningTrace): void {
    ...
    console.log(
      `   Is Dash: ${trace.step1_motionDetection.isDash ? "✅ YES" : "❌ NO"}`
    );

    console.log("\n🔄 STEP 2: Rotation Direction Detection");
    console.log(
      `   Rotation: ${trace.step2_rotationDetection.rotationDirection}`
    );
    console.log(`   Turns: ${trace.step2_rotationDetection.turns}`);
    console.log(
      `   Is CW: ${trace.step2_rotationDetection.isCW ? "✅ YES" : "❌ NO"}`
    );
    console.log(
      `   Is CCW: ${trace.step2_rotationDetection.isCCW ? "✅ YES" : "❌ NO"}`
    );
    console.log(
      `   Is NoRot: ${trace.step2_rotationDetection.isNoRot ? "✅ YES" : "❌ NO"}`
    );
    ...
    console.log(
      `   Inferred Grid Mode: ${trace.step3_gridModeDetection.inferredGridMode}`
    );

    console.log("\n📍 STEP 4: Arrow Location Calculation");
    console.log(
      `   Calculated Location: ${trace.step4_arrowLocationCalculation.calculatedLocation}`
    );

    console.log("\n📦 STEP 5: Base Adjustment");
    console.log(
      `   Base: (${trace.step5_baseAdjustment.x}, ${trace.step5_baseAdjustment.y})`
    );
    console.log(`   Source: ${trace.step5_baseAdjustment.source}`);

    console.log("\n🔢 STEP 6: Directional Tuple Generation");
    console.log(
      `   Base values: (${trace.step6_directionalTuples.baseX}, ${trace.step6_directionalTuples.baseY})`
    );
    ...
    if (trace.step6_directionalTuples.allSameAsBase) {
      console.log(
        "   ⚠️ WARNING: All tuples are identical to base - NO TRANSFORMATION!"
      );
    } else {
    ...
    console.log(
      `   Quadrant Index: ${trace.step7_quadrantIndex.quadrantIndex}`
    );
    console.log(`   Quadrant Name: ${trace.step7_quadrantIndex.quadrantName}`);

    console.log("\n✨ STEP 8: Final Adjustment");
    console.log(
      `   Selected Tuple: (${trace.step8_finalAdjustment.selectedTuple.x}, ${trace.step8_finalAdjustment.selectedTuple.y})`
    );
    console.log(
      `   Final Adjustment: (${trace.step8_finalAdjustment.finalAdjustment.x}, ${trace.step8_finalAdjustment.finalAdjustment.y})`
    );
  ...
  function analyzeTraces(traces: PositioningTrace[]): void {
    ...
    console.log(
      `1. Motion Type Detection: ${allDash ? "✅ All detected as DASH" : "❌ Some NOT detected as DASH"}`
    );

    // Check if all are detected as CW
    const allCW = traces.every((t) => t.step2_rotationDetection.isCW);
    console.log(
      `2. Rotation Detection: ${allCW ? "✅ All detected as CW" : "❌ Some NOT detected as CW"}`
    );

    // Check if all use Diamond mode
    const allDiamond = traces.every(
      (t) => t.step3_gridModeDetection.inferredGridMode === GridMode.DIAMOND
    );
    console.log(
      `3. Grid Mode Detection: ${allDiamond ? "✅ All using DIAMOND mode" : "❌ Mixed grid modes"}`
    );
    ...
    traces.forEach((t) => {
      console.log(
        `   ${t.motionName} → Arrow at: ${t.step4_arrowLocationCalculation.calculatedLocation}`
      );
    });

    // Check if transformations were applied
    const allTransformed = traces.every(
      (t) => t.step6_directionalTuples.transformationApplied
    );
    console.log(
    ...
    const allFinalTransformed = traces.every(
      (t) => t.step8_finalAdjustment.transformationApplied
    );
    console.log(
      `6. Final Adjustment: ${allFinalTransformed ? "✅ All finals transformed" : "❌ Some finals NOT transformed"}`
    );
    ...
    const uniqueFinalAdjustments = new Set(
      traces.map(
        (t) =>
          `(${t.step8_finalAdjustment.finalAdjustment.x}, ${t.step8_finalAdjustment.finalAdjustment.y})`
      )
    );
    if (uniqueFinalAdjustments.size === 1) {
      console.log(
        "   ⚠️ WARNING: All dash arrows have IDENTICAL final adjustments!"
      );
      console.log(
        "   This suggests the quadrant-specific transformation is NOT working."
      );
    } else {
      console.log(
        `   ✅ Found ${uniqueFinalAdjustments.size} different final adjustments (expected: 4)`
      );
    }
