...
class TestServiceRegistrationManager:
    ...
    def setup_method(self):
        ...
        self.service = ServiceRegistrationManager(progress_callback=mock_progress_callback)
    ...
    def test_register_event_system_success(self):
        ...
        with patch('application.services.core.service_registration_manager.get_event_bus') as mock_get_bus:
            mock_event_bus = Mock()
    ...
    def test_register_event_system_import_error(self):
        ...
        with patch('application.services.core.service_registration_manager.get_event_bus', side_effect=ImportError("No module")):
    ...
    def test_register_workbench_services(self):
        ...
        with patch('application.services.core.service_registration_manager.configure_workbench_services') as mock_configure:
            # Test registration
    ...
    def test_register_all_services_sequence(self):
        ...
        with patch.object(self.service, 'register_event_system') as mock_event, \
             ...
             patch.object(self.service, 'register_pictograph_services') as mock_pictograph, \
    ...
    def test_error_handling_in_registration(self):
        ...
        self.mock_container.register_singleton.side_effect = Exception("Registration failed")
    ...
    def test_registration_order_dependency(self):
        ...
        with patch.object(self.service, 'register_event_system', side_effect=track_calls('event')), \
             patch.object(self.service, 'register_core_services', side_effect=track_calls('core')), \
             patch.object(self.service, 'register_motion_services', side_effect=track_calls('motion')), \
             patch.object(self.service, 'register_layout_services', side_effect=track_calls('layout')), \
             patch.object(self.service, 'register_pictograph_services', side_effect=track_calls('pictograph')), \
             patch.object(self.service, 'register_workbench_services', side_effect=track_calls('workbench')):
            
            self.service.register_all_services(self.mock_container)
            
            # Verify correct order
            expected_order = ['event', 'core', 'motion', 'layout', 'pictograph', 'workbench']
