    ...
    try:
        ...
        try:
            with Mock() as mock_panel_factory:
                ...
                mock_panel_factory.create_workbench_panel.return_value = (
                    Mock(),
                    Mock(),
                )
...
async def test_async_event_processing():
    ...
    try:
        ...
        for i in range(num_events):
            event = SequenceUpdatedEvent(
                sequence_id=f"async_{i}", change_type="async_test"
            )
        ...
        if len(async_events_processed) != num_events:
            raise Exception(
                f"Not all events processed: {len(async_events_processed)}/{num_events}"
            )
...
def run_manual_tests():
    ...
    try:
        ...
        runner.results.append(
            {"test": "Async Event Processing", "status": "PASS", "result": async_result}
        )
        print("✅ PASS - Async Event Processing")
        print(f"Result: {async_result}")
    except Exception as e:
        runner.results.append(
            {"test": "Async Event Processing", "status": "FAIL", "error": str(e)}
        )
        print("❌ FAIL - Async Event Processing")
        print(f"Error: {e}")
...
def run_performance_benchmark():
    ...
    try:
        ...
        for i in range(5000):
            event = SequenceUpdatedEvent(
                sequence_id=f"bench_{i}", change_type="benchmark"
            )
            event_bus.publish(event)
        end_time = time.time()
