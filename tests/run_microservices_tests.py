...
class MicroservicesTestRunner:
    ...
    def run_manual_tests(self) -> dict[str, Any]:
        ...
        if not manual_test_path.exists():
            return {"status": "SKIPPED", "reason": "Manual testing suite not found"}
    ...
    def run_phase_1_unit_tests(self):
        ...
        print("\n🧪 PHASE 1: UNIT TESTS")
        print("Testing individual microservices components...")
        ...
        for test_file, test_name in unit_tests:
            ...
            print(
                f"{status_icon} {test_name}: {result['passed']}/{result['tests_run']} passed"
            )

    def run_phase_2_integration_tests(self):
        """Run Phase 2: Integration Tests."""
        print("\n🔗 PHASE 2: INTEGRATION TESTS")
        print("Testing component interactions and event flows...")
    ...
    def run_phase_3_performance_tests(self):
        ...
        performance_tests = [("test_performance.py", "Performance & Load Tests")]
    ...
    def run_phase_4_full_system_tests(self):
        ...
        print("Testing complete microservices ecosystem...")
    ...
    def run_manual_verification(self):
        ...
        print("\n🔍 MANUAL VERIFICATION")
        print("Running manual testing and verification procedures...")
        ...
        status_icon = "✅" if result["status"] == "PASSED" else "❌"
        print(
            f"{status_icon} Manual Testing Suite: {result.get('passed', 0)}/{result.get('tests_run', 0)} passed"
        )
    ...
    def print_detailed_report(self):
        ...
        for i, (phase_name, phase_data) in enumerate(
            zip(
                phase_names,
                [
                    self.results["phase_1_unit"],
            ...
            print(
                f"   Tests: {phase_summary['passed']}/{phase_summary['tests_run']} passed"
            )
            ...
            for test_name, test_result in phase_data.items():
                if isinstance(test_result, dict):
                    ...
                    print(
                        f"   {test_icon} {test_name}: {test_result.get('passed', 0)}/{test_result.get('tests_run', 0)}"
                    )
        ...
        performance_result = self.results["phase_3_performance"].get(
        ...
        # Manual verification
        manual_result = self.results["manual_verification"].get(
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="TKA Microservices Infrastructure Test Runner"
    )
    parser.add_argument(
        "--project-root", default=r"f:\CODE\TKA", help="Project root directory"
    )
    parser.add_argument(
        "--phase",
        choices=["1", "2", "3", "4", "manual", "all"],
        default="all",
