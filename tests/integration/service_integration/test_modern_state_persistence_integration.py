...
def test_modern_state_persistence():
    ...
    try:
        ...
        success = settings_service.execute_setting_command(
            "test", "demo_key", "demo_value"
        )
        ...
        for i in range(100):
            settings_service.execute_setting_command(
                "perf_test", f"key_{i}", f"value_{i}"
            )
        ...
        print(
            f"✅ Performance test: 200 operations in {duration:.3f}s ({200 / duration:.1f} ops/sec)"
        )
