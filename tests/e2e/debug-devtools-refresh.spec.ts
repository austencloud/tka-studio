...
test.describe('DevTools Refresh White Screen Investigation', () => {
  test('Scenario 1: Normal load (baseline)', async ({ page }) => {
    ...
    await page.screenshot({ path: 'debug-scenario-1-normal-load.png', fullPage: true });
    console.log('Screenshot saved: debug-scenario-1-normal-load.png');
  });

  test('Scenario 2: Load with DevTools open from start', async ({ page, context }) => {
    const consoleMessages: string[] = [];
    const errors: string[] = [];
    const warnings: string[] = [];
    ...
    await page.screenshot({ path: 'debug-scenario-2-devtools-open.png', fullPage: true });
    console.log('Screenshot saved: debug-scenario-2-devtools-open.png');
  });

  test('Scenario 3: Refresh with DevTools open (FAILING CASE)', async ({ page, context }) => {
    const consoleMessages: string[] = [];
    const errors: string[] = [];
    const warnings: string[] = [];
    ...
    await page.screenshot({ path: 'debug-scenario-3-refresh-failing.png', fullPage: true });
