...
test.describe("Direct Background Change Test", () => {
  test("should change background when directly calling updateBodyBackground", async ({
    page,
  }) => {
    ...
    const initialState = await page.evaluate(() => {
      return {
        ...
        gradientNext:
          document.documentElement.style.getPropertyValue("--gradient-next"),
        bodyClasses: document.body.className,
      };
    });

    console.log(
      "   Initial --gradient-cosmic:",
      initialState.gradientCosmic || "none"
    );
    ...
    const result = await page.evaluate(async () => {
      try {
        // Import the module
        const { updateBodyBackground, BackgroundType } = await import(
          "$lib/shared/background"
        );

        console.log("🧪 TEST: Successfully imported background module");
        console.log(
          "🧪 TEST: Available BackgroundType values:",
    ...
    const finalState = await page.evaluate(() => {
      return {
        ...
        gradientNext:
          document.documentElement.style.getPropertyValue("--gradient-next"),
        bodyClasses: document.body.className,
        bodyHasTransitioningClass: document.body.classList.contains(
    ...
    const gradientChanged =
      finalState.gradientCosmic.includes("#667eea") ||
    ...
    if (gradientChanged && hasAuroraClass) {
      ...
      expect(gradientChanged, "Gradient should contain Aurora colors").toBe(
        true
      );
