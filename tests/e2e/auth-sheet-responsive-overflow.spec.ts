...
test.describe("Auth Sheet Responsive Layout", () => {
  ...
  async function measureAuthSheetOverflow(page: any) {
    ...
    const elementsInBounds = await container.evaluate((el: HTMLElement) => {
      ...
      return {
        fitsInViewport:
          containerRect.bottom <= viewportHeight && containerRect.top >= 0,
  ...
  for (const device of testDevices) {
    test(`${device.name} - Sign In mode should fit without overflow`, async ({
      page,
    }) => {
      // Set viewport
      ...
      console.log(
        `  Viewport: ${measurements.viewport.width}x${measurements.viewport.height}px`
      );
      console.log(
        `  Container height: ${measurements.container.height.toFixed(0)}px`
      );
      console.log(
        `  Container scroll height: ${measurements.container.scrollHeight}px`
      );
      console.log(
        `  Container has overflow: ${measurements.container.hasVerticalOverflow}`
      );
      console.log(
        `  Content scrollable: ${measurements.content?.isScrollable}`
      );
      console.log(`  Fits in viewport: ${measurements.bounds.fitsInViewport}`);
      console.log(
        `  Header height: ${measurements.header?.height.toFixed(0)}px`
      );
      ...
      if (!measurements.bounds.fitsInViewport) {
        console.log(
          `  ⚠️ Bottom overflow: ${measurements.bounds.bottomOverflow.toFixed(0)}px`
        );
      }
      ...
      if (device.expectSubtitleVisible) {
        ...
        const subtitleDisplay = await subtitle.evaluate(
      ...
      console.log(
        `  Content area scrollable: ${measurements.content?.isScrollable ? "✓ (expected)" : "✓ (no scroll needed)"}`
      );

      // 6. No element should overflow the bottom of viewport
      if (measurements.bounds.bottomOverflow > 0) {
        console.log(
          `  ❌ OVERFLOW: ${measurements.bounds.bottomOverflow.toFixed(0)}px extends beyond viewport`
        );
      }
    ...
    test(`${device.name} - Sign Up mode should fit without overflow`, async ({
      page,
    }) => {
      // Set viewport
      await page.setViewportSize({
  ...
  test("OAuth buttons visible in both sign-in and sign-up modes", async ({
    page,
  }) => {
  ...
  test("Touch targets meet WCAG minimum size on smallest device", async ({
    page,
  }) => {
    ...
    const buttons = page.locator(
      ".submit-button, .toggle-button, .password-toggle, .auth-sheet__close"
    );
    ...
    for (let i = 0; i < buttonCount; i++) {
      ...
      console.log(
        `  ${size.className}: ${size.width.toFixed(0)}x${size.height.toFixed(0)}px`
      );
  ...
  test("Content remains accessible when scrolling is needed", async ({
    page,
  }) => {
  ...
  test("Dynamic viewport height (dvh) adapts to keyboard on mobile", async ({
    page,
  }) => {
    await page.setViewportSize({ width: 375, height: 667 }); // iPhone SE 2/3
    await page.goto("/");
    ...
    console.log(
      `  computed height: ${initialHeight.computedHeight.toFixed(0)}px`
    );
