...
test.describe('Edit Panel Mobile Animation Test', () => {
	test('should observe edit panel animation at Z Fold 5 viewport (344x882)', async ({ page }) => {
		...
		const constructButton = page.locator('button:has-text("Construct")').first();
		...
		const startPositions = page.locator('[data-testid="start-position-option"]').or(page.locator('button:has-text("α")'));
		if (await startPositions.first().isVisible({ timeout: 3000 }).catch(() => false)) {
		...
		if (await variationsButton.isVisible({ timeout: 3000 }).catch(() => false)) {
			...
			if (await firstVariation.isVisible({ timeout: 3000 }).catch(() => false)) {
		...
		if (beatCount > 0) {
			...
			if (await editButton.isVisible()) {
				...
				const panelVisibleBefore = await editPanel.isVisible().catch(() => false);
				console.log(`📋 Edit panel visible before: ${panelVisibleBefore}`);

				// Take screenshot before clicking
				await page.screenshot({ path: 'before-edit-panel.png', fullPage: true });
				...
				await page.screenshot({ path: 'during-edit-panel-animation.png', fullPage: true });
				...
				if (panelBounds) {
					...
					console.log(`   - Bottom edge: ${panelBounds.y + panelBounds.height}`);
				}
				...
				console.log(`🔍 Has 'mobile' class: ${panelClasses?.includes('mobile')}`);
				...
				if (panelBounds) {
					const isFullWidth = Math.abs(panelBounds.width - (viewport?.width || 0)) < 10;
					const isFullHeight = Math.abs(panelBounds.height - (viewport?.height || 0)) < 10;
					const isAtRight = panelBounds.x + panelBounds.width >= (viewport?.width || 0) - 5;
					const isAtBottom = panelBounds.y + panelBounds.height >= (viewport?.height || 0) - 5;
					...
					if (isFullHeight && isAtRight) {
						console.log('   ⚠️ DETECTED: Panel slides from RIGHT (current behavior)');
					} else if (isFullWidth && isAtBottom) {
						console.log('   ✅ DETECTED: Panel slides from BOTTOM (desired behavior)');
					}
				}

				console.log('\n📋 SUMMARY:');
				console.log('   The edit panel is currently sliding in from the right side.');
				console.log('   For mobile devices (344x882), it should slide up from the bottom.');
