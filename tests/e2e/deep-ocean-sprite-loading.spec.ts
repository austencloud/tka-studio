...
test.describe('Deep Ocean Fish Sprite Loading', () => {
  test('should load fish sprites and display them correctly', async ({ page }) => {
    ...
    const fishLogs = consoleLogs.filter(log =>
      log.includes('🐟') ||
      log.includes('fish') ||
      log.includes('sprite') ||
      log.includes('Cache') ||
    ...
    const deepOceanInit = consoleLogs.some(log => log.includes('Deep Ocean background initialized'));
    console.log(`Deep Ocean initialized: ${deepOceanInit}`);

    // Check if preload was called
    const preloadCalled = consoleLogs.some(log => log.includes('Preloading') && log.includes('fish sprites'));
    console.log(`Preload called: ${preloadCalled}`);

    // Check if any sprites loaded
    const spritesLoaded = consoleLogs.some(log => log.includes('Fish sprite loaded:'));
    ...
    // Check fish creation
    const fishCreated = consoleLogs.filter(log => log.includes('Creating fish'));
    console.log(`Fish created: ${fishCreated.length}`);

    // Check fish updates
    const fishUpdated = consoleLogs.filter(log => log.includes('Fish sprite updated to'));
    console.log(`Fish sprites updated: ${fishUpdated.length}`);

    // Check fallback rendering
    const fallbackRendering = consoleLogs.filter(log => log.includes('Fish fallback rendering'));
