...
test.describe("Animation Panel Slide Animation", () => {
  ...
  test("should slide up smoothly when play button is clicked", async ({ page }) => {

    // 1. Navigate to Build module
    const buildTab = page.locator(".nav-tab-container").filter({ hasText: "Build" });
    ...
    await page.screenshot({ path: 'test-results/before-play.png', fullPage: false });
    ...
    await page.screenshot({ path: 'test-results/after-play.png', fullPage: false });
    ...
    console.log('📊 Animation test complete - check screenshots in test-results/');
  });

  test("sequence actions sheet should slide up smoothly for comparison", async ({ page }) => {
    ...
    await page.screenshot({ path: 'test-results/before-actions.png', fullPage: false });

    await actionsButton.click();

    await page.waitForTimeout(400);
    await page.screenshot({ path: 'test-results/after-actions.png', fullPage: false });
