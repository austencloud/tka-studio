...
test('Library Save & Reload Flow', async ({ page }) => {
  ...
  const actionsButton = page.locator('button:has-text("Actions"), [data-testid="sequence-actions"]').first();
  await actionsButton.click().catch(() => {
    // Try three-dot menu
    page.click('[data-testid="more-options"], button:has-text("⋮")').catch(() => {});
  });
  await page.waitForTimeout(500);

  // Click Save
  const saveButton = page.locator('button:has-text("Save"), [data-testid="save-sequence"]').first();
  ...
  await page.click('button:has-text("Sequences"), [data-testid="sequences-tab"]').catch(() => {});
  await page.waitForTimeout(500);

  // Step 4: Find the saved sequence
  const sequenceCards = page.locator('[data-testid="sequence-card"], [data-testid="sequence-item"]');
  ...
  const loadButton = page.locator('button:has-text("Load"), button:has-text("Edit"), button:has-text("Open")').first();
  ...
  // Step 7: Re-save
  await page.click('button:has-text("Actions"), [data-testid="sequence-actions"]').catch(() => {
    page.click('[data-testid="more-options"], button:has-text("⋮")').catch(() => {});
  });
  await page.waitForTimeout(300);
