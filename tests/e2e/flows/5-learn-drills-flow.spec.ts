...
test('Learn Drills Flow', async ({ page }) => {
  ...
  await expect(page.locator('[data-testid="drill-card"], [data-testid="flashcard"]')).toBeVisible();
  ...
  for (let i = 0; i < 5; i++) {
    ...
    const answerOption = page.locator('[data-testid="answer-option"], [data-testid="drill-answer"] button').first();
    await answerOption.click();
    await page.waitForTimeout(500);

    // Step 4: See result (correct/incorrect feedback should appear)
    const resultFeedback = page.locator('[data-testid="result-feedback"], text=Correct, text=Incorrect');
    await expect(resultFeedback.first()).toBeVisible({ timeout: 2000 }).catch(() => {});
    await page.waitForTimeout(800);

    // Step 5: Proceed to next drill
    const nextButton = page.locator('button:has-text("Next"), button:has-text("Continue"), [data-testid="next-drill"]').first();
