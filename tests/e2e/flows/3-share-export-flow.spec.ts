...
test("Share/Export Flow - GIF Export", async ({ page }) => {
  ...
  const shareButton = page
    .locator('button:has-text("Share"), [data-testid="share-button"]')
    .first();
  ...
  await expect(
    page.locator('[data-testid="share-panel"], text=Share')
  ).toBeVisible();

  // Step 2: Select GIF export option
  const gifOption = page.locator(
    'button:has-text("GIF"), [data-testid="export-gif"]'
  );
  ...
  await page
    .click('[data-testid="size-small"], button:has-text("Small")')
    .catch(() => {});
  await page.waitForTimeout(200);

  // Adjust quality if controls exist
  await page
    .click('[data-testid="quality-high"], button:has-text("High")')
    .catch(() => {});
  await page.waitForTimeout(200);
  ...
  // Close share panel
  await page
    .click('button:has-text("Close"), [data-testid="close-share"]')
    .catch(() => {});
