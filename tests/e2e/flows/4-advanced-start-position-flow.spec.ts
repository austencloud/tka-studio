...
test("Advanced Start Position Picker Flow", async ({ page }) => {
  ...
  const advancedToggle = page
    .locator(
      'button:has-text("Advanced"), button:has-text("Show All"), [data-testid="toggle-advanced"]'
    )
    .first();
  ...
  await expect(
    page.locator('[data-testid="advanced-start-position-picker"]')
  ).toBeVisible();

  // Step 2: Browse through all 16 variations (scroll through grid)
  const advancedGrid = page.locator(
    '[data-testid="advanced-start-position-picker"]'
  );
  ...
  const advancedPosition = page
    .locator('[data-testid="advanced-start-position-picker"] button')
    .nth(4);
  ...
  const beatCount = await page
    .locator('[data-testid="beat-grid"] [data-beat-number]')
    .count();
  expect(beatCount).toBeGreaterThan(0);

  // Step 5: Clear and return to default picker
  const clearButton = page
    .locator('button:has-text("Clear"), [data-testid="clear-button"]')
    .first();
  ...
  await page
    .click('button:has-text("Confirm"), button:has-text("Yes")')
    .catch(() => {});
  await page.waitForTimeout(500);
  ...
  await page
    .click(
      'button:has-text("Default"), button:has-text("Hide"), [data-testid="toggle-default"]'
    )
    .catch(() => {});
  await page.waitForTimeout(300);
