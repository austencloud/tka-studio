...
test('Mobile PWA Install Flow', async ({ browser }) => {
  ...
  const installPrompt = page.locator('[data-testid="pwa-install-prompt"], [data-testid="mobile-install-prompt"]');
  ...
  if (await installPrompt.isVisible({ timeout: 3000 }).catch(() => false)) {
    ...
    const dismissButton = page.locator('button:has-text("Dismiss"), button:has-text("Later"), button:has-text("Not Now")').first();
    ...
    const installGuide = page.locator('[data-testid="pwa-install-guide"], text=Install Instructions');
    if (await installGuide.isVisible({ timeout: 2000 }).catch(() => false)) {
      ...
      // Close guide
      await page.click('button:has-text("Close"), button:has-text("Got it")').catch(() => {});
    ...
    if (pwaDetected) {
      ...
      console.log('ℹ️  App not in standalone mode (normal for desktop browser)');
    }
  } else {
    console.log('ℹ️  PWA install prompt did not appear (may require HTTPS or already installed)');
  }

  // Verify app is still functional on mobile
  await expect(page.locator('[data-testid="menu-button"], button:has-text("Menu")')).toBeVisible();
