...
test('Generate Flow - Basic Auto-Creation', async ({ page }) => {
  ...
  const generateButton = page.locator('.main-tab-btn').filter({ hasText: 'Generate' });
  ...
  const circularOption = genModeCard.locator('.toggle-option').filter({ hasText: 'Circular' });
  ...
  const generateSequenceButton = page.locator('button').filter({ hasText: /generate/i }).first();
  await generateSequenceButton.click();
  await page.waitForTimeout(1500);

  // Step 5: Play the generated sequence
  const animateButton = page.locator('.main-tab-btn').filter({ hasText: 'Animate' });
