...
test('Complete Construct Flow', async ({ page }) => {
  ...
  await expect(page.locator('[data-testid="construct-tab-content"]')).toBeVisible();

  // Step 1: Select a start position
  const startPositionPicker = page.locator('[data-testid="start-position-picker"]');
  ...
  const constructButton = page.locator('.main-tab-btn').filter({ hasText: 'Construct' });
