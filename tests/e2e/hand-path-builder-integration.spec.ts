...
test.describe("Hand Path Builder Integration", () => {
  test.beforeEach(async ({ page }) => {
    ...
    const gesturalTab = page
      .locator(
        '[data-build-mode-id="gestural"], button:has-text("Hand Path"), button:has-text("Gestural")'
      )
      .first();
  ...
  test('should show "Configure Your Settings" message in top bar', async ({
    page,
  }) => {
  ...
  test('should show workspace after clicking "Start Drawing"', async ({
    page,
  }) => {
  ...
  test("sequence length picker should be visible before starting", async ({
    page,
  }) => {
    // Sequence length picker should be in the tool panel
    const lengthPicker = page.locator(
      '.sequence-length-picker, .length-picker, [data-testid="sequence-length-picker"]'
    );
