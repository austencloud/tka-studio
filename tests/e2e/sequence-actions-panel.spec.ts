...
test.describe("Sequence Actions Panel", () => {
  ...
  test("should open and close via button click", async ({ page }) => {
    ...
    const actionsPanel = page
      .locator('.actions-panel, [role="dialog"]')
      .filter({
    ...
    const closeButton = actionsPanel
      .locator('button[aria-label*="close" i], button .fa-times')
      .first();
  ...
  test("should reopen after swipe gesture close", async ({ page }) => {
    ...
    if (!sequenceActionsButton) {
      console.log(
        "⚠️ Could not find sequence actions button, trying generic approach"
      );
    ...
    await page.screenshot({
      path: "test-screenshots/after-reopen-attempt.png",
    });
    ...
    const openLog = logs.find((log) =>
      log.includes("🟢 handleOpenSequenceActions")
    );
    if (openLog) {
  ...
  test("should track state changes in console", async ({ page }) => {
    ...
    expect(logs.some((log) => log.includes("📊") || log.includes("🟢"))).toBe(
      true
    );

    // Close panel via button
    const closeButton = page
      .locator('.actions-panel button[aria-label*="close" i]')
      .first();
    await closeButton.click();
    await page.waitForTimeout(500);
    ...
    console.log(
      `Open logs (🟢): ${logs.filter((l) => l.includes("🟢")).length}`
    );
    console.log(
      `State change logs (📊): ${logs.filter((l) => l.includes("📊")).length}`
    );
    console.log(
      `Close logs (🔴): ${logs.filter((l) => l.includes("🔴")).length}`
    );
  });
});
