...
test.describe("Settings UI Background Change", () => {
  test("should change background through Settings UI exactly as user does", async ({
    page,
  }) => {
    ...
    const dialogVisible = await page
      .locator('[class*="settings"], [class*="dialog"]')
      .first()
      .isVisible();
    ...
    const backgroundElements = await page
      .locator('*:has-text("Background")')
      .all();
    console.log(
      `   Found ${backgroundElements.length} elements with "Background" text`
    );
    ...
    const hasAuroraGradient =
      finalBg.cosmic.includes("#667eea") || finalBg.next.includes("#667eea");
    ...
    console.log(
      ...
      finalBg.storedBackgroundType === "aurora"
    );

    // Check console logs for updateBodyBackground calls
    const updateCalls = logs.filter((l) =>
      l.includes("updateBodyBackground called with")
    );
    ...
    if (!hasAuroraGradient && !hasAuroraClass) {
      ...
      await page.screenshot({
        path: "test-results/background-not-changed.png",
        fullPage: true,
      });
      ...
      console.log(
        "   Gradient:",
        hasAuroraGradient ? "Changed" : "Not changed"
      );
