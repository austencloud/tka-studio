...
test.describe("SequenceActionsSheet Reopen Issue", () => {
  test.beforeEach(async ({ page }) => {
    ...
    const buttonExists = await page
      .locator('[data-testid="sequence-actions-button"]')
      .count();
    if (buttonExists === 0) {
  ...
  test("should reopen SequenceActionsSheet after swiping it away", async ({
    page,
    browserName,
  }) => {
    ...
    if (!isVisible) {
      console.log(
        "⚠️ SKIPPING TEST: Sequence actions button not visible (likely no sequence loaded)"
      );
      test.skip();
      return;
    }
    ...
    const sheetVisible = await page.isVisible(
    ...
    const sheetReopened = await page.isVisible(
      '[data-testid="sequence-actions-sheet"]'
    );
  ...
  test("should reopen SequenceActionsSheet after clicking close button", async ({
    page,
  }) => {
