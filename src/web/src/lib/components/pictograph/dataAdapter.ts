...
export function legacyToModernPictographData(
  ...
  return {
    ...
    motions: {
      blue:
        (legacy as { motions?: { blue?: MotionData } }).motions?.blue ||
        createMotionData(),
      red:
        (legacy as { motions?: { red?: MotionData } }).motions?.red ||
        createMotionData(),
    },
...
export function legacyToModernArrowData(
  ...
  const calculatedLocation = calculateArrowLocation({
    start_loc:
      (legacy.startLoc as string) || (legacy.start_loc as string) || "",
    end_loc: (legacy.endLoc as string) || (legacy.end_loc as string) || "",
    motion_type:
      (legacy.motionType as string) ||
      (legacy.motion_type as string) ||
      "static",
  });
  ...
  return {
    ...
    location:
      calculatedLocation ||
      (legacy.loc as string) ||
      (legacy.location as string) ||
      "center",
    start_orientation:
      (legacy.startOri as string) ||
      (legacy.start_orientation as string) ||
      "in",
    end_orientation:
      (legacy.endOri as string) || (legacy.end_orientation as string) || "in",
    rotation_direction:
      (legacy.propRotDir as string) ||
      (legacy.rotation_direction as string) ||
      "clockwise",
    turns: (legacy.turns as number) || 0,
    is_mirrored:
      (legacy.svgMirrored as boolean) ||
      (legacy.is_mirrored as boolean) ||
      false,
    position_x:
      (legacy.coords as { x?: number })?.x ||
      (legacy.coordinates as { x?: number })?.x ||
      0,
    position_y:
      (legacy.coords as { y?: number })?.y ||
      (legacy.coordinates as { y?: number })?.y ||
      0,
    rotation_angle:
      (legacy.rotAngle as number) || (legacy.rotation_angle as number) || 0,
    coordinates:
    ...
    svg_mirrored:
      (legacy.svgMirrored as boolean) ||
      (legacy.svg_mirrored as boolean) ||
      false,
    is_visible:
      legacy.is_visible !== undefined ? (legacy.is_visible as boolean) : true,
...
export function legacyToModernPropData(
  ...
  return {
    id: (legacy.id as string) || crypto.randomUUID(),
    prop_type:
      (legacy.propType as PropType) ||
      (legacy.prop_type as PropType) ||
      PropType.STAFF,
    color,
    orientation:
      (legacy.ori as Orientation) ||
      (legacy.orientation as Orientation) ||
      Orientation.IN,
    rotation_direction:
...
export function ensureModernPictographData(
  ...
  if (
    ...
    return {
      ...
      letter_type:
        typeof data.letter_type === "string"
          ? (data.letter_type as LetterType)
          : null,
      is_blank: typeof data.is_blank === "boolean" ? data.is_blank : false,
      is_mirrored:
        typeof data.is_mirrored === "boolean" ? data.is_mirrored : false,
      metadata:
