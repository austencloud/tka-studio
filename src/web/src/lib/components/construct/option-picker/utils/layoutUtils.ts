...
export const getResponsiveLayout = memoizeLRU(
  (
    ...
    let scaleFactor =
      deviceConfig?.scaleFactor ?? (isMobileDevice ? 0.95 : 1.0);
...
function getGridGap(params: {
  ...
  const deviceConfig =
    ...
    DEVICE_CONFIG.desktop;

  let gapSize =
    deviceConfig.gap +
    (GAP_ADJUSTMENTS[layoutCategory as keyof typeof GAP_ADJUSTMENTS] || 0);
...
const calculateOptionSize = memoizeLRU(
  (config: {
    ...
    const totalVerticalGap =
      Math.max(0, Math.ceil(count / columns) - 1) * gapSize;
    const availableWidth =
      containerWidth - horizontalPadding - totalHorizontalGap;
    const availableHeight =
      containerHeight - verticalPadding - totalVerticalGap;
...
function getBaseColumnCount(
  ...
  if (layoutCategory === "twoItems") {
    ...
    const useVerticalLayout =
      aspect === "tall" || (aspect === "square" && isPortrait);
