	...
	<div class="display-content">
		...
		{:else if !state.state.selectedThumbnail && !state.state.metadataStats}
			<div class="empty-state">
				...
				<p>Click on a thumbnail to extract and analyze its metadata, or use "Batch Analyze" to analyze all sequences</p>
			</div>
		{:else if state.state.metadataStats}
				...
				<div class="batch-summary">
					...
					<div class="summary-grid">
						<div class="summary-card">
							...
							<div class="summary-value">{state.state.metadataStats.batchSummary.sequencesAnalyzed}</div>
						</div>
						
						<div class="summary-card healthy">
							<div class="summary-title">Healthy Sequences</div>
							<div class="summary-value">{state.state.metadataStats.batchSummary.healthySequences}</div>
							<div class="summary-subtitle">({Math.round(state.state.metadataStats.batchSummary.healthySequences / state.state.metadataStats.batchSummary.sequencesAnalyzed * 100)}%)</div>
						</div>
						
						<div class="summary-card unhealthy">
							<div class="summary-title">Unhealthy Sequences</div>
							<div class="summary-value">{state.state.metadataStats.batchSummary.unhealthySequences}</div>
							<div class="summary-subtitle">({Math.round(state.state.metadataStats.batchSummary.unhealthySequences / state.state.metadataStats.batchSummary.sequencesAnalyzed * 100)}%)</div>
						</div>
						
						<div class="summary-card">
							<div class="summary-title">Average Health Score</div>
							<div class="summary-value">{state.state.metadataStats.batchSummary.averageHealthScore}%</div>
						</div>
					</div>
					...
					<div class="summary-sections">
						...
						{#if state.state.metadataStats.batchSummary.totalErrors > 0}
							<div class="error-summary">
								<h4>🚨 Most Common Errors ({state.state.metadataStats.batchSummary.totalErrors} total)</h4>
								<ul class="issue-list">
						...
						{#if state.state.metadataStats.batchSummary.totalWarnings > 0}
							<div class="warning-summary">
								<h4>⚠️ Most Common Warnings ({state.state.metadataStats.batchSummary.totalWarnings} total)</h4>
								<ul class="issue-list">
								...
								<ul class="ranking-list">
									{#each state.state.metadataStats.batchSummary.worstSequences as sequence}
										<li class="ranking-item">
											...
											<span class="health-score score-{Math.floor(sequence.healthScore / 20)}">{sequence.healthScore}%</span>
										</li>
									{/each}
								</ul>
							</div>
			...
			{:else}
				...
				<div class="metadata-summary">
					<h3>📈 Analysis for {state.state.selectedThumbnail?.word || 'Unknown'}</h3>
				
				<!-- Health Score -->
				<div class="health-score" class:excellent={state.state.metadataStats.healthScore >= 90}
					 class:good={state.state.metadataStats.healthScore >= 70 && state.state.metadataStats.healthScore < 90}
					 class:warning={state.state.metadataStats.healthScore >= 50 && state.state.metadataStats.healthScore < 70}
					 class:poor={state.state.metadataStats.healthScore < 50}>
					<div class="score-circle">
						<span class="score-value">{state.state.metadataStats.healthScore}</span>
					...
					<div class="score-details">
						...
						<p class="issue-count">
							{state.state.metadataStats.errorCount} errors, {state.state.metadataStats.warningCount} warnings
						</p>
					</div>
				</div>
				...
				<div class="stats-grid">
					<div class="stat-item">
						...
						<span class="stat-value">{state.state.metadataStats.realBeatsCount}</span>
					</div>
					<div class="stat-item">
					...
					<div class="stat-item">
						...
						<span class="stat-value">{state.state.metadataStats.startPositionCount}</span>
					</div>
					<div class="stat-item">
						<span class="stat-label">Author:</span>
						<span class="stat-value" class:error={state.state.metadataStats.authorMissing}>
					...
					<div class="stat-item">
						...
						<span class="stat-value" class:error={state.state.metadataStats.levelMissing || state.state.metadataStats.levelZero}>
							{state.state.metadataStats.hasLevel ? 
								(state.state.metadataStats.levelZero ? '⚠️ 0 (needs calculation)' : state.state.metadataStats.level) : 
								'❌ Missing'
							}
						</span>
					</div>
					<div class="stat-item">
						...
						<span class="stat-value" class:error={state.state.metadataStats.startPositionMissing}>
				...
				<div class="issues-section">
					...
					{#if state.state.metadataStats.hasErrors}
						<div class="errors-panel">
							<h4>❌ Critical Errors ({state.state.metadataStats.errorCount})</h4>
							<ul class="issue-list">
								{#if state.state.metadataStats.authorMissing}
									<li class="error-item">Missing author information - required for Browse tab filtering</li>
								{/if}
								{#if state.state.metadataStats.levelMissing}
									<li class="error-item">Missing difficulty level - affects sequence sorting</li>
								{/if}
								{#if state.state.metadataStats.levelZero}
									<li class="error-item">Level is 0 - indicates difficulty calculation needed</li>
								{/if}
								{#if state.state.metadataStats.startPositionMissing}
									<li class="error-item">Missing start position - affects sequence validation</li>
								{/if}
								{#if state.state.metadataStats.missingLetters.length > 0}
									<li class="error-item">Missing beat letters in beats: {state.state.metadataStats.missingLetters.join(', ')}</li>
								{/if}
								{#if state.state.metadataStats.missingMotionData.length > 0}
									<li class="error-item">Missing motion data in beats: {state.state.metadataStats.missingMotionData.join(', ')}</li>
								{/if}
								{#if state.state.metadataStats.missingRequiredFields.length > 0}
									<li class="error-item">Missing required fields ({state.state.metadataStats.missingRequiredFields.length} instances)</li>
								{/if}
							</ul>
						</div>
					{/if}
					...
					{#if state.state.metadataStats.hasWarnings}
						<div class="warnings-panel">
							<h4>⚠️ Warnings ({state.state.metadataStats.warningCount})</h4>
							<ul class="issue-list">
								{#if state.state.metadataStats.authorInconsistent}
									<li class="warning-item">Author information is inconsistent across beats</li>
								{/if}
								{#if state.state.metadataStats.levelInconsistent}
									<li class="warning-item">Level information is inconsistent across beats</li>
								{/if}
								{#if state.state.metadataStats.startPositionInconsistent}
									<li class="warning-item">Multiple different start positions found</li>
								{/if}
								{#if state.state.metadataStats.duplicateBeats.length > 0}
									<li class="warning-item">Duplicate beat numbers detected: {state.state.metadataStats.duplicateBeats.join(', ')}</li>
								{/if}
								{#if state.state.metadataStats.invalidMotionTypes.length > 0}
									<li class="warning-item">{state.state.metadataStats.invalidMotionTypes.length} invalid motion types found</li>
								{/if}
							</ul>
						</div>
					{/if}
