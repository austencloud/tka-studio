...
export function createBrowseSequenceMetadata(sequence: SequenceData): BrowseSequenceMetadata {
	return {
		id: sequence.id,
		name: sequence.name,
		word: sequence.word,
		thumbnails: [...sequence.thumbnails],
		...(sequence.sequence_length !== undefined && { sequenceLength: sequence.sequence_length }),
		...(sequence.author !== undefined && { author: sequence.author }),
		...(sequence.level !== undefined && { level: sequence.level }),
		...(sequence.date_added !== undefined && { dateAdded: sequence.date_added }),
...
export function updateBrowseState(state: BrowseState, updates: Partial<BrowseState>): BrowseState {
