...
export class LessonConfigService {
	 ...
	 */
	static supportsQuestionFormat(lessonType: LessonType, format: QuestionFormat): boolean {
	 ...
	 */
	static supportsAnswerFormat(lessonType: LessonType, format: AnswerFormat): boolean {
		const config = this.getLessonConfig(lessonType);
