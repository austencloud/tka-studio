...
export class QuestionGeneratorService {
	...
	static generateQuestion(
		...
		switch (lessonType) {
			case LessonType.PICTOGRAPH_TO_LETTER:
				return this.generatePictographToLetterQuestion(questionId, pictographDataset);
			case LessonType.LETTER_TO_PICTOGRAPH:
				return this.generateLetterToPictographQuestion(questionId, pictographDataset);
			case LessonType.VALID_NEXT_PICTOGRAPH:
				return this.generateValidNextPictographQuestion(questionId, pictographDataset);
			default:
	...
	private static generatePictographToLetterQuestion(
		...
		const wrongLetters = this.generateWrongLetters(correctLetter, availableLetters, 3);
	...
	private static generateLetterToPictographQuestion(
		...
		const wrongPictographs = this.generateWrongPictographs(correctLetter, availableLetters, 3);
