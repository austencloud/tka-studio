...
import type { BrowseSequenceMetadata, ISequenceIndexService } from '$lib/services/interfaces';
...
export class SequenceIndexService implements ISequenceIndexService {
	...
	async getSuggestions(partialQuery: string, maxSuggestions = 10): Promise<string[]> {
	...
	private addToIndex(index: Map<string, Set<string>>, key: string, sequenceId: string): void {
		if (!index.has(key)) {
	...
	private calculateRelevanceScore(sequence: BrowseSequenceMetadata, query: string): number {
		let score = 0;
	...
	private async scanSequenceDirectory(): Promise<BrowseSequenceMetadata[]> {
		...
		for (let i = 0; i < knownSequences.length; i++) {
			...
			result.dateAdded = new Date(Date.now() - Math.random() * 365 * 24 * 60 * 60 * 1000);
