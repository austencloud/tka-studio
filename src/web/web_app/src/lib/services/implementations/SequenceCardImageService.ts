...
export class SequenceCardImageService implements ISequenceCardImageService {
	 ...
	 */
	async generateSequenceCardImage(sequence: SequenceData, options: ExportOptions): Promise<Blob> {
		try {
	...
	async generateBatchImages(
		...
		try {
			console.log(`Starting batch image generation for ${sequences.length} sequences`);
			...
			for (let i = 0; i < sequences.length; i++) {
				...
				try {
					...
					console.error(`Failed to generate image for sequence "${sequence.name}":`, error);
			...
			console.log(`Batch generation completed. ${results.filter(r => r.success).length}/${sequences.length} successful`);
			return results;
	 ...
	 */
	async getCachedImage(sequenceId: string, options: ExportOptions): Promise<Blob | null> {
	 ...
	 */
	async preloadImages(sequences: SequenceData[], options: ExportOptions): Promise<void> {
		...
		const preloadPromises = sequences.map(async (sequence) => {
			try {
				...
				console.warn(`Failed to preload image for sequence "${sequence.name}":`, error);
			}
		});
	 ...
	 */
	async getCacheStats(): Promise<{ size: number; count: number; hitRate: number }> {
	...
	private async generateImageFromSequence(
		...
		try {
			...
			const beatSVGs = await Promise.all(
				sequence.beats.map(beat => this.pictographRenderingService.renderBeat(beat))
			);

			// 2. Compose sequence layout SVG
			const sequenceSVG = await this.composeSequenceLayout(beatSVGs, sequence, options);

			// 3. Add metadata overlays (title, beat numbers, etc.)
			const finalSVG = await this.addMetadataOverlays(sequenceSVG, sequence, options);
	...
	private async composeSequenceLayout(
		...
		beatSVGs.forEach((beatSVG, index) => {
			...
			const beatGroup = document.createElementNS('http://www.w3.org/2000/svg', 'g');
			beatGroup.setAttribute('transform', `translate(${x}, ${y})`);
			
			// Scale beat SVG to fit beatSize
			const beatContainer = document.createElementNS('http://www.w3.org/2000/svg', 'svg');
			beatContainer.setAttribute('width', beatSize.toString());
			beatContainer.setAttribute('height', beatSize.toString());
			beatContainer.setAttribute('viewBox', beatSVG.getAttribute('viewBox') || '0 0 100 100');
	...
	private async addMetadataOverlays(
		...
		sequenceGroup.setAttribute('transform', `translate(${padding}, ${padding * 2})`);
		...
		if (options.includeMetadata) {
			...
			// Add beat count
			const beatCountText = document.createElementNS('http://www.w3.org/2000/svg', 'text');
			...
			if (sequence.difficulty_level && options.includeDifficulty) {
				const difficultyText = document.createElementNS('http://www.w3.org/2000/svg', 'text');
	 ...
	 */
	private async svgToCanvas(svg: SVGElement, options: ExportOptions): Promise<HTMLCanvasElement> {
