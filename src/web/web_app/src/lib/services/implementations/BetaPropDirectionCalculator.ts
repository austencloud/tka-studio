...
type DiamondLoc = Location.NORTH | Location.SOUTH | Location.EAST | Location.WEST;
type BoxLoc = Location.NORTHEAST | Location.SOUTHEAST | Location.SOUTHWEST | Location.NORTHWEST;
...
export class BetaPropDirectionCalculator {
	...
	private directionMapRadialShift: Record<Loc, Partial<Record<Loc, Direction>>> = {
		...
		[Location.NORTHEAST]: { [Location.NORTHWEST]: UPRIGHT, [Location.SOUTHEAST]: UPRIGHT },
		[Location.SOUTHEAST]: { [Location.NORTHEAST]: DOWNRIGHT, [Location.SOUTHWEST]: DOWNRIGHT },
		[Location.SOUTHWEST]: { [Location.NORTHWEST]: DOWNLEFT, [Location.SOUTHEAST]: DOWNLEFT },
		[Location.NORTHWEST]: { [Location.NORTHEAST]: UPLEFT, [Location.SOUTHWEST]: UPLEFT },
	};
	...
	private directionMapNonRadialShift: Record<Loc, Partial<Record<Loc, Direction>>> = {
		...
		[Location.NORTHEAST]: { [Location.SOUTHEAST]: UPLEFT, [Location.NORTHWEST]: DOWNRIGHT },
		[Location.SOUTHEAST]: { [Location.NORTHEAST]: UPRIGHT, [Location.SOUTHWEST]: UPRIGHT },
		[Location.SOUTHWEST]: { [Location.NORTHWEST]: UPLEFT, [Location.SOUTHEAST]: DOWNRIGHT },
		[Location.NORTHWEST]: { [Location.NORTHEAST]: DOWNLEFT, [Location.SOUTHWEST]: DOWNLEFT },
	};
	 ...
	 */
	private handleShiftMotion(_prop: PropData, motionData: MotionData): Direction | null {
		const isRadial = this.endsWithRadialOrientation();
		const startLoc =
			(motionData as unknown as { start_loc?: string; start_location?: string }).start_loc ??
			(motionData as unknown as { start_location?: string }).start_location ??
			'';
		const endLoc =
			(motionData as unknown as { end_loc?: string; end_location?: string }).end_loc ??
	...
	private endsWithRadialOrientation(): boolean {
		const redEndOri =
			(this.motionData.red as unknown as { end_orientation?: string; end_ori?: string })
				.end_orientation ??
			(this.motionData.red as unknown as { end_ori?: string }).end_ori;
		const blueEndOri =
			(this.motionData.blue as unknown as { end_orientation?: string; end_ori?: string })
				.end_orientation ??
