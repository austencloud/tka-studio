...
let currentPageSequences = $derived(() => {
	const startIndex = (sequenceCardState.currentPage - 1) * sequenceCardState.itemsPerPage;
...
let progressMessage = $derived(() => {
	const { isExporting, isRegenerating, exportProgress, selectedLength } = sequenceCardState;
...
let canExport = $derived(() => {
	return !sequenceCardState.isExporting && 
...
let cacheStatus = $derived(() => {
	return {
		...
		estimatedSize: Math.min(filteredSequences.length, sequenceCardState.cacheSizeLimit)
	};
});
...
function calculateOptimalLayout(
	...
	const cardWidth = (availableWidth - (CARD_SPACING * (preferredColumns - 1))) / preferredColumns;
	...
	return {
		...
		utilization: (itemCount * cardWidth * cardHeight) / (availableWidth * availableHeight)
	};
}
...
export function updateExportSetting<K extends keyof typeof exportSettingsState>(
	...
	(exportSettingsState as any)[K] = value;
}
...
// Import existing sequence state functions
import { getSequences } from './sequenceState.svelte';
