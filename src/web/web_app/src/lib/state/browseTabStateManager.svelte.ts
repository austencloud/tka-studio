...
export class BrowseTabStateManager {
	 ...
	 * Save view state
	 */
	async saveViewState(mode: 'grid' | 'list', gridColumns?: number): Promise<void> {
		if (!browser) return;
	 ...
	 */
	restoreScrollPosition(container: HTMLElement, scrollState: BrowseScrollState): void {
	 ...
	 */
	private mapSortMethodToString(method: SortMethod): 'name_asc' | 'name_desc' | 'difficulty' | 'length' | 'recent' | 'author' {
		switch (method) {
