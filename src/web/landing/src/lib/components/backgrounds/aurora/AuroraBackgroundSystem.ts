...
export class AuroraBackgroundSystem implements BackgroundSystem {
	...
	public setQuality(quality: QualityLevel): void {
		...
		if (this.isInitialized) {
			...
			while (this.sparkles.length < numSparkles) this.sparkles.push(this.createSparkle());
		}
	}
	...
	private drawWavyGradient(ctx: CanvasRenderingContext2D, dimensions: Dimensions): void {
		// Create wavy gradient background similar to desktop version
		const gradient = ctx.createLinearGradient(0, dimensions.height, dimensions.width, 0);
	...
	private drawBlobs(ctx: CanvasRenderingContext2D, dimensions: Dimensions): void {
		for (const blob of this.blobs) {
	...
	private drawSparkles(ctx: CanvasRenderingContext2D, dimensions: Dimensions): void {
		for (const sparkle of this.sparkles) {
