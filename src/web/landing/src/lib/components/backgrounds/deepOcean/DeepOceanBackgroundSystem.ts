...
export class DeepOceanBackgroundSystem implements BackgroundSystem {
	...
	initialize(dimensions: Dimensions, quality: QualityLevel): void {
		...
		console.log(`🌊 Deep Ocean background initialized with ${this.state.bubbles.length} bubbles, ${this.state.marineLife.length} marine life`);
	}
	...
	private initializeLightRays(dimensions: Dimensions): void {
		const rayCount = this.quality === 'high' ? 8 : this.quality === 'medium' ? 5 : 3;
		this.state.lightRays = [];
		
		for (let i = 0; i < rayCount; i++) {
			this.state.lightRays.push({
				x: (dimensions.width / rayCount) * i + Math.random() * (dimensions.width / rayCount),
	...
	private createMarineLife(dimensions: Dimensions): MarineLife {
		const types: Array<'fish' | 'jellyfish' | 'seaweed'> = ['fish', 'jellyfish', 'seaweed'];
	...
	private updateBubbles(dimensions: Dimensions): void {
		for (let i = this.state.bubbles.length - 1; i >= 0; i--) {
			...
			bubble.x += Math.sin(this.animationTime * bubble.sway + bubble.swayOffset) * 0.5;
	...
	private drawOceanGradient(ctx: CanvasRenderingContext2D, dimensions: Dimensions): void {
	...
	private drawJellyfish(ctx: CanvasRenderingContext2D, marine: MarineLife): void {
		const size = marine.size;
	...
	private drawParticles(ctx: CanvasRenderingContext2D, layerOpacity: number): void {
		ctx.save();
		this.state.particles.forEach(particle => {
			if ((layerOpacity > 0.5 && particle.size > 2) || (layerOpacity <= 0.5 && particle.size <= 2)) {
	...
	getMetrics(): PerformanceMetrics {
		const totalParticles = this.state.bubbles.length + this.state.marineLife.length + this.state.particles.length;
		
		return {
			fps: 60, // Estimated
			warnings: totalParticles > 100 ? ['High particle count may impact performance'] : [],
