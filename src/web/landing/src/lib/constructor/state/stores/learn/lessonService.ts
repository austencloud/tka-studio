...
function generateLetterToPictographQuestion(
  ...
  const availableLetters = [
    ...new Set(allPictographs.map((p) => p.letter)),
  ...
  const correctPictograph = ensurePictographComplete(
    pictographsForLetter[randomIndex]
  );

  // Collect all pictographs with different letters
  const otherLetterPictographs = allPictographs.filter(
    (p) => p.letter !== letter
  );
  ...
  const wrongLetters = shuffledLetters.slice(
    0,
    (lessonConfig.options?.numOptions || 4) - 1
  );
  ...
  wrongLetters.forEach((letter) => {
    ...
    if (pictographsForLetter.length > 0) {
      const randomIndex = Math.floor(
        Math.random() * pictographsForLetter.length
      );
      wrongOptions.push(
        ensurePictographComplete(pictographsForLetter[randomIndex])
      );
    }
  });
...
function generatePictographToLetterQuestion(
  ...
  const randomIndex = Math.floor(Math.random() * pictographsForLetter.length);
  const pictograph = ensurePictographComplete(
    pictographsForLetter[randomIndex]
  );
  ...
  const wrongOptions = shuffledLetters.slice(
    0,
    (lessonConfig.options?.numOptions || 4) - 1
  );
...
function generateMotionTypeQuestion(lessonConfig: LessonConfig, index: number) {
  ...
  const selectedPictograph =
    pictographsWithMotion[index % pictographsWithMotion.length];
  ...
  const selectedWrongOptions = wrongOptions.slice(
    0,
    (lessonConfig.options?.numOptions || 4) - 1
  );
...
function generatePositionRecognitionQuestion(
  lessonConfig: LessonConfig,
  index: number
) {
  // Get all available pictographs
  ...
  // Select a pictograph for the question
  const selectedPictograph =
    pictographsWithPosition[index % pictographsWithPosition.length];
...
export function checkAnswerLogic(
  ...
  switch (lessonConfig.id) {
    ...
    default:
      console.error(
        `Unknown lesson type for answer checking: ${lessonConfig.id}`
      );
      return false;
  }
}
