...
from desktop.modern.core.interfaces.start_position_services import IStartPositionSelectionService
from desktop.modern.core.commands.start_position_commands import SetStartPositionCommand
from desktop.modern.core.service_locator import get_event_bus
...
class StartPositionSelectionService(IStartPositionSelectionService):
    ...
    def validate_selection(self, position_key: str) -> bool:
        ...
        try:
            ...
            if len(parts) != 2:
                logger.warning(f"Position key should have exactly two parts: {position_key}")
                return False
            
            # For start positions, both parts should be the same
            if parts[0] != parts[1]:
                logger.debug(f"Position key is not a start position (parts differ): {position_key}")
    ...
    def create_selection_command(self, position_key: str) -> SetStartPositionCommand:
        ...
        """
        try:
            # Get the event bus for command integration
            event_bus = get_event_bus()
            
            if not event_bus:
                logger.warning("Event bus not available, command may not integrate properly")
        ...
        try:
            ...
            if "_" in position_key:
                ...
                if len(parts) >= 2:
                    ...
                    logger.debug(f"Extracted end position '{end_position}' from '{position_key}'")
                    return end_position
    ...
    def normalize_position_key(self, position_key: str) -> str:
        ...
        try:
            ...
            if "_" not in position_key:
                ...
                logger.debug(f"Normalized single position '{position_key}' to start position '{normalized}'")
                return normalized
