...
from desktop.modern.core.interfaces.learn_services import IAnswerValidationService, IQuizSessionService
...
class AnswerValidationService(IAnswerValidationService):
    ...
    def check_answer(self, question: QuestionData, selected_answer: Any) -> bool:
        ...
        try:
            ...
            logger.error(f"Failed to check answer for question {question.question_id}: {e}")
            return False
    ...
    def _compare_pictographs(self, correct: Dict, selected: Dict) -> bool:
        ...
        try:
            ...
            try:
                from data.constants import LETTER, START_POS, END_POS
    ...
    def record_answer(self, session_id: str, question_id: str, correct: bool) -> None:
        ...
        try:
            ...
            if session:
                updates = {
                    "questions_answered": len(self._answer_history[session_id])
                }
                ...
                success = self.session_service.update_session_progress(session_id, **updates)
                if not success:
                    logger.warning(f"Failed to update session progress for {session_id}")
    ...
    def get_correct_streak(self, session_id: str) -> int:
        ...
        try:
            ...
            logger.error(f"Failed to calculate correct streak for session {session_id}: {e}")
            return 0
    ...
    def get_longest_streak(self, session_id: str) -> int:
        ...
        try:
            ...
            logger.error(f"Failed to calculate longest streak for session {session_id}: {e}")
            return 0
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            total_answers = sum(len(history) for history in self._answer_history.values())
            
            return {
                "tracked_sessions": total_sessions,
                "total_answers_recorded": total_answers,
                "average_answers_per_session": total_answers / total_sessions if total_sessions > 0 else 0,
            }
