<!--
	RightPanel.svelte - REFACTORED

	Clean, professional right panel component that routes between sub-tabs.
...
<script lang="ts">
  ...
  import { RightPanelNavigationController } from "../navigation/RightPanelNavigationController";
  import { createAnimationPanelState } from "../state/animation-panel/AnimationPanelState.svelte";
  import type { IAnimationStateRef, IRightPanelProps } from "../types/build-tab-types";
  ...
  let navigationController: RightPanelNavigationController | null = null;

  // Animation panel state (extracted to separate service)
  let animationPanelState = $state<ReturnType<typeof createAnimationPanelState> | null>(null);
  ...
  onMount(() => {
    ...
    animationPanelState = createAnimationPanelState(hapticService);
    navigationController = new RightPanelNavigationController(buildTabState, constructTabState);
