...
export class CAPExplanationTextGenerator implements ICAPExplanationTextGenerator {
  ...
  private readonly descriptions: Record<CAPComponent, string> = {
    [CAPComponent.ROTATED]: "Rotates the sequence 180 degrees, flipping all movements to their opposite positions.",
    [CAPComponent.MIRRORED]: "Mirrors the sequence horizontally, creating a reflection of all movements.",
    [CAPComponent.SWAPPED]: "Swaps the left and right hand movements throughout the sequence.",
    [CAPComponent.COMPLEMENTARY]: "Applies complementary transformations to create variations of the base sequence."
  };
  ...
  public generateExplanationText(selectedComponents: Set<CAPComponent>): string {
    ...
    if (selected.length === 1) {
      ...
      const componentInfo = CAP_COMPONENTS.find(c => c.component === component);
