...
// Dynamic import for pdfjs-dist (browser-only)
...
export class PDFService implements IPDFService {
  ...
  private async ensureCacheInitialized(): Promise<void> {
    if (!this.cacheInitialized) {
      try {
        ...
        console.warn(
          "📦 PDFService: Cache initialization failed, proceeding without cache:",
          error
        );
      }
    }
  ...
  async convertPageToImage(
    ...
    try {
      ...
      console.error(
        `📖 PDFService: Error converting page ${pageNumber}`,
        error
      );
      throw new Error(
        `Failed to convert page ${pageNumber}: ${error instanceof Error ? error.message : "Unknown error"}`
      );
    }
  }
