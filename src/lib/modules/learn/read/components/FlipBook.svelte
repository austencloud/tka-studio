...
<script lang="ts">
  ...
  // Props
  const { pdfUrl = "/static/Level 1.pdf", config = {} } = $props<{
    pdfUrl?: string;
  ...
  {:else}
    <div class="flipbook-container">
      ...
      <div class="flipbook-display">
        <div bind:this={flipBookContainer} class="flipbook-element"></div>
      </div>
    </div>
  {/if}
  ...
  <div class="flipbook-controls">
    ...
    <button
      ...
      disabled={readState.currentPage >= readState.totalPages() ||
        readState.loadingState.isLoading}
    >
      Next →
    </button>
  </div>
...
<style>
  ...
  .flipbook-display {
    ...
    max-height: 768px; /* Prevent it from being too tall */
