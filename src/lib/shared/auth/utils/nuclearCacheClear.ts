...
export async function diagnoseCacheState(): Promise<CacheDiagnostics> {
  ...
  try {
    ...
    console.log('📦 [Cache Diagnostics] IndexedDB Databases:', diagnostics.indexedDBDatabases);
  } catch (error) {
  ...
  try {
    ...
    console.log('🗄️ [Cache Diagnostics] localStorage Keys:', diagnostics.localStorageKeys);
  } catch (error) {
  ...
  try {
    ...
    console.log('📋 [Cache Diagnostics] sessionStorage Keys:', diagnostics.sessionStorageKeys);
  } catch (error) {
    console.error('❌ [Cache Diagnostics] Failed to list sessionStorage:', error);
  }
  ...
  if (oldProjectReferences.indexedDB.length > 0 || oldProjectReferences.localStorage.length > 0) {
...
export async function nuclearCacheClear(): Promise<void> {
      ...
      try {
        await new Promise<void>((resolve, reject) => {
          ...
          deleteRequest.onerror = () => {
            console.error(`❌ [NUCLEAR] Failed to delete IndexedDB: ${db.name}`);
            failedItems.push(`IndexedDB: ${db.name}`);
            reject(deleteRequest.error);
          };

          deleteRequest.onblocked = () => {
            console.warn(`⚠️ [NUCLEAR] Delete blocked (close other tabs): ${db.name}`);
  ...
  try {
    ...
    console.log(`💣 [NUCLEAR] Clearing ${keysBefore.length} localStorage items`);
    ...
    if (keysAfter.length === 0) {
      ...
      console.error(`⚠️ [NUCLEAR] localStorage still has ${keysAfter.length} items`);
  ...
  try {
    ...
    console.log(`💣 [NUCLEAR] Clearing ${keysBefore.length} sessionStorage items`);
    ...
    if (keysAfter.length === 0) {
      ...
      console.error(`⚠️ [NUCLEAR] sessionStorage still has ${keysAfter.length} items`);
  ...
  if (failedItems.length > 0) {
    ...
    console.error('⚠️ You may need to close other tabs or restart your browser');
  }
  ...
  const hasRemainingFirebaseData =
    postClearDiagnostics.indexedDBDatabases.some(db =>
      db.includes('firebase') || db.includes('firestore')
    ) ||
    postClearDiagnostics.localStorageKeys.some(key =>
      key.includes('firebase') || key.includes('firestore')
    );

  if (hasRemainingFirebaseData) {
    console.error('🚨 [NUCLEAR] WARNING: Firebase data still present after clear!');
...
${diagnostics.localStorageKeys.slice(0, 10).map(key => `  • ${key}`).join('\n')}
...
${diagnostics.cookies.slice(0, 10).map(c => `  • ${c}`).join('\n')}
