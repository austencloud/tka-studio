...
export class StarfieldBackgroundSystem implements BackgroundSystem {
	...
	private drawStars(ctx: CanvasRenderingContext2D, dimensions: Dimensions): void {
		...
		for (const star of this.stars) {
			...
			if (x2d < -10 || x2d > dimensions.width + 10 || 
				y2d < -10 || y2d > dimensions.height + 10) {
				continue;
			}
			...
			if (size > 1) {
				...
				glowGradient.addColorStop(0, `rgba(${color.r}, ${color.g}, ${color.b}, ${brightness})`);
				glowGradient.addColorStop(0.5, `rgba(${color.r}, ${color.g}, ${color.b}, ${brightness * 0.3})`);
	...
	private drawNebulaEffect(ctx: CanvasRenderingContext2D, dimensions: Dimensions): void {
		...
		const nebulaGradient = ctx.createRadialGradient(
			dimensions.width * 0.3, dimensions.height * 0.7, 0,
			dimensions.width * 0.3, dimensions.height * 0.7, dimensions.width * 0.4
		);
		...
		const nebula2Gradient = ctx.createRadialGradient(
			...
			dimensions.width * 0.8, dimensions.height * 0.2, dimensions.width * 0.3
		);
