...
export class OptionPickerLayoutManager {
	 ...
	 */
	static calculateLayout(params: LayoutCalculationParams): LayoutCalculationResult {
		const {
			count,
			containerWidth,
			containerHeight,
		...
		const enhancedDeviceInfo = getEnhancedDeviceType(containerWidth, isMobileUserAgent);
