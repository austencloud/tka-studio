...
export function createTabCrossfade(options: Partial<CrossfadeConfig> = {}) {
	const { duration = 300, delay = 0, easing = quintOut, fallback = enhancedFade } = options;

	const transitions = new Map();

	function crossfadeTransition(node: Element, params: CrossfadeConfig & { key: string }) {
		const {
			key,
...
export const slideLeft = (node: Element) =>
	slideFade(node, {
		direction: 'left',
		duration: 300,
	});

export const slideRight = (node: Element) =>
	slideFade(node, {
		direction: 'right',
		duration: 300,
	});

export const slideUp = (node: Element) =>
	slideFade(node, {
		direction: 'up',
		duration: 300,
	});

export const slideDown = (node: Element) =>
	slideFade(node, {
		direction: 'down',
		duration: 300,
	});
