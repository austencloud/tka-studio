...
export async function transitionToMainTab(
	...
	if (fadeState.mainTabTransition.isTransitioning) {
		console.warn(`Already transitioning from ${fadeState.mainTabTransition.fromTab} to ${fadeState.mainTabTransition.toTab}`);
...
export async function transitionToSubTab(
	...
	if (fadeState.subTabTransition.isTransitioning) {
		console.warn(`Already transitioning from ${fadeState.subTabTransition.fromTab} to ${fadeState.subTabTransition.toTab}`);
 ...
 */
export function completeSubTabTransition(transitionId: string, fromTab: string, toTab: string): void {
	if (!fadeState.orchestrator) {
		console.warn('Fade orchestrator not initialized');
		return;
	}
