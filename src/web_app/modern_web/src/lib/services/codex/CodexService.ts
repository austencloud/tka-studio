		...
		Object.entries(CODEX_LETTER_MAPPING).forEach(([letter, mapping]) => {
			...
			if (matchingRow) {
				...
				if (pictograph) {
					...
					console.warn(`⚠️ Failed to convert CSV row to pictograph for letter "${letter}"`);
				}
			} else {
				...
				console.log(`Available rows for letter "${letter}":`, letterRows.length > 0 ? letterRows.slice(0, 3) : 'none');
			}
		});
