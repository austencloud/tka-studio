...
export interface BrowseSortState {
	method: 'name_asc' | 'name_desc' | 'difficulty' | 'length' | 'recent' | 'author';
	direction: 'asc' | 'desc';
	appliedAt: Date;
}
...
export class BrowseStatePersistenceService implements IBrowseStatePersistenceService {
	...
	async loadBrowseState(): Promise<CompleteBrowseState | null> {
		try {
			...
			if (parsed.selection?.lastSelectedAt) {
				parsed.selection.lastSelectedAt = new Date(parsed.selection.lastSelectedAt);
			}
	...
	isStateValid(state: CompleteBrowseState): boolean {
		...
		// Check age
		const ageInDays = (Date.now() - state.lastUpdated.getTime()) / (1000 * 60 * 60 * 24);
