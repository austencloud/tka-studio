<script lang="ts">
  ...
  // Import OptionDataService for option loading
  ...
  export let data: any; // LayoutData;
  ...
  onMount(() => {
    ...
    // Dev-only: forward browser console logs to the dev server terminal
    if (dev) {
      const levels = ["log", "info", "warn", "error", "debug"] as const;
      levels.forEach((level) => {
        const original = console[level].bind(console);
        console[level] = ((...args: any[]) => {
          // Always keep local console behavior
          original(...args);
          // Forward to dev server (best-effort, non-blocking)
          try {
            const payload = {
              level,
              args: args.map((a) => {
            ...
            // Prefer sendBeacon when available to avoid blocking
            if ("sendBeacon" in navigator) {
              const blob = new Blob([JSON.stringify(payload)], {
                type: "application/json",
              });
              (navigator as any).sendBeacon("/__client-log", blob);
            } else {
              fetch("/__client-log", {
                method: "POST",
    ...
    if (data?.csvData && !data.error) {
      try {
        ...
        // Initialize OptionDataService with CSV data
        optionDataService.initialize(data.csvData);
        initialized = true; // Mark as initialized
      } catch (initError) {
        console.error(
          "Layout onMount: Error calling initializePictographData:",
          initError
        );
      ...
      console.error(
        "Layout onMount: Error received from load function:",
        data.error
      );
