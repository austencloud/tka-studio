...
<script lang="ts">
  ...
  $: mirrorManager = effectiveArrowData
    ? new ArrowSvgMirrorManager(effectiveArrowData)
    : null;
  ...
  $: if (svgData && effectiveArrowData?.coords) {
    ...
    const mirrorTransform = effectiveArrowData.svgMirrored
      ? "scale(-1, 1)"
      : "";
  ...
  function handleLoadError(error: unknown) {
    ...
    if (import.meta.env.DEV) {
      ...
      console.error(
        "Arrow load error: " + ((error as Error)?.message || "Unknown error")
      );
    }
