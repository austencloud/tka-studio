<script lang="ts">
  ...
  const effectiveGridMode = $derived(
    props.gridMode ?? $settingsStore.defaultGridMode
  );
  ...
  function validateGridData(data: GridData): boolean {
    ...
    if (
      !data.allHandPointsNormal ||
      Object.keys(data.allHandPointsNormal).length === 0
    ) {
  ...
  function initializeGridData() {
    ...
    try {
      ...
      if (
        ...
        throw new Error(
          `Invalid circle coordinates for grid mode: ${effectiveGridMode}`
        );
      }
      ...
      gridError = true;
      gridErrorMessage =
        error instanceof Error ? error.message : "Unknown grid error";

      // Dispatch error event both ways for compatibility
      props.onError?.(gridErrorMessage);
      dispatch("error", { message: gridErrorMessage });

      // Do NOT create fallback data - let the error be visible
      // Grid loading failed and should remain failed
    }
  }
