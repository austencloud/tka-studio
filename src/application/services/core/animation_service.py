...
class CoreAnimationService(IAnimationService):
    ...
    def create_animation_command(
        ...
        try:
            ...
            logger.debug(f"🎬 [ANIMATION_SERVICE] Created animation command: {command_id}")
            
            return command
            
        except Exception as e:
            self._performance_stats["errors"] += 1
            logger.error(f"❌ [ANIMATION_SERVICE] Failed to create animation command: {e}")
            raise
    ...
    def create_animation_sequence(
        ...
        try:
            ...
            logger.debug(f"🎬 [ANIMATION_SERVICE] Created animation sequence with {len(commands)} commands")
            
            return sorted_commands
            
        except Exception as e:
            self._performance_stats["errors"] += 1
            logger.error(f"❌ [ANIMATION_SERVICE] Failed to create animation sequence: {e}")
            return commands
    ...
    def calculate_animation_values(
        ...
        try:
            ...
            self._performance_stats["errors"] += 1
            logger.error(f"❌ [ANIMATION_SERVICE] Failed to calculate animation values: {e}")
