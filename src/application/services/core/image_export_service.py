...
class CoreImageExportService(IImageExportService):
    ...
    def create_export_commands(
        ...
        try:
            ...
            canvas_size, layout_info = self.calculate_layout_dimensions(beat_count, export_options)
            
            # Create background command
            background_cmd = self._create_background_command(canvas_size, export_options)
            commands.append(background_cmd)
            
            # Create header commands (title, difficulty, etc.)
            header_commands = self._create_header_commands(sequence_data, layout_info, export_options)
            commands.extend(header_commands)
            
            # Create beat commands
            beat_commands = self._create_beat_commands(
                sequence_data.get("beats", []), 
                layout_info, 
                export_options
            )
            ...
            footer_commands = self._create_footer_commands(sequence_data, layout_info, export_options)
    ...
    def generate_export_data(
        ...
        try:
            ...
            image_data = ImageData(
                ...
                background_color=Color.from_hex(export_options.get("background_color", "#FFFFFF")),
    ...
    def _create_beat_commands(
        ...
        for i, beat_data in enumerate(beats):
            ...
            x = layout_info["content_start_x"] + col * (layout_info["beat_size"] + layout_info["margin"])
            y = layout_info["content_start_y"] + row * (layout_info["beat_size"] + layout_info["margin"])
