...
from application.services.core.types import Point, RenderCommand, Size
from application.services.core.pictograph_renderer import IPictographAssetProvider
...
class CorePropRenderingService(IPropRenderingService):
    ...
    def create_prop_render_command(
        ...
        try:
            ...
            prop_asset = self._asset_provider.get_prop_asset(prop_type, color)
            
            if not prop_asset:
                logger.error(f"❌ [PROP_SERVICE] Failed to get prop asset for {prop_type}/{color}")
            ...
            command = RenderCommand(
                ...
                metadata={
                    "prop_type": prop_type,
                    "motion_type": motion_data.motion_type.value if motion_data.motion_type else None,
                    "has_beta_positioning": pictograph_data is not None
                }
            )
            
            self._performance_stats["props_rendered"] += 1
            logger.debug(f"🎭 [PROP_SERVICE] Created render command for {color} {prop_type}")
    ...
    def calculate_prop_position(
        ...
        try:
            ...
            if pictograph_data:
                ...
                return Point(
                    base_position.x + beta_offset.x,
                    base_position.y + beta_offset.y
                )
    ...
    def _determine_prop_type(
        self, 
        motion_data: MotionData, 
        pictograph_data: Optional[PictographData]
    ) -> str:
    ...
    def _calculate_beta_positioning_offset(
        ...
        try:
            ...
            if prop_mgmt_service.should_apply_beta_positioning(pictograph_data):
                blue_offset, red_offset = prop_mgmt_service.calculate_separation_offsets(pictograph_data)
    ...
    def _create_error_command(self, position: Point, target_size: Size) -> RenderCommand:
        ...
        return RenderCommand(
            ...
            color="red",
            layer_order=1,
            metadata={"error": "Failed to render prop"}
        )
