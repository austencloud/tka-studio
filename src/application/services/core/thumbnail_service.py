...
from application.services.core.types import (
    ImageData, ImageFormat, Size, Color, ComponentHandle
)
...
class IThumbnailFactory(ABC):
    ...
    def batch_create_thumbnails(
        self,
        sequence_specs: List[ThumbnailSpec],
        options: Optional[Dict] = None
    ) -> List[ThumbnailData]:
...
class CoreThumbnailService(IThumbnailFactory):
    ...
    def create_thumbnail_data(
        ...
        try:
            ...
            logger.error(f"Failed to create thumbnail for {sequence_spec.sequence_id}: {e}")
            return ThumbnailData(
                thumbnail_id=self._generate_thumbnail_id(),
                spec=sequence_spec,
                error_message=str(e)
    ...
    def _load_sequence_thumbnail_image(self, spec: ThumbnailSpec) -> Optional[ImageData]:
...
class FileSystemImageLoader(IThumbnailImageLoader):
    ...
    def load_image(self, image_path: str, target_size: Size) -> Optional[ImageData]:
        ...
        try:
            ...
            file_format = format_map.get(image_path.suffix.lower(), ImageFormat.PNG)
            ...
            return ImageData(
                ...
                format=file_format,
                data=image_data,
                metadata={"source": str(image_path), "loader": "filesystem"}
            )
...
def create_thumbnail_service(image_loader: Optional[IThumbnailImageLoader] = None, asset_base_path: str = "assets") -> CoreThumbnailService:
    """
...
def convert_sequence_data_to_spec(sequence_data, thumbnail_width: int) -> ThumbnailSpec:
    ...
    return ThumbnailSpec(
        ...
        sequence_name=getattr(sequence_data, 'name', None),
...
def batch_convert_sequences_to_specs(
    ...
    return [
        convert_sequence_data_to_spec(seq, thumbnail_width) 
