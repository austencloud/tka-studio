...
from PyQt6.QtGui import QColor, QFont, QFontMetrics, QImage, QPainter, QPen, QBrush
...
from application.services.core.types import Color, ImageData, ImageFormat, Point, Size
...
class QtImageExportEngine:
    ...
    def render_to_qt_image(self, image_data: ImageData) -> QImage:
        ...
        try:
            ...
            if image_data.background_color:
                bg_color = QtImageExportConverter.color_to_qt(image_data.background_color)
            ...
            logger.debug(f"Successfully rendered image: {image_data.width}x{image_data.height}")
            return qt_image
    ...
    def _execute_render_commands(self, painter: QPainter, commands: List[Dict[str, Any]]):
        ...
        for command in sorted_commands:
            try:
                ...
                logger.error(f"Failed to execute command {command.get('type', 'unknown')}: {e}")
    ...
    def _render_rectangle_command(self, painter: QPainter, command: Dict[str, Any]):
        ...
        try:
            ...
            size = command.get('size', Size(100, 100))
            color = command.get('color', '#000000')
            fill_color = command.get('fill_color', None)
            ...
            # Set pen and brush
            qt_color = QColor(color) if isinstance(color, str) else QtImageExportConverter.color_to_qt(color)
            painter.setPen(QPen(qt_color, stroke_width))
            
            if fill_color:
                qt_fill = QColor(fill_color) if isinstance(fill_color, str) else QtImageExportConverter.color_to_qt(fill_color)
...
class QtImageExportAdapter:
    ...
    def __init__(self, 
                 core_service: Optional[CoreImageExportService] = None):
    ...
    def render_sequence_image(
        ...
        try:
            ...
            image_data = self.core_service.generate_export_data(sequence_data, export_options)
    ...
    def calculate_image_dimensions(
        ...
        try:
            canvas_size, _ = self.core_service.calculate_layout_dimensions(beat_count, export_options)
