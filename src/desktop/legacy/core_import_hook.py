...
class CoreImportFinder:
    ...
    def _find_best_core_directory(self, calling_path: Path, module_name: str) -> Optional[Path]:
        ...
        for core_dir in self.core_directories:
            ...
            if module_path.exists():
                ...
                try:
                    ...
                    distance = len(calling_path.parts) + len(core_dir.parts) - 2 * len(Path(common_path).parts)
...
def install_core_import_hook():
    ...
    sys.meta_path = [finder for finder in sys.meta_path 
                    if not isinstance(finder, CoreImportFinder)]
