...
class TabSwitchingConsistencyTester:
    ...
    def _test_initial_browse_tab_layout(self) -> bool:
        ...
        if not self._switch_to_tab("browse"):
            self._record_test_result(
                "Initial Browse Tab Layout", False, "Failed to switch to browse tab"
            )
            return False
        ...
        self._record_test_result(
            ...
            is_valid,
            (
                f"Ratio: {measurement.get('aspect_ratio', 0):.2f}"
                if measurement
                else "No measurement"
            ),
        )
    ...
    def _test_tab_switching_sequence(self) -> bool:
        ...
        for i, tab_name in enumerate(tabs_to_test):
            self.logger.info(
                f"  Switching to {tab_name} (step {i+1}/{len(tabs_to_test)})"
            )

            if not self._switch_to_tab(tab_name):
                self._record_test_result(
                    f"Switch to {tab_name}", False, "Failed to switch"
                )
                all_valid = False
                continue
    ...
    def _test_fast_consecutive_switches(self) -> bool:
        ...
        for i, tab_name in enumerate(tabs):
            if not self._switch_to_tab(tab_name):
                self._record_test_result(
                    f"Fast switch to {tab_name}", False, "Failed to switch"
                )
                all_valid = False
                continue
    ...
    def _test_window_resize_during_switches(self) -> bool:
        ...
        if not self._switch_to_tab("browse"):
            self._record_test_result(
                "Resize Test Setup", False, "Failed to switch to browse tab"
            )
            return False
    ...
    def _test_browse_tab_return_consistency(self) -> bool:
        ...
        for tab_name in other_tabs:
            ...
            if not self._switch_to_tab("browse"):
                self._record_test_result(
                    f"Return from {tab_name}", False, "Failed to return to browse"
                )
                all_valid = False
                continue
    ...
    def _measure_layout(self, context: str) -> Dict:
        ...
        try:
            ...
            right_width = (
                self.main_widget.right_stack.width()
            ...
            left_ratio = (
                left_width / total_content_width if total_content_width > 0 else 0
            )
            right_ratio = (
                right_width / total_content_width if total_content_width > 0 else 0
            )
...
if __name__ == "__main__":
    ...
    print(
        "Import and call run_tab_switching_consistency_test(main_widget) from your application."
    )
