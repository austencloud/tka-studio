...
from ..enums import MotionType, Orientation, Location, PropRotationDirection
...
class MotionAttributes:
    ...
    def __post_init__(self):
        ...
        if isinstance(self.turns, str) and self.turns != 'fl':
            raise ValueError(f"Invalid turn value: {self.turns}. String turns must be 'fl'")
    ...
    def with_orientations(self, start_ori: Orientation, end_ori: Orientation) -> 'MotionAttributes':
        """Create a new MotionAttributes with different orientations."""
        from dataclasses import replace
        return replace(self, start_ori=start_ori, end_ori=end_ori)
    
    def to_float_state(self, prefloat_motion_type: MotionType, prefloat_prop_rot_dir: PropRotationDirection) -> 'MotionAttributes':
