...
from desktop.modern.domain.models.enums import Letter, GridPosition, Timing, Direction
...
class LetterDeterminationPictographData:
    ...
    def is_start_position(self) -> bool:
        ...
        return self.letter is None or (self.letter and str(self.letter.value).startswith('α'))
    ...
    def is_shift_float_hybrid(self) -> bool:
        ...
        return ((blue_is_float and red_is_shift) or 
                (red_is_float and blue_is_shift))
    ...
    def with_orientations(self, 
                         blue_start_ori=None, blue_end_ori=None,
                         red_start_ori=None, red_end_ori=None) -> 'LetterDeterminationPictographData':
    ...
    def with_turns(self, blue_turns=None, red_turns=None) -> 'LetterDeterminationPictographData':
    ...
    def with_prefloat_attributes(self, color: str, motion_type, prop_rot_dir) -> 'LetterDeterminationPictographData':
        """Create new instance with prefloat attributes."""
        from dataclasses import replace
        
        if color == 'blue':
            new_blue_motion = self.blue_motion.with_prefloat_attributes(motion_type, prop_rot_dir)
            new_pictograph_data = self.pictograph_data.update(
                motions={
                    'blue': new_blue_motion.to_motion_data(),
            ...
            new_red_motion = self.red_motion.with_prefloat_attributes(motion_type, prop_rot_dir)
            new_pictograph_data = self.pictograph_data.update(
                motions={
    ...
    def from_legacy_dict(cls, data: dict) -> 'LetterDeterminationPictographData':
        ...
        def dict_to_extended_motion(attr_dict: dict) -> ExtendedMotionData:
            ...
            if "prefloat_motion_type" in attr_dict and attr_dict["prefloat_motion_type"]:
                prefloat_motion_type = MotionType(attr_dict["prefloat_motion_type"])
            
            prefloat_prop_rot_dir = None
            if "prefloat_prop_rot_dir" in attr_dict and attr_dict["prefloat_prop_rot_dir"]:
                prefloat_prop_rot_dir = RotationDirection(attr_dict["prefloat_prop_rot_dir"])
        ...
        pictograph_data = PictographData(
            ...
            start_position=GridPosition(data["start_pos"]) if data.get("start_pos") else None,
    ...
    @classmethod
    def from_pictograph_data(cls, pictograph_data: PictographData, **kwargs) -> 'LetterDeterminationPictographData':
