...
class ActiveAnimation:
    ...
    def get_elapsed_time(self, current_time: float) -> float:
        ...
        if self.state == AnimationState.PAUSED:
            return (self.pause_time or current_time) - self.start_time - self.total_paused_duration
    ...
    def calculate_progress(self, current_time: float) -> float:
        ...
        return animation_elapsed / self.config.duration if self.config.duration > 0 else 1.0
        ...
        while True:
            ...
            if elapsed < config.delay:
                ...
                if animation_elapsed >= config.duration:
                    ...
                    progress = animation_elapsed / config.duration if config.duration > 0 else 1.0
...
class CoreAnimationEngine(IAnimationEngine):
    ...
    async def _instant_animation(self, target: AnimationTarget, config: AnimationConfig) -> str:
    ...
    def _interpolate_value(self, start_value: Any, end_value: Any, progress: float) -> Any:
    ...
    def _emit_animation_event(self, animation: ActiveAnimation, state: AnimationState) -> None:
