...
class SimpleUITester:
    ...
    def _initialize_components_with_data(self, sequence_data):
        ...
        self.workbench, self.graph_editor = (
            ComponentInitializer.initialize_workbench_and_graph_editor(
    ...
    def test_workbench_buttons(self) -> AITestResult:
        ...
        if not self.workbench:
            return AITestResult(success=False, errors=["Workbench not initialized"])

        # Get button references
        button_map = ComponentInitializer.get_workbench_button_references(
            self.workbench
        )

        if not button_map:
            return AITestResult(success=False, errors=["No workbench buttons found"])
    ...
    def test_graph_editor_interactions(self) -> AITestResult:
        ...
        if not self.graph_editor:
            return AITestResult(success=False, errors=["Graph editor not initialized"])
