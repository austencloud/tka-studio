...
class MethodExtractor:
    ...
    def analyze_method(method_func: Callable) -> MethodAnalysis:
        ...
        try:
            ...
            if is_too_long:
                suggested_extractions = MethodExtractor._suggest_extractions(source_lines, responsibilities)
    ...
    @staticmethod
    def _suggest_extractions(source_lines: List[str], responsibilities: List[str]) -> List[str]:
        ...
        if 'validation' in responsibilities:
            suggestions.append("Extract validation logic into _validate_inputs() method")
        
        if 'initialization' in responsibilities:
            suggestions.append("Extract initialization logic into _initialize_components() method")
        ...
        if 'ui_creation' in responsibilities:
            suggestions.append("Extract UI creation into _create_ui_components() method")
        
        if 'service_registration' in responsibilities:
            suggestions.append("Extract service registration into _register_services() method")
        
        if len(responsibilities) > 3:
            suggestions.append(f"Method has {len(responsibilities)} responsibilities - consider splitting into {len(responsibilities)} smaller methods")
        ...
        if comment_sections:
            suggestions.append("Consider extracting each commented step into its own method")
...
class RefactoredMethodPatterns:
    ...
    @staticmethod
    def error_recovery_pattern(self, operation_name, primary_action, fallback_action=None):
        """
...
def generate_refactoring_report(class_obj, class_name: str) -> str:
    ...
    for method_name, analysis in analyses.items():
        report.append(f"\n🔍 Method: {method_name}")
        report.append(f"   Lines: {analysis.line_count} (max: {MethodExtractor.MAX_METHOD_LENGTH})")
