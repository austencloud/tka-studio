...
def create_export_service_container(
    ...
    Returns:
        Configured DIContainer with export services
    """
    container = DIContainer()
    register_export_services(container, base_export_directory)
    return container


def get_export_services_info() -> dict:
    """
    Get information about the export services for debugging.
    ...
    return {
        "services": [
            "IExportDirectoryService -> ExportDirectoryService",
            "ISequenceDataTransformer -> SequenceDataTransformer",
            "ISequenceJsonExporter -> SequenceJsonExporter",
            "IExportContainerManager -> ExportContainerManager",
            "IWorkbenchExportOrchestrator -> WorkbenchExportService",
            "WorkbenchExportService -> WorkbenchExportService (backward compatibility)",
        ],
        "description": "Decomposed export services following Single Responsibility Principle",
        "architecture": "Orchestrator pattern with specialized services",
    }
