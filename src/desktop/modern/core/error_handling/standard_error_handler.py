...
class StandardErrorHandler:
    ...
    def handle_ui_error(
        ...
        if fallback_action:
            try:
                ...
                StandardErrorHandler.handle_service_error(
                    f"Fallback for {operation}", fallback_error, logger, ErrorSeverity.WARNING
                )
    ...
    def handle_dependency_resolution_error(
        ...
        if available_services:
            context["available_services"] = ", ".join(available_services[:5])  # Limit for readability
    ...
    def handle_circular_dependency_error(
        ...
        StandardErrorHandler.handle_service_error(
            "Circular dependency detection", Exception(error_msg), 
            logger, ErrorSeverity.CRITICAL, context
        )
