...
class ImageExportSettingsManager(QObject, IImageExportSettingsManager):
    """
    ...
    def get_export_format(self) -> str:
        ...
        """
        try:
            format_name = self.settings.value("export/format", self.DEFAULT_FORMAT, type=str)
            
            # Validate format
            if format_name not in self.SUPPORTED_FORMATS:
                logger.warning(f"Invalid export format: {format_name}, using {self.DEFAULT_FORMAT}")
    ...
    def get_export_quality(self) -> int:
        ...
        """
        try:
            quality = self.settings.value("export/quality", self.DEFAULT_QUALITY, type=int)
    ...
    def set_export_dimensions(self, width: int, height: int) -> bool:
        ...
        try:
            if width <= 0 or height <= 0:
                logger.warning(f"Invalid export dimensions: {width}x{height} (must be positive)")
                return False
            
            # Check for reasonable limits (avoid memory issues)
            max_dimension = 32768  # 32K pixels
            if width > max_dimension or height > max_dimension:
                logger.warning(f"Export dimensions too large: {width}x{height} (max {max_dimension})")
                return False
            ...
            if old_dimensions != (width, height):
                ...
                logger.info(f"Export dimensions changed from {old_dimensions[0]}x{old_dimensions[1]} to {width}x{height}")
    ...
    def get_include_background(self) -> bool:
        ...
        """
        try:
            return self.settings.value("export/include_background", self.DEFAULT_INCLUDE_BACKGROUND, type=bool)
        except Exception as e:
    ...
    def get_scale_factor(self) -> float:
        ...
        """
        try:
            scale = self.settings.value("export/scale_factor", self.DEFAULT_SCALE_FACTOR, type=float)
    ...
    def get_estimated_file_size(self, format_name: str = None, width: int = None, 
                               height: int = None, quality: int = None) -> str:
        """
        Get estimated file size for current settings.
        
        Args:
            format_name: Format (uses current if None)
            width: Width (uses current if None)
            height: Height (uses current if None)
            quality: Quality (uses current if None)
            
        Returns:
            Estimated file size as a human-readable string
        """
        try:
            if format_name is None:
                format_name = self.get_export_format()
            ...
            if format_name == "PNG":
                # PNG: roughly 3-4 bytes per pixel for typical content
                size_bytes = pixels * 3.5
            elif format_name == "JPEG":
                # JPEG: varies significantly with quality
                compression_ratio = max(10, 100 - quality) / 10
                size_bytes = pixels * 3 / compression_ratio
            elif format_name in ["SVG", "PDF"]:
                # Vector formats: much smaller, depends on complexity
                size_bytes = min(100000, pixels / 100)  # Rough estimate
            elif format_name == "TIFF":
                # TIFF: similar to PNG but can be larger
                size_bytes = pixels * 4
            elif format_name == "BMP":
                # BMP: uncompressed, 3 bytes per pixel
                size_bytes = pixels * 3
            else:
                size_bytes = pixels * 3  # Default estimate
