...
class UserProfileSettingsManager(QObject, IUserProfileSettingsManager):
    """
    ...
    def get_current_user(self) -> str:
        ...
        """
        try:
            return self.settings.value("users/current_user", self.DEFAULT_USER, type=str)
    ...
    def get_user_setting(self, username: str, setting_key: str, default: Any = None) -> Any:
        """
        ...
        try:
            ...
            logger.error(f"Failed to get user setting {setting_key} for {username}: {e}")
            return default
    ...
    def set_user_setting(self, username: str, setting_key: str, value: Any) -> None:
        ...
        try:
            ...
            if self._is_global_setting(setting_key):
                logger.warning(f"Setting {setting_key} is global and should not be user-specific")
                return
            ...
            if old_value != value:
                ...
                logger.debug(f"User setting {setting_key} for {username} changed to {value}")
                
        except Exception as e:
            logger.error(f"Failed to set user setting {setting_key} for {username}: {e}")
    ...
    def copy_user_settings(self, from_user: str, to_user: str) -> bool:
        ...
        try:
            ...
            logger.info(f"Copied {len(source_settings)} settings from {from_user} to {to_user}")
            return True
            
        except Exception as e:
            logger.error(f"Failed to copy user settings from {from_user} to {to_user}: {e}")
            return False
