...
class FramePoolService:
    ...
    def checkout_frame(self) -> Optional["OptionPictograph"]:
        ...
        Returns the first available (hidden) frame, or reuses the first frame
        if all are in use. This matches Legacy behavior.

        Returns:
            Available PictographOptionFrame or None if pool not initialized
        """
        if not self._initialized:
            logger.error("Frame pool not initialized - call initialize_pool() first")
            return None

        if not self._pool:
            logger.error("Frame pool is empty")
            return None

        # Find first available (hidden) frame
        for frame in self._pool:
            if not frame.isVisible():
                self._checkout_count += 1
                logger.debug(
                    f"Checked out frame (total checkouts: {self._checkout_count})"
                )
                return frame

        # If no available frame, reuse the first one (Legacy behavior)
        frame = self._pool[0]
        self._checkout_count += 1
    ...
    def get_pool_statistics(self) -> dict:
        ...
        """
        visible_count = sum(1 for frame in self._pool if frame.isVisible())

        return {
            "total_frames": len(self._pool),
            "visible_frames": visible_count,
            "available_frames": len(self._pool) - visible_count,
