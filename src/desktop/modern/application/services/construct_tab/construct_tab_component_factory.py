...
class ConstructTabComponentFactory(IConstructTabComponentFactory):
    ...
    def create_workbench(self) -> tuple[QWidget, Any]:
        ...
        try:
            # Try to use the new simplified workbench
            from desktop.modern.presentation.components.workbench.simplified_workbench import (
                SimplifiedSequenceWorkbench,
            )
            ...
            # Create workbench
            workbench = SimplifiedSequenceWorkbench(
            ...
            print(f"Warning: Could not create simplified workbench: {e}")

            # Fallback to original workbench
            from desktop.modern.presentation.components.sequence_workbench.sequence_workbench import (
                SequenceWorkbench,
            )

            # Resolve dependencies for original workbench
            layout_service = self._container.resolve(ILayoutService)
            beat_selection_service: BeatSelectionService = self._container.resolve(
                "BeatSelectionService"
            )
            ...
            self._report_progress(20, "Workbench created (fallback)")
            return widget, workbench
    ...
    def create_generate_panel(self) -> tuple[QWidget, Any]:
        ...
        try:
            # Import here to avoid circular dependencies
            from desktop.modern.presentation.components.generate_controls.generate_controls import (
                GenerateControls,
            )

            # Create generate panel
            generate_panel = GenerateControls(self._container)
            generate_panel.initialize()
            widget = generate_panel.get_widget()
