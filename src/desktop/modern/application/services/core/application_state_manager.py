...
from desktop.modern.core.dependency_injection.settings_service_registration import register_settings_services
...
class ApplicationStateManager:
    ...
    def initialize_application_state(self) -> bool:
        ...
        try:
            ...
            if restored_memento:
                logger.info("📂 Restored application state from previous session")
                
                # Apply window state if main window is available
                if self.main_window and restored_memento.window_geometry:
                    try:
                        self.main_window.restoreGeometry(restored_memento.window_geometry)
                ...
                if session_result.success and session_result.session_restored:
                    logger.info("📊 Restored session state")
    ...
    def export_application_state(self, file_path: Optional[Path] = None) -> bool:
        ...
        try:
            ...
            if success:
                logger.info(f"📤 Application state exported to {file_path or 'default location'}")
    ...
    def get_state_statistics(self) -> dict:
        ...
        try:
            ...
            stats = {
                ...
                "last_interaction": current_session.last_interaction.isoformat() if current_session else None,
...
def create_application_state_manager(container: DIContainer, 
                                   main_window: Optional[QMainWindow] = None) -> ApplicationStateManager:
    """
    ...
    try:
        ...
        manager = ApplicationStateManager(settings_service, session_tracker, main_window)
...
def integrate_with_application_startup(container: DIContainer, 
                                     main_window: QMainWindow,
                                     app: QApplication) -> ApplicationStateManager:
    """
    ...
    """
    try:
        logger.info("🚀 Integrating state persistence with application startup...")
        ...
        logger.info(f"✅ State persistence integration complete (restored: {state_restored})")
...
def setup_state_persistence_for_window(main_window: QMainWindow, 
                                     container: DIContainer) -> ApplicationStateManager:
    """
