...
class TabDefinition:
    ...
    def __init__(self, tab_id: str, display_name: str, creator_func: Callable, is_required: bool = True):
        self.tab_id = tab_id
...
class TabFactory:
    ...
    def __init__(self):
        ...
        self.tab_definitions = [
            TabDefinition("construct", "🔧 Construct", self._create_construct_tab, True),
            TabDefinition("browse", "🔍 Browse", self._create_browse_tab, True),
            TabDefinition("learn", "🧠 Learn", self._create_learn_tab, False),
            TabDefinition("sequence_card", "📋 Sequence Card", self._create_sequence_card_tab, False),
        ]
        ...
        for tab_def in self.tab_definitions:
            try:
                ...
                if tab_def.is_required:
                    logger.error(f"❌ Required tab {tab_def.tab_id} failed, using fallback")
                else:
                    logger.warning(f"⚠️ Optional tab {tab_def.tab_id} failed, using fallback")
    ...
    def _create_browse_tab(self, container: "DIContainer") -> QWidget:
        ...
        from desktop.modern.presentation.tabs.browse.browse_tab_with_services import BrowseTab
    ...
    def get_creation_status(self) -> Dict[str, bool]:
        ...
        return {tab_id: tab_id in self.created_tabs for tab_id in [td.tab_id for td in self.tab_definitions]}
