...
class ConstructTabView(QWidget):
    ...
    def get_workbench(self):
        """Get workbench reference from layout manager."""
        if self._layout_manager:
            return getattr(self._layout_manager, "workbench", None)
        return None

    def get_layout_manager(self) -> Optional[ConstructTabLayoutManager]:
        """Get the layout manager for controller integration."""
