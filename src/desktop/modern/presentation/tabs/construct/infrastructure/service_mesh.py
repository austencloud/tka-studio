...
from desktop.modern.core.events.event_bus import get_event_bus, IEventBus
...
class ServiceProxy:
    ...
    def __init__(self, target_service: Any, service_name: str, event_bus: IEventBus):
        ...
        self.circuit_breaker = CircuitBreaker(
            CircuitBreakerConfig(), service_name
        )
    ...
    def _wrap_method(self, method: Callable, method_name: str) -> Callable:
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                duration = time.time() - start_time
                self._publish_metrics_event(method_name, duration, success=False, error=str(e))
                
                raise e
        
        return wrapper
    
    def _publish_metrics_event(self, method_name: str, duration: float, 
...
class ServiceRegistry:
    ...
    def get(self, service_name: str) -> Optional[ServiceProxy]:
...
class ComponentServiceMesh:
    ...
    def get_service(self, service_name: str) -> Optional[ServiceProxy]:
        """Get service by name."""
        return self.registry.get(service_name)
    
    def setup_mesh_for_construct_tab(self, components: Dict[str, Any]) -> Dict[str, ServiceProxy]:
        ...
        self.logger.info(f"Service mesh setup complete with {len(proxied_components)} components")
    ...
    def publish_mesh_status(self):
        """Publish mesh status through event bus.""" 
        status = self.get_mesh_status()
        
        event = UIStateChangedEvent(
            component="service_mesh",
            state_key="mesh_status",
            old_value=None,
            new_value=status,
            source="service_mesh"
        )
...
def create_service_mesh_for_construct_tab(components: Dict[str, Any]) -> ComponentServiceMesh:
