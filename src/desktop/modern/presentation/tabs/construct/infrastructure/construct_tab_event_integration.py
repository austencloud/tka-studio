...
class ConstructTabEventIntegration:
    ...
    def _subscribe_to_sequence_events(self):
        ...
        self.event_bus.subscribe(
            "sequence.created",
            self._handle_sequence_created
        )
        
        self.event_bus.subscribe(
            "sequence.beat_added", 
            self._handle_beat_added
        )
        
        self.event_bus.subscribe(
            "sequence.beat_updated",
            self._handle_beat_updated  
        )
        
        self.event_bus.subscribe(
            "sequence.beat_removed",
            self._handle_beat_removed
        )
    ...
    def _subscribe_to_ui_events(self):
        ...
        self.event_bus.subscribe(
            "ui.start_position.state_changed",
            self._handle_start_position_state_change
        )
        
        self.event_bus.subscribe(
            "ui.option_picker.state_changed", 
            self._handle_option_picker_state_change
        )
    ...
    def publish_beat_updated(self, sequence_id, beat_number, field, old_value, new_value):
