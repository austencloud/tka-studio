...
from desktop.modern.core.commands.command_system import ICommand, CommandResult
...
class SetupServiceMeshStep(SagaStep):
    ...
    async def execute(self, context: SagaContext) -> SagaStepResult:
        ...
        try:
            ...
            components = {
                "workbench": context.shared_data.get("workbench"),
                "start_position_picker": context.shared_data.get("start_position_picker"),
                "option_picker": context.shared_data.get("option_picker"),
            }
            
            # Setup service mesh
            proxied_components = self.service_mesh.setup_mesh_for_construct_tab(components)
...
class ConstructTabInitializationSaga:
    ...
    async def execute(self, progress_callback: Optional[Callable] = None) -> bool:
        ...
        for step in reversed(self.executed_steps):
            try:
                ...
                if result.success:
                    ...
                    self.logger.error(f"Compensation failed for {step.step_id}: {result.error_message}")
