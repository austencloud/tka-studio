...
from desktop.modern.core.events.event_bus import get_event_bus, TypeSafeEventBus
...
class ConstructTabEventIntegration:
    ...
    def __init__(self, event_bus: Optional[TypeSafeEventBus] = None):
    ...
    def _subscribe_to_events(self):
        ...
        self.subscription_ids["ui_state_changed"] = self.event_bus.subscribe(
            "ui.start_position_picker.state_changed", self._handle_start_position_selected
        )
    ...
    def _connect_component_signals(self):
        ...
        if start_position_picker and hasattr(start_position_picker, "start_position_selected"):
        ...
        if option_picker and hasattr(option_picker, "pictograph_selected"):
            option_picker.pictograph_selected.connect(
                self._publish_pictograph_selected
            )
        ...
        if workbench:
            if hasattr(workbench, "sequence_modified"):
                workbench.sequence_modified.connect(
                    self._publish_sequence_modified
                )
    ...
    def _publish_pictograph_selected(self, pictograph_data: Dict[str, Any]):
        ...
        self.event_bus.publish(event)
        self.logger.info(f"Published pictograph selected: {pictograph_data.get('letter', 'unknown')}")
    ...
    def publish_ui_state_change(self, component: str, state_key: str, old_value: Any, new_value: Any):
    ...
    def _handle_sequence_updated(self, event: SequenceUpdatedEvent):
        ...
        self.logger.info(f"Handling sequence updated: {event.sequence_id} - {event.change_type}")
    ...
    def _handle_beat_added(self, event: BeatAddedEvent):
        ...
        self.logger.info(f"Handling beat added: sequence {event.sequence_id}, beat {event.beat_position}")
    ...
    def _handle_beat_updated(self, event: BeatUpdatedEvent):
        ...
        self.logger.info(f"Handling beat updated: sequence {event.sequence_id}, beat {event.beat_number}")
    ...
    def _handle_beat_removed(self, event: BeatRemovedEvent):
        ...
        self.logger.info(f"Handling beat removed: sequence {event.sequence_id}, position {event.old_position}")
    ...
    def _is_start_position_set(self) -> bool:
        ...
        if workbench and hasattr(workbench, "_beat_frame_section"):
            ...
            if beat_frame_section and hasattr(beat_frame_section, "_start_position_data"):
    ...
    def _refresh_option_picker_from_workbench(self):
        ...
        if option_picker and workbench:
            if hasattr(option_picker, "refresh_from_sequence") and hasattr(workbench, "get_current_sequence"):
                current_sequence = workbench.get_current_sequence()
                if current_sequence:
                    option_picker.refresh_from_sequence(current_sequence)
    ...
    def redo_last_command(self):
        ...
        if result.success:
            self.logger.info("Command redone successfully")
        else:
            self.logger.error(f"Redo failed: {result.error_message}")
        return result
    ...
    def shutdown(self):
        ...
        for event_type, subscription_id in self.subscription_ids.items():
...
def create_event_integration(event_bus: Optional[TypeSafeEventBus] = None) -> ConstructTabEventIntegration:
    """Factory function to create event integration."""
