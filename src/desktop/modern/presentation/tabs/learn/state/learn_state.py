...
from desktop.modern.domain.models.learn import QuizSession, QuestionData, LessonResults
...
class ProgressState:
    ...
    time_remaining: Optional[int] = None  # seconds for countdown mode
...
class LearnState:
    ...
    lesson_results: Optional[LessonResults] = None
    ...
    def is_lesson_active(self) -> bool:
        ...
        return (self.current_view == LearnView.LESSON_WORKSPACE and 
    ...
    def can_transition_to_lesson(self) -> bool:
        ...
        return (self.current_view == LearnView.LESSON_SELECTOR and
                not self.is_in_error_state() and
                not self.ui_state.is_loading)
    
    def can_transition_to_results(self) -> bool:
        """Check if we can transition to results view."""
        return (self.current_view == LearnView.LESSON_WORKSPACE and
                self.current_session is not None and
                (self.current_session.is_completed or 
                 self.progress_state.questions_answered >= self.progress_state.total_questions))
    
    def can_answer_question(self) -> bool:
        """Check if user can answer the current question."""
        return (self.is_lesson_active() and
                self.current_question is not None and
                not self.ui_state.is_paused and
                not self.ui_state.show_feedback)
    
    def get_completion_percentage(self) -> float:
        """Get lesson completion percentage."""
        if self.progress_state.total_questions == 0:
            return 0.0
        return (self.progress_state.questions_answered / self.progress_state.total_questions) * 100.0
    
    def get_accuracy_percentage(self) -> float:
        """Get current accuracy percentage."""
        if self.progress_state.questions_answered == 0:
            return 0.0
        return (self.progress_state.correct_answers / self.progress_state.questions_answered) * 100.0
    ...
    def get_required_layout_mode(self) -> LayoutMode:
        ...
        return LayoutMode.HORIZONTAL if self.needs_horizontal_layout() else LayoutMode.VERTICAL
