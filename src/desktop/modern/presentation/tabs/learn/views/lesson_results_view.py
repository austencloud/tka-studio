...
class ResultsStatsWidget(QWidget):
    ...
    def update_stat(self, label: str, value: str, value_color: str = "white") -> None:
        """Update an existing statistic."""
        if label in self.stat_labels:
            _, value_label = self.stat_labels[label]
            ...
            new_style = re.sub(
                r"color:\s*[^;]+;", f"color: {value_color};", current_style
            )
...
class LessonResultsView(QWidget):
    ...
    def __init__(self, parent: Optional[QWidget] = None):
        ...
        self._current_results: Optional[LessonResults] = None
