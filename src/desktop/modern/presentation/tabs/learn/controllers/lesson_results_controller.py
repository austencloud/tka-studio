        ...
        try:
            ...
            if results.accuracy_percentage >= 90:
                ...
                insights["recommendations"].append(
                    "Review lesson materials and practice more"
                )

            # Analyze speed
            if (
                results.average_time_per_question
                and results.average_time_per_question < 5
            ):
                insights["strengths"].append("Quick response time")
            elif (
                results.average_time_per_question
                and results.average_time_per_question > 15
            ):
                insights["areas_for_improvement"].append(
                    "Consider working on response speed"
                )

            # Analyze completion
            completion_rate = (
                results.questions_answered / results.total_questions
            ) * 100
            if completion_rate == 100:
                insights["strengths"].append("Completed all questions")
            elif completion_rate < 80:
                insights["areas_for_improvement"].append(
                    "Try to complete more questions"
                )
