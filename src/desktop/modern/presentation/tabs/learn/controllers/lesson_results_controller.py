...
from ..state import LearnStateManager, SessionCreationError
from ..views import LessonResultsView
...
class LessonResultsController(QObject):
    ...
    def display_results(self, results: LessonResults) -> None:
        ...
        """
        try:
            logger.info(f"Displaying results for {results.lesson_type.value}: {results.accuracy_percentage:.1f}%")
    ...
    def _log_performance_summary(self, results: LessonResults) -> None:
        ...
        logger.info(f"Lesson Performance Summary:")
        ...
        logger.info(f"  Questions: {results.questions_answered}/{results.total_questions}")
    ...
    def _handle_restart_lesson(self, lesson_type: LessonType) -> None:
        ...
        try:
            ...
            if not quiz_mode:
                ...
                logger.warning("No quiz mode found in results, defaulting to fixed question mode")
            ...
            self.state_manager.set_error(SessionCreationError(
                lesson_type.value,
                "unknown",
    ...
    def get_current_results(self) -> LessonResults:
        """Get current lesson results."""
        current_state = self.state_manager.get_state()
        return current_state.lesson_results
    
    def export_results(self) -> dict:
        """Export results data for external use."""
        try:
            current_state = self.state_manager.get_state()
            if not current_state.lesson_results:
                return {}
            
            results = current_state.lesson_results
            return {
                "lesson_type": results.lesson_type.value,
                "quiz_mode": results.quiz_mode.value,
                "accuracy_percentage": results.accuracy_percentage,
                "questions_answered": results.questions_answered,
                "total_questions": results.total_questions,
                "correct_answers": results.correct_answers,
                "incorrect_guesses": results.incorrect_guesses,
                "completion_time_seconds": results.completion_time_seconds,
                "grade_letter": results.grade_letter,
                "performance_level": results.performance_level,
                "completed_at": results.completed_at.isoformat(),
                "average_time_per_question": results.average_time_per_question,
                "streak_longest_correct": results.streak_longest_correct
            }
    ...
    def get_performance_insights(self) -> dict:
        ...
        try:
            ...
            if results.accuracy_percentage >= 90:
                insights["strengths"].append("Excellent accuracy")
                insights["recommendations"].append("Try a more challenging lesson")
            elif results.accuracy_percentage >= 70:
                insights["strengths"].append("Good understanding of concepts")
                insights["recommendations"].append("Practice to improve consistency")
            else:
                insights["areas_for_improvement"].append("Accuracy needs improvement")
                insights["recommendations"].append("Review lesson materials and practice more")
            
            # Analyze speed
            if results.average_time_per_question and results.average_time_per_question < 5:
                insights["strengths"].append("Quick response time")
            elif results.average_time_per_question and results.average_time_per_question > 15:
                insights["areas_for_improvement"].append("Consider working on response speed")
            
            # Analyze completion
            completion_rate = (results.questions_answered / results.total_questions) * 100
            if completion_rate == 100:
                insights["strengths"].append("Completed all questions")
            elif completion_rate < 80:
                insights["areas_for_improvement"].append("Try to complete more questions")
