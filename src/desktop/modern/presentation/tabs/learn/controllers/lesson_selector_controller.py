...
from ..state import LearnStateManager, LessonNotAvailable, SessionCreationError
from ..views import LessonSelectorView
...
class LessonSelectorController(QObject):
    ...
    def set_default_mode(self, mode: QuizMode) -> None:
        """Set default quiz mode."""
        self.view.set_selected_mode(mode)

    def get_current_selection(self) -> tuple:
        """Get current lesson and mode selection."""
        mode = self.view.mode_toggle.get_selected_mode()
        return None, mode  # No lesson selected until button is clicked
