...
class AnswerOptions(QWidget):
    ...
    def disable_option(self, option: Any) -> None:
        """Disable a specific answer option."""
        widget = self.option_to_widget.get(option)
        if widget:
            widget.setEnabled(False)
            logger.debug(f"Disabled option: {option}")

    def enable_all_options(self) -> None:
        """Enable all answer options."""
        for widget in self.option_widgets:
            widget.setEnabled(True)
        logger.debug("Enabled all options")

    def disable_all_options(self) -> None:
        """Disable all answer options."""
        for widget in self.option_widgets:
            widget.setEnabled(False)
        logger.debug("Disabled all options")

    def clear_options(self) -> None:
        """Clear all options and reset state."""
        self._clear_options()
        self.current_question = None
        self.current_format = ""
        logger.debug("Answer options cleared")
