...
class GenerateTabController(QObject):
    ...
    def _create_fallback_services(self) -> None:
        ...
        logger.warning("Using fallback generation services")
        
        # Import fallback implementations
        from desktop.modern.application.services.generation.test_doubles.mock_generation_service import (
    ...
    def handle_generate_request(self, config: GenerationConfig) -> None:
        ...
        """
        try:
            logger.info(f"Generation requested: {config.mode.value}, length={config.length}")
    ...
    def reset_to_default_config(self) -> None:
        ...
        try:
            ...
            self._current_state = self._current_state.with_config(default_config)
            self._update_ui_if_available()
