    ...
    try:
        ...
        if result.success:
            ...
            if undo_result.success:
                ...
                logger.warning(f"⚠️ Start position command undo test FAILED: {undo_result.error_message}")
            ...
            if redo_result.success:
                ...
                logger.warning(f"⚠️ Start position command redo test FAILED: {redo_result.error_message}")
            ...
            logger.error(f"❌ Start position command execution test FAILED: {result.error_message}")
            return False
...
def test_beat_command():
    ...
    try:
        ...
        if result.success:
            ...
            logger.error(f"❌ Beat addition command test FAILED: {result.error_message}")
            return False
...
def test_state_manager():
    ...
    try:
        ...
        logger.info(f"   Initial sequence: {initial_sequence.length if initial_sequence else 0} beats")
        logger.info(f"   Initial start position: {initial_start_pos.letter if initial_start_pos else 'None'}")
...
def run_all_tests():
    ...
    if passed == total:
        logger.info("🎉 All tests PASSED! Event-driven architecture is working correctly.")
        return True
    else:
