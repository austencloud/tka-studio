...
class SequenceImageRegenerator:
    ...
    def progress_callback(self, progress: ExportProgress) -> None:
        ...
        logger.info(f"Progress: {percentage:.1f}% ({progress.current}/{progress.total}) - {progress.message}")
    ...
    def regenerate_all_images(
        ...
        try:
            ...
            logger.info(f"Starting image regeneration...")
...
def main():
    ...
    parser.add_argument(
        "--source",
        type=Path,
        help="Source directory containing sequence data"
    )
    parser.add_argument(
        "--export",
        type=Path,
        help="Export directory for generated images"
    )
    parser.add_argument(
        "--force",
        action="store_true",
