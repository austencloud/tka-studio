    ...
    def verify_all_images(self, export_path: Path = None) -> bool:
        ...
        try:
            ...
            for i, (word, image_path) in enumerate(image_files):
                logger.info(f"Verifying {i+1}/{len(image_files)}: {word}/{image_path.name}")
            ...
            success_rate = self.verification_results["valid_images"] / self.verification_results["total_images"]
    ...
    def _verify_cache_compatibility(self, word: str, image_path: Path) -> None:
        ...
        try:
            ...
            if hasattr(self.cache_service, 'load_image'):
                cached_image = self.cache_service.load_image(sequence_id, str(image_path))
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Verify exported TKA sequence card images")
    parser.add_argument(
        "--export-path",
        type=Path,
        help="Path to exported images directory"
    )
    parser.add_argument(
        "--verbose",
        action="store_true",
