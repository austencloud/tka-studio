...
def create_data_config_from_env() -> DataConfig:
    """
    ...
    Raises:
        ValueError: If TKA_DATA_DIR environment variable is not set
    """
    env_path = os.environ.get("TKA_DATA_DIR")
    if not env_path:
...
def create_data_config_with_fallback() -> DataConfig:
    ...
    This is for cases where you need a config and can't handle exceptions.
    ...
    try:
        return create_data_config()
    except Exception as e:
        # Log the error and create a fallback config
        logger.warning(f"Failed to create data config: {e}")
