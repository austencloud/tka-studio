...
class QtProfiler(QObject if QT_AVAILABLE else object):
    ...
    def __init__(self, parent: Optional[QObject] = None, config: Optional[PerformanceConfig] = None):
        if QT_AVAILABLE:
    ...
    def start_profiling(self) -> Result[bool, AppError]:
        ...
        if not QT_AVAILABLE:
            return failure(app_error(
                ...
                "PyQt6 not available for Qt profiling"
            ))

        if not self.config.monitoring.qt_metrics:
            return failure(app_error(
                ErrorType.CONFIG_ERROR,
                "Qt metrics disabled in configuration"
            ))
    ...
    def eventFilter(self, obj: QObject, event: QEvent) -> bool:
        ...
        result = super().eventFilter(obj, event) if hasattr(super(), 'eventFilter') else False
