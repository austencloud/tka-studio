...
class EventLogger:
    ...
    def _setup_subscriptions(self):
        ...
        try:
            ...
            self.event_bus.subscribe("sequence.start_position_set", self._log_sequence_event)
            
            logger.info("🔍 EventLogger subscriptions setup complete")
...
def enable_event_logging():
    ...
    if _event_logger:
        ...
        logger.warning("⚠️ Event logger not initialized - call setup_event_logger() first")
...
def log_debug_info():
    ...
    try:
        from core.service_locator import get_sequence_state_manager, get_command_processor
        ...
        if state_manager:
            ...
            logger.info(f"   Start Position: {start_pos.letter if start_pos else 'None'}")
        else:
        ...
        if command_processor:
            ...
            if command_processor.can_undo():
                logger.info(f"   Undo Description: {command_processor.get_undo_description()}")
            if command_processor.can_redo():
                logger.info(f"   Redo Description: {command_processor.get_redo_description()}")
        else:
