...
class SmartLogger:
            ...
            def wrapper(*args, **kwargs):
                ...
                try:
                    ...
                    if duration_ms > self.config.performance_threshold_ms:
                        ...
                        if level in [LogLevel.VERBOSE, LogLevel.DEBUG]:
                            self._log_operation_details(operation_name, args, kwargs, result)
                    else:
                    ...
                    if self.config.error_always_verbose:
                        self._log_operation_details(operation_name, args, kwargs, None, error=e)
    ...
    def log_batch_operation(self, batch_name: str, items_processed: int, 
                          total_duration_ms: float, errors: int = 0):
    ...
    def log_with_suppression(self, level: int, message: str, suppress_key: str = None):
        ...
        if suppress_key in self.repeated_log_counts:
            ...
            if self.repeated_log_counts[suppress_key] <= self.config.max_repeated_logs:
                ...
                self.logger.log(level, f"{message} (repeated {self.repeated_log_counts[suppress_key]} times)")
        else:
            self.repeated_log_counts[suppress_key] = 1
            self.logger.log(level, message)
    ...
    def _log_operation_details(self, operation_name: str, args: tuple, 
                              kwargs: dict, result: Any, error: Exception = None):
