...
class ArrowPositioningLogger:
    ...
    def log_directional_processing(self, operation_name: str = "directional_processing"):
        ...
        def decorator(func):
            def wrapper(*args, **kwargs):
                ...
                try:
                    ...
                    if duration_ms > 20.0:  # 20ms threshold for individual tuple processing
                        self.smart_logger.logger.warning(
                            f"⚠️ SLOW directional processing: {duration_ms:.1f}ms"
                        )
                    
                    # Suppress the detailed "Generated directional tuples" logs
                    # Suppress the "Quadrant index" logs  
                    # Suppress the "Final adjustment" logs
                    
                    return result
                    
                except Exception as e:
                    if self.current_letter:
                        self.letter_operations[self.current_letter]['errors'].append(str(e))
    ...
    def log_lookup_operation(self, operation_name: str = "adjustment_lookup"):
        ...
        def decorator(func):
            def wrapper(*args, **kwargs):
                ...
                # Suppress "Using special placement" and "Using default calculation" logs
                # These are now handled by the summary
    ...
    def log_sequence_positioning_summary(self, sequence_name: str, total_letters: int,
                                       total_arrows: int, total_duration_ms: float,
                                       error_count: int = 0):
...
def get_arrow_positioning_logger(config: LoggingConfig = None) -> ArrowPositioningLogger:
