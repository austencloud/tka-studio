...
def setup_arrow_positioning_logging_only(quiet: bool = True) -> None:
    """
    Quick setup to reduce arrow positioning verbosity without changing other logging.
    
    This is perfect for immediate relief from verbose positioning logs.
    
    Args:
        quiet: If True, uses production-level suppression
    """
    config = LoggingEnvironments.get_production_config() if quiet else LoggingEnvironments.get_development_config()
    
    # Just configure arrow positioning
    arrow_logger = get_arrow_positioning_logger(config)
    
    print(f"🔧 Arrow positioning logging configured (quiet={quiet})")
...
def _detect_environment() -> str:
    ...
    if any(test_indicator in os.environ for test_indicator in ['PYTEST_CURRENT_TEST', 'TESTING']):
        return 'testing'
...
def enable_quiet_mode():
    """Enable quiet mode - minimal logging for production-like behavior."""
    setup_arrow_positioning_logging_only(quiet=True)
    
    # Also reduce other common noise
    ...
    print("🔇 Quiet mode enabled - minimal logging")
...
def configure_from_environment():
    ...
    # TKA_QUIET_POSITIONING: Suppress arrow positioning verbosity
    if os.getenv('TKA_QUIET_POSITIONING', '').lower() in ['true', '1', 'yes']:
        setup_arrow_positioning_logging_only(quiet=True)
