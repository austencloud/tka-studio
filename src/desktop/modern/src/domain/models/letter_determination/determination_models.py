...
class LetterDeterminationResult:
    ...
    @classmethod
    def success(cls, letter: Letter, confidence: float, strategy: str, 
                attributes: Dict[str, Any] = None) -> 'LetterDeterminationResult':
...
class MotionComparisonContext:
    ...
    def __post_init__(self):
        ...
        if not (0.0 <= self.tolerance_threshold <= 1.0):
            raise ValueError(f"Tolerance threshold must be between 0.0 and 1.0, got: {self.tolerance_threshold}")
    
    @property
    def is_strict_matching(self) -> bool:
        """Check if strict matching mode is enabled."""
        return (self.strict_orientation_matching and 
                not self.swap_prop_rot_dir and
                self.tolerance_threshold < 0.01)
