...
class SequenceCardDisplayAdaptor(QObject):
    ...
    sequences_loaded = pyqtSignal(list)  # Emitted when sequences are loaded and ready for display
    ...
    def __init__(self, display_service: ISequenceCardDisplayService, parent: Optional[QObject] = None):
        """
    ...
    def _on_sequences_loaded(self, sequences: List[SequenceCardData]) -> None:
        ...
        logger.info(f"Adaptor: sequences loaded, emitting signal with {len(sequences)} sequences")
    ...
    def display_sequences(self, length: int, column_count: int) -> None:
        ...
        logger.info(f"Adaptor: display_sequences called with length={length}, columns={column_count}")
