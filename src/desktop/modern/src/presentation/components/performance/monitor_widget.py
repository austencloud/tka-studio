...
try:
    from PyQt6.QtWidgets import (QWidget, QVBoxLayout, QHBoxLayout, QLabel, 
                                ...
                                QTabWidget, QPushButton, QTextEdit, QGroupBox,
                                QGridLayout, QFrame)
    ...
    def update_memory_display(self):
        ...
        try:
            ...
            if current_memory > self.config.profiling.memory_threshold_mb:
                self.current_memory_label.setStyleSheet("color: red; font-weight: bold;")
                self.performance_alert.emit("memory", f"Memory usage ({current_memory:.1f}MB) exceeds threshold")
            else:
    ...
    def update_functions_display(self):
        ...
        try:
            ...
            for i, func_metrics in enumerate(bottlenecks):
                ...
                self.functions_table.setItem(i, 1, QTableWidgetItem(str(func_metrics.call_count)))
                self.functions_table.setItem(i, 2, QTableWidgetItem(f"{func_metrics.total_time * 1000:.1f}"))
                self.functions_table.setItem(i, 3, QTableWidgetItem(f"{func_metrics.avg_time * 1000:.1f}"))
                self.functions_table.setItem(i, 4, QTableWidgetItem(f"{func_metrics.memory_total:.1f}"))
        ...
        try:
            if hasattr(self.qt_profiler, 'event_metrics'):
                ...
                for i, event_metrics in enumerate(events):
                    self.qt_events_table.setItem(i, 0, QTableWidgetItem(event_metrics.event_type))
                    self.qt_events_table.setItem(i, 1, QTableWidgetItem(str(event_metrics.count)))
                    self.qt_events_table.setItem(i, 2, QTableWidgetItem(f"{event_metrics.total_time * 1000:.1f}"))
                    self.qt_events_table.setItem(i, 3, QTableWidgetItem(f"{event_metrics.avg_time * 1000:.1f}"))
    ...
    def update_recommendations_display(self):
        ...
        try:
            ...
            for i, rec in enumerate(recommendations[:10], 1):  # Show top 10
                ...
                text += f"   {rec.get('recommendation', 'No recommendation available')}\n\n"
