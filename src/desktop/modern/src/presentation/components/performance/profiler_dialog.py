...
class ProfilerControlDialog(QDialog if QT_AVAILABLE else object):
    ...
    def load_configuration(self):
        ...
        try:
            ...
            self.overhead_threshold.setValue(self.config.profiling.overhead_threshold_percent)
            self.bottleneck_threshold.setValue(self.config.profiling.bottleneck_threshold_ms)
            self.memory_threshold.setValue(int(self.config.profiling.memory_threshold_mb))
    ...
    def reset_configuration(self):
        ...
        try:
            ...
            self.overhead_threshold.setValue(default_config.profiling.overhead_threshold_percent)
            self.bottleneck_threshold.setValue(default_config.profiling.bottleneck_threshold_ms)
            self.memory_threshold.setValue(int(default_config.profiling.memory_threshold_mb))
        ...
        try:
            ...
            if result.is_success():
                ...
                QMessageBox.information(
                    self,
                    "Session Stopped",
                    "Profiling session stopped successfully."
                )
            else:
                QMessageBox.critical(
                    self,
                    "Session Error",
                    f"Failed to stop session: {result.error}"
                )
    ...
    def refresh_status(self):
        ...
        try:
            ...
            if self.profiler.current_session:
                ...
                status_info.append(f"Functions Tracked: {len(self.profiler._function_stats)}")
            
            # Configuration status
            status_info.append(f"\nConfiguration:")
            status_info.append(f"  Profiling Enabled: {self.config.profiling.enabled}")
            status_info.append(f"  Monitoring Enabled: {self.config.monitoring.enabled}")
            status_info.append(f"  Memory Tracking: {self.config.monitoring.memory_tracking}")
