...
class GraphEditor(QFrame):
    ...
    def set_selected_start_position(self, start_position_data) -> bool:
        ...
        try:
            # Basic validation for beat data
            ...
            # Update adjustment panel if available
            if self._adjustment_panel:
                self._adjustment_panel.set_start_position_data(start_position_data)

            logger.debug("Start position data set successfully")
            return True
    ...
    def set_visibility(self, is_visible: bool) -> bool:
        """Set the visibility state with error handling."""
        try:
    ...
    def validate_current_state(self) -> ValidationResult:
        ...
        try:
            ...
            # Validate beat index consistency
            if self._selected_beat_index is not None and self._current_sequence:
                if not (
                    0 <= self._selected_beat_index < len(self._current_sequence.beats)
                ):
                    result.add_error(
                        f"Beat index {self._selected_beat_index} out of range",
                        "selected_beat_index",
