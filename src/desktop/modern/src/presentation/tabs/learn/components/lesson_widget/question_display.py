...
class QuestionDisplay(QWidget):
    ...
    def _display_pictograph(self, pictograph_data: Any) -> None:
        ...
        """
        try:
            logger.info(f"🎨 Displaying pictograph question: {type(pictograph_data)}")

            # Extract PictographData from the question data structure
            actual_pictograph_data = None
            if isinstance(pictograph_data, dict) and "data" in pictograph_data:
                actual_pictograph_data = pictograph_data["data"]
                logger.info(
                    f"   Extracted PictographData: {type(actual_pictograph_data)}"
                )
            elif isinstance(pictograph_data, PictographData):
                actual_pictograph_data = pictograph_data
                logger.info("   Direct PictographData provided")
            else:
                logger.warning(
                    f"   Unexpected pictograph data format: {type(pictograph_data)}"
                )

            if actual_pictograph_data and isinstance(
                actual_pictograph_data, PictographData
            ):
                # Create direct pictograph view for learn tab (no widget wrapper)
                pictograph_widget = create_learn_view(parent=self, context="question")
                ...
                # Set reasonable size constraints for Learn Tab context
                pictograph_widget.setMinimumSize(200, 200)
                pictograph_widget.setMaximumSize(600, 600)

                # Fit to container as a square for optimal display
                container_size = min(600, 600)  # Use max size as container reference
                pictograph_widget.fit_to_container(
                    container_size, container_size, maintain_square=True
                )
                ...
                self.content_widget = pictograph_widget
                self.layout.addWidget(self.content_widget)

                logger.info(
                    "✅ Displayed real pictograph using direct view with auto-scaling"
                )
            else:
                # Fallback to placeholder if no valid pictograph data
                self._display_pictograph_placeholder(
                    f"Invalid pictograph data: {type(pictograph_data)}"
                )

        except Exception as e:
            logger.error(f"Failed to display pictograph: {e}")
            self._display_pictograph_placeholder(f"Error: {e}")

    def _display_pictograph_placeholder(
        self, message: str = "Pictograph Display\n(Implementation Pending)"
    ) -> None:
        """Display a placeholder when pictograph rendering fails."""
        pictograph_label = QLabel(message)
        ...
        logger.debug(f"Displayed pictograph placeholder: {message}")
