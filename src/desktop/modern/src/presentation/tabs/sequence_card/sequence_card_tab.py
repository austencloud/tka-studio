...
class SequenceCardTab(QWidget):
    ...
    def _setup_connections(self) -> None:
        ...
        # Display service signals
        if hasattr(self.display_service, 'loading_state_changed'):
            self.display_service.loading_state_changed.connect(self._on_loading_state_changed)
        if hasattr(self.display_service, 'progress_updated'):
            self.display_service.progress_updated.connect(self._on_progress_updated)
    ...
    def _on_column_count_changed(self, column_count: int) -> None:
        ...
        # Update header description
        self.header.set_description_text(f"Updating display to show {column_count} preview columns...")
        ...
        self.display_service.display_sequences(current_length, column_count)
    ...
    def showEvent(self, event) -> None:
        """Handle tab show event - initialize on first show."""
        ...
        if not self.initialized:
            ...
            # Set up navigation with saved settings
            ...
            QTimer.singleShot(50, lambda: self._initialize_content(saved_length, saved_columns))
            
            # Emit state change
            self.tab_state_changed.emit("sequence_card_initialized")

    def _initialize_content(self, length: int, column_count: int) -> None:
        """Initialize tab content."""
        try:
            logger.info(f"Initializing content with length {length}, columns {column_count}")
            self.display_service.display_sequences(length, column_count)
