...
class ThumbnailGenerationService:
    ...
    def _convert_to_legacy_format(self, sequence: SequenceData) -> Optional[list]:
        ...
        try:
            ...
            legacy_sequence = self._legacy_data_converter.convert_modern_to_legacy_sequence(sequence)
            
            logger.debug(f"Converted sequence to legacy format: {len(legacy_sequence)} entries")
    ...
    def _generate_with_legacy_system(
        ...
        try:
            ...
            if not self._legacy_thumbnail_generator:
                self._legacy_thumbnail_generator = self._get_legacy_thumbnail_generator()
            ...
            thumbnail_path = self._legacy_thumbnail_generator.generate_and_save_thumbnail(
