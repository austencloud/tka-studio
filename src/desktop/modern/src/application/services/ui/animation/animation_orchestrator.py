...
class ModernAnimationOrchestrator(IAnimationOrchestrator):
    ...
    def __init__(
        ...
        renderer: QtAnimationRenderer,
        event_bus: IEventBus,
        stack_adapter: QtStackWidgetAdapter,
        effect_manager: QtGraphicsEffectManager,
        settings_integration: QtSettingsIntegration
    ):
    ...
    async def fade_stack_transition(
        ...
        fade_out_id = await self.fade_target(current_target, fade_in=False, config=config)
    ...
    def _on_animation_frame(self, event) -> None:
        ...
        if event.metadata:
            ...
            if property_name and value is not None:
                ...
                asyncio.create_task(self.renderer.render_frame(
                    event.target,
                    property_name,
                    value,
                    event.progress
...
class LegacyFadeManagerWrapper:
    ...
    def _create_widget_fader(self):
        ...
        class WidgetFaderAdapter:
            ...
            def fade_widgets(self, widgets, fade_in, duration=250, callback=None):
                config = AnimationConfig(duration=duration/1000.0)  # Convert ms to seconds
            ...
            def fade_and_update(self, widgets, callback, duration=250):
                ...
                async def run_fade_and_update():
                    await self.orchestrator.transition_targets(widgets, update_callback, config)
                    if final_callback:
                        final_callback()
    ...
    def _create_parallel_stack_fader(self):
        ...
        class ParallelStackFaderAdapter:
            ...
            def fade_both_stacks(self, right_stack, right_new_index, left_stack, 
                               left_new_index, width_ratio, duration=300, callback=None):
                config = AnimationConfig(duration=duration/1000.0)
                ...
                async def run_parallel_fade():
                    ...
                    if right_stack.currentWidget():
                        tasks.append(self.orchestrator.fade_target(
...
def create_modern_animation_system(settings_coordinator=None) -> tuple[ModernAnimationOrchestrator, LegacyFadeManagerWrapper]:
    """
    ...
    from .adapters.qt_adapters import create_qt_animation_components
