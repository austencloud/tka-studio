...
class WindowResizeCoordinator(QObject):
    ...
    def register_component(self, component: IPictographRescalable) -> None:
        ...
        if component not in self._registered_components:
            ...
            print(f"🔧 [RESIZE_COORDINATOR] Registered component: {type(component).__name__}")
            ...
            if self._current_window_width:
                try:
                    ...
                    print(f"⚠️ [RESIZE_COORDINATOR] Error scaling component on registration: {e}")
    ...
    def unregister_component(self, component: IPictographRescalable) -> None:
        ...
        if component in self._registered_components:
            ...
            print(f"🔧 [RESIZE_COORDINATOR] Unregistered component: {type(component).__name__}")
    ...
    def notify_window_resize(self, new_width: int) -> None:
        ...
        if (self._current_window_width is None or 
            abs(new_width - self._current_window_width) >= self._resize_threshold):
            
            old_width = self._current_window_width
            self._current_window_width = new_width
            
            print(f"🔧 [RESIZE_COORDINATOR] Window resized: {old_width} → {new_width}px")
    ...
    def _handle_window_resize(self, new_width: int) -> None:
        ...
        if not self._registered_components:
            print(f"🔧 [RESIZE_COORDINATOR] No components registered for re-scaling")
            return
        
        print(f"🔧 [RESIZE_COORDINATOR] Re-scaling {len(self._registered_components)} components for width {new_width}px")
        
        successful_rescales = 0
        failed_rescales = 0
        
        for component in self._registered_components[:]:  # Copy list to avoid modification during iteration
            try:
                component.rescale_for_window_size(new_width)
                successful_rescales += 1
            except Exception as e:
                print(f"⚠️ [RESIZE_COORDINATOR] Error re-scaling component {type(component).__name__}: {e}")
                failed_rescales += 1
                ...
                if hasattr(component, '_rescale_failures'):
                    ...
                    if component._rescale_failures >= 3:
                        ...
                        print(f"⚠️ [RESIZE_COORDINATOR] Removed failing component: {type(component).__name__}")
                else:
                    component._rescale_failures = 1
        
        print(f"✅ [RESIZE_COORDINATOR] Re-scaling complete: {successful_rescales} successful, {failed_rescales} failed")
    ...
    def clear_all_components(self) -> None:
        ...
        count = len(self._registered_components)
        self._registered_components.clear()
        print(f"🔧 [RESIZE_COORDINATOR] Cleared {count} registered components")
