...
class SequenceMetadataExtractionService:
    ...
    def _process_raw_metadata(self, raw_metadata: Dict[str, Any], image_path: str) -> Dict[str, Any]:
    ...
    def _extract_length_from_filename(self, filename: str) -> int:
        ...
        for length in [16, 12, 10, 8, 6, 5, 4, 3, 2]:
            if f"_{length}_" in filename or f"length_{length}" in filename or filename.endswith(f"_{length}"):
                return length
