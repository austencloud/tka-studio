...
from core.interfaces.learn_services import ILearnDataService
from core.interfaces.organization_services import IFileSystemService
from domain.models.learn import LessonResults, LessonType
...
class LearnDataService(ILearnDataService):
    ...
    def _setup_data_directories(self) -> None:
        ...
        try:
            ...
            for directory in [self.data_dir, self.progress_dir, self.results_dir, self.sessions_dir]:
    ...
    def save_lesson_progress(self, session_id: str, progress_data: Dict[str, Any]) -> bool:
        """
        ...
        try:
            ...
            logger.error(f"Failed to save lesson progress for session {session_id}: {e}")
            return False
    ...
    def load_lesson_progress(self, session_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error(f"Failed to load lesson progress for session {session_id}: {e}")
            return None
    ...
    def get_lesson_history(self, lesson_type: LessonType, limit: int = 10) -> List[LessonResults]:
        ...
        try:
            ...
            if self.results_dir.exists():
                for results_file in self.results_dir.glob("*.json"):
                    ...
                    try:
                        ...
                        logger.warning(f"Failed to parse results file {results_file}: {e}")
                        continue
            ...
            logger.debug(f"Found {len(limited_results)} results for lesson type {lesson_type.value}")
    ...
    def delete_lesson_progress(self, session_id: str) -> bool:
        ...
        try:
            ...
            logger.error(f"Failed to delete lesson progress for session {session_id}: {e}")
            return False
            ...
            if self.progress_dir.exists():
                for progress_file in self.progress_dir.glob("*.json"):
                    try:
                        ...
                        logger.warning(f"Failed to cleanup progress file {progress_file}: {e}")
            ...
            for directory, stat_key in [
                ...
                if directory.exists():
                    ...
                    for file_path in files:
                        try:
                            stats["total_disk_usage_mb"] += file_path.stat().st_size / (1024 * 1024)
                        except Exception:
                            pass
