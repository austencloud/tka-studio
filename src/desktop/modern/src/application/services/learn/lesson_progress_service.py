...
from domain.models.learn import LessonResults, QuizMode
...
class LessonProgressService(ILessonProgressService):
    ...
    def get_progress_info(self, session_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            progress_info = {
                ...
                "lesson_type": session.lesson_type.value if session.lesson_type else None,
    ...
    def is_lesson_complete(self, session_id: str) -> bool:
        ...
        try:
            ...
            if session.quiz_mode == QuizMode.FIXED_QUESTION:
                ...
                logger.warning(f"Unknown quiz mode for session {session_id}: {session.quiz_mode}")
                return False
            ...
            logger.error(f"Failed to check lesson completion for session {session_id}: {e}")
            return False
    ...
    def should_advance_to_next_question(self, session_id: str) -> bool:
        ...
        try:
            ...
            logger.error(f"Failed to check if should advance for session {session_id}: {e}")
            return False
    ...
    def get_performance_summary(self, session_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"Failed to get performance summary for session {session_id}: {e}")
            return {}
