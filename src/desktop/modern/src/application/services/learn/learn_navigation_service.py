...
from core.interfaces.learn_services import ILearnNavigationService
        ...
        try:
            ...
            if self._navigation_history:
                ...
                if previous_view == self.VIEW_LESSON_SELECTOR:
                    ...
                    logger.warning("Cannot navigate back to lesson view without session ID")
                    self.navigate_to_lesson_selector()
                elif previous_view == self.VIEW_RESULTS:
                    # For results view, we need session ID - this might need special handling
                    logger.warning("Cannot navigate back to results view without session ID")
                    self.navigate_to_lesson_selector()
