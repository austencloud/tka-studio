...
class QtImageExportServiceAdapter:
    ...
    def render_sequence_image(
        ...
        try:
            ...
            image_data = self._core_service.generate_export_data(sequence_data, export_options)
            ...
            logger.info(f"🖼️ [QT_EXPORT_ADAPTER] Rendered sequence image: {image_data.width}x{image_data.height}")
    ...
    def _execute_render_commands(self, painter: QPainter, commands: List[Dict[str, Any]]):
        ...
        try:
            ...
            logger.error(f"❌ [QT_EXPORT_ADAPTER] Failed to execute render commands: {e}")
    ...
    def _execute_single_command(self, painter: QPainter, command: Dict[str, Any]):
        ...
        try:
            ...
            if command_type == "background":
                ...
                logger.warning(f"⚠️ [QT_EXPORT_ADAPTER] Unknown command type: {command_type}")
                
        except Exception as e:
            logger.error(f"❌ [QT_EXPORT_ADAPTER] Failed to execute command {command.get('type')}: {e}")
