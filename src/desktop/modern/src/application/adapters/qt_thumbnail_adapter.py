...
from application.services.core.thumbnail_service import (
    CoreThumbnailService, IThumbnailImageLoader, ThumbnailSpec, ThumbnailData,
    convert_sequence_data_to_spec
)
from application.services.core.types import ImageData, ImageFormat, Size
...
class QtThumbnailWidgetFactory:
    ...
    def create_thumbnail_widget(
        ...
        try:
            ...
            # Sequence name
            name_label = QLabel(
                thumbnail_data.spec.sequence_name or 
                f"Sequence {thumbnail_data.spec.sequence_id[:8]}"
            )
        ...
        try:
            if thumbnail_data.has_error:
                ...
                if qt_pixmap and isinstance(qt_pixmap, QPixmap):
                    ...
                    placeholder_text = thumbnail_data.placeholder_text or "📄\nThumbnail"
    ...
    def _create_error_widget(self, size: Size) -> QWidget:
        ...
        error_widget.setStyleSheet("background-color: #ffe6e6; border: 1px solid #cc0000;")
...
class QtThumbnailFactoryAdapter:
    ...
    def create_thumbnail(
        ...
        try:
            ...
            return self.widget_factory.create_thumbnail_widget(thumbnail_data, sort_method)
            
        except Exception as e:
            logger.error(f"Failed to create thumbnail: {e}")
            return self.widget_factory._create_error_widget(Size(thumbnail_width, thumbnail_width))
    ...
    def batch_create_thumbnails(
        ...
        try:
            ...
            from application.services.core.thumbnail_service import batch_convert_sequences_to_specs
            ...
            for thumbnail_data in thumbnail_data_list:
                widget = self.widget_factory.create_thumbnail_widget(thumbnail_data, sort_method)
            ...
            return [
                self.widget_factory._create_error_widget(error_size)
                for _ in sequences
            ]
