...
class TestManualInspectionExport:
    ...
    def test_export_real_tka_sequence_for_manual_inspection(self):
        ...
        print(f"📊 Export options: Full (word, user info, difficulty, beat numbers, start position)")
        
        # Create the image using the modern export system
        print("\n🔄 Creating image with modern export system...")
        image = self.export_service.create_sequence_image(real_sequence_data, word, options)
        ...
        # Quality assertions
        assert analysis['unique_colors'] > 20, f"Expected >20 colors for real pictographs, got {analysis['unique_colors']}"
        assert analysis['has_varied_content'], "Image should have varied visual content"
        assert analysis['content_percentage'] > 5.0, f"Expected >5% content coverage, got {analysis['content_percentage']:.1f}%"
        ...
        minimal_image = self.export_service.create_sequence_image(real_sequence_data, word, minimal_options)
    ...
    def _get_real_sequence_data(self):
        ...
        try:
            ...
            pictographs = real_data_service.get_pictographs_by_letter('A')
                ...
                for i, pictograph in enumerate(pictographs[:6]):  # Take up to 6 beats
                    ...
                    if beat_data and beat_data.has_pictograph:
                        ...
                        sequence_entry = {
                            ...
                            "start_pos": getattr(pictograph_data, 'start_position', 'alpha'),
                            "end_pos": getattr(pictograph_data, 'end_position', 'beta'),
                            "blue_attributes": self._extract_motion_attributes(pictograph_data, 'blue'),
                            "red_attributes": self._extract_motion_attributes(pictograph_data, 'red')
                        }
                        sequence_data.append(sequence_entry)
                
                if sequence_data:
                    print(f"✅ Loaded {len(sequence_data)} real pictographs from TKA dataset")
    ...
    def _analyze_image_quality(self, image: QImage) -> dict:
        ...
        step = max(1, min(image.width(), image.height()) // 100)  # Sample every N pixels
        ...
        analysis['has_varied_content'] = content_pixels > (total_pixels * 0.05)  # >5% non-white
        analysis['content_percentage'] = (content_pixels / total_pixels) * 100 if total_pixels > 0 else 0
        
        # Complexity score based on color variety and content coverage
        analysis['complexity_score'] = min(100, analysis['unique_colors'] + analysis['content_percentage'])
