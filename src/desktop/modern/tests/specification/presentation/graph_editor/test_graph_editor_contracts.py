...
class TestGraphEditorPublicAPIContracts:
    ...
    def test_set_beat_data_contract(self, qapp, all_mock_services, sample_beat_data):
        """PERMANENT: set_beat_data must accept BeatData and index."""
        from presentation.components.graph_editor import GraphEditor
        ...
        # Test set_beat_data method exists and accepts correct parameters
        assert hasattr(graph_editor, 'set_beat_data')
        
        # Should not raise exception
        graph_editor.set_beat_data(sample_beat_data, 0)
        
        # Clean up
        graph_editor.deleteLater()
    
    def test_set_sequence_data_contract(self, qapp, all_mock_services, sample_sequence_data):
        """PERMANENT: set_sequence_data must accept SequenceData."""
        from presentation.components.graph_editor import GraphEditor
        ...
        # Test set_sequence_data method exists and accepts correct parameters
        assert hasattr(graph_editor, 'set_sequence_data')
        
        # Should not raise exception
        graph_editor.set_sequence_data(sample_sequence_data)
...
class TestGraphEditorStateManagementContracts:
    ...
    def test_state_consistency_contract(self, qapp, all_mock_services, sample_beat_data):
    ...
    def test_none_handling_contract(self, qapp, all_mock_services):
        ...
        graph_editor.set_beat_data(None, -1)
        
        # Should handle None sequence data without exception
        graph_editor.set_sequence_data(None)
...
class TestGraphEditorArchitecturalContracts:
    ...
    def test_immutable_domain_model_contract(self, qapp, all_mock_services, sample_beat_data):
