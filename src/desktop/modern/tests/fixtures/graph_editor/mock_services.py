...
class MockGraphEditorService:
    ...
    def set_selected_beat(self, beat_data: BeatData, beat_index: int) -> None:
        ...
        self._record_call("set_selected_beat", beat_data=beat_data, beat_index=beat_index)
    ...
    def apply_turn_adjustment(self, beat_data: BeatData, arrow_color: str, new_turns: float) -> bool:
        """Apply turn adjustment to beat data."""
        self._record_call("apply_turn_adjustment", 
                         beat_data=beat_data, arrow_color=arrow_color, new_turns=new_turns)
        return True
        
    def apply_orientation_adjustment(self, beat_data: BeatData, arrow_color: str, orientation: str) -> bool:
        """Apply orientation adjustment to beat data."""
        self._record_call("apply_orientation_adjustment",
                         beat_data=beat_data, arrow_color=arrow_color, orientation=orientation)
        return True
    ...
    def _record_call(self, method_name: str, **kwargs) -> None:
        ...
        self.method_calls.append({
            "method": method_name,
            "args": kwargs,
            "call_count": len([c for c in self.method_calls if c["method"] == method_name]) + 1
        })
...
class MockDataFlowService:
    ...
    def process_orientation_change(self, beat_data: BeatData, arrow_color: str, orientation: str) -> BeatData:
    ...
    def determine_panel_mode(self, beat_data: Optional[BeatData]) -> str:
        ...
        if beat_data is None:
            return "orientation"
        if hasattr(beat_data, 'metadata') and beat_data.metadata.get('is_start_position'):
            return "orientation"
        return "turns"
