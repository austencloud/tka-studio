...
from application.services.settings.visibility_state_manager import ModernVisibilityStateManager
from application.services.pictograph.global_visibility_service import GlobalVisibilityService
from presentation.components.ui.settings.visibility.visibility_pictograph_preview import VisibilityPictographPreview
...
class TestModernVisibilityIntegration:
    ...
    def test_dependency_logic(self):
        ...
        assert tka_effective is False, "TKA should be hidden when not all motions visible"
    ...
    def test_global_visibility_service(self):
        ...
        # Test global update
        result = self.global_service.apply_visibility_change(
            "glyph", "TKA", False
        )
    ...
    def test_performance_with_multiple_pictographs(self):
        ...
        logger.info(f"✓ Performance test passed ({update_time:.3f}s for 50 pictographs)")
    ...
    def run_comprehensive_test(self) -> Dict[str, Any]:
        ...
        if overall_success:
            logger.info(f"✅ All integration tests passed! ({success_rate:.1%} success rate)")
        else:
