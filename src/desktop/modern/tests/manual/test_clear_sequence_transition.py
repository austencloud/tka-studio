...
def test_clear_sequence_logic():
    ...
    try:
        ...
        print(f"Empty sequence: length={empty_sequence.length}, beats={len(empty_sequence.beats)}")
        ...
        has_beats = (
            ...
            and not (empty_sequence.length == 1 and empty_sequence.beats[0].is_blank if empty_sequence.beats else False)
        ...
        has_beats_with_content = (
            ...
            and not (sequence_with_beats.length == 1 and sequence_with_beats.beats[0].is_blank if sequence_with_beats.beats else False)
            and sequence_with_beats.metadata.get("cleared") is not True
        )
        print(f"Sequence with beats: length={sequence_with_beats.length}, has_beats={has_beats_with_content}")
...
def test_persistence_consistency():
    ...
    try:
        ...
        if default_seq == cleared_seq:
            ...
            print(f"Differences:")
            for i, (default_item, cleared_item) in enumerate(zip(default_seq, cleared_seq)):
