...
class TestSequenceCardTabIntegration:
    ...
    def temp_dictionary(self):
        ...
        test_data = [
            ...
            ("banana", [
                ("banana_16_beat.png", 16),
                ("banana_12_sequence.png", 12)
            ]),
            ("cherry", [
                ("cherry_length_10.png", 10),
                ("cherry_2.png", 2)
            ]),
        ]
    ...
    def container_with_services(self, temp_dictionary):
        ...
        data_service.get_all_sequences = lambda base_path=None: original_method(temp_dictionary)
    ...
    def test_export_integration(self, qtbot, integrated_tab):
        ...
        with patch.object(export_service, 'export_all_sequences', return_value=True) as mock_export:
...
class TestSequenceCardRealWorldScenarios:
    ...
    @pytest.fixture
    def realistic_container(self):
        """Create container with realistic configuration."""
        container = DIContainer()
        register_sequence_card_services(container)
        
        # Add any other services that might be needed in real app
        # (This would include settings services, file system services, etc.)
    ...
    def test_with_real_dictionary_path(self, mock_get_path, realistic_container, tmp_path):
        ...
        for word in ["test1", "test2"]:
            ...
            word_dir.mkdir()
            (word_dir / f"{word}_16.png").write_bytes(b'\x89PNG\r\n\x1a\n' + b'\x00' * 100)
