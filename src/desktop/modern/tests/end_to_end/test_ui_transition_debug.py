...
class UITransitionDebugger:
    ...
    def test_clear_sequence_transitions(self) -> bool:
        ...
        print("🧹 [DEBUG] Testing clear sequence with detailed transition monitoring...")
    ...
    def analyze_transition_log(self):
        ...
        if "index_0" in initial_state:
            ...
            print("⚠️ Initially shows option picker (index 1) - this might be expected with content")
        else:
            print(f"❓ Initial state unclear: {initial_state}")
        
        # Check immediate after clear
        after_clear_states = [entry for entry in self.transition_log if "AFTER_CLEAR" in entry["event"]]
        if after_clear_states:
            ...
            if "index_0" in immediate_after:
                ...
                print("❌ Immediately after clear: option picker (index 1) - THIS IS THE PROBLEM!")
            else:
