...
class StartupAfterClearTester:
    ...
    def simulate_fresh_startup(self) -> bool:
        ...
        try:
            ...
            layout_manager = construct_tab.layout_manager if hasattr(construct_tab, 'layout_manager') else None
    ...
    def analyze_startup_behavior(self):
        ...
        if len(unique_states) == 1:
            ...
            for i in range(1, len(self.startup_log)):
                if self.startup_log[i]["picker_state"] != self.startup_log[i-1]["picker_state"]:
        ...
        if self.startup_log:
            ...
            if "index_0" in final_state:
                print("✅ Final startup state: start position picker (index 0) - CORRECT")
            elif "index_1" in final_state:
                print("❌ Final startup state: option picker (index 1) - THIS IS THE PROBLEM!")
            else:
