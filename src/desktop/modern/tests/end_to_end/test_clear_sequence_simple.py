    ...
    def create_real_test_sequence(self) -> bool:
        ...
        try:
            ...
            if len(loaded) >= 3:
                ...
                print(f"📝 [SIMPLE_TEST] Word: '{loaded[0].get('word')}', Author: '{loaded[0].get('author')}'")
                print(f"📝 [SIMPLE_TEST] Start pos: {loaded[1].get('sequence_start_position')}")
                print(f"📝 [SIMPLE_TEST] Beat 1: {loaded[2].get('letter')}")
                return True
            else:
                print(f"❌ [SIMPLE_TEST] Failed to create sequence: {len(loaded)} items")
                return False
    ...
    def validate_clear_results(self) -> bool:
        ...
        try:
            ...
            if len(current_sequence) != 1:
                print(f"❌ [SIMPLE_TEST] Wrong sequence length: expected 1, got {len(current_sequence)}")
                return False
            ...
            if metadata.get("word") != "":
                print(f"❌ [SIMPLE_TEST] Word not cleared: expected '', got '{metadata.get('word')}'")
                return False
