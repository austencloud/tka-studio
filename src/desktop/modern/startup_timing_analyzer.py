...
class StartupTimingAnalyzer:
    ...
    def generate_performance_report(self):
        ...
        for i, timing in enumerate(sorted_operations[:10], 1):
            percentage = (timing.duration_ms / sum(t.duration_ms for t in self.timings.values())) * 100
            print(f"{i:2d}. {timing.name:<40} {timing.duration_ms:>8.1f}ms ({percentage:>5.1f}%)")
    ...
    def _analyze_phases(self):
        ...
        phases = {
            "Initialization": ["QApplication creation", "ApplicationFactory", "Container creation"],
            "Service Registration": ["Service registration", "DI container", "Event system"],
            "UI Setup": ["UI creation", "Tab widget", "Option picker", "Pictograph pool"],
    ...
    try:
        ...
        with analyzer.time_operation("TKAMainWindow creation"):
            ...
            def instrumented_init(self, *args, **kwargs):
                with analyzer.time_operation("TKAMainWindow.__init__", "TKAMainWindow creation"):
                    # Time orchestrator creation
                    if len(args) > 0 or 'container' in kwargs:
                        container_arg = args[0] if args else kwargs.get('container')
                        if container_arg:
                            with analyzer.time_operation("ApplicationOrchestrator import", "TKAMainWindow.__init__"):
                                from application.services.core.application_orchestrator import ApplicationOrchestrator

                            with analyzer.time_operation("ApplicationOrchestrator creation", "TKAMainWindow.__init__"):
                                orchestrator = ApplicationOrchestrator(container=container_arg)

                            with analyzer.time_operation("orchestrator.initialize_application", "TKAMainWindow.__init__"):
