...
def diagnose_learn_tab():
    ...
    try:
        ...
        if letter_data:
            ...
            initial_candidates = [p for p in letter_data if p.get('start_pos') == p.get('end_pos')]
            print(f"✅ Pictographs with start_pos == end_pos: {len(initial_candidates)}")
            ...
            if len(start_positions) < 4:
                print(f"❌ PROBLEM: Only {len(start_positions)} unique start positions found!")
                ...
                if question:
                    ...
                    if question.question_content:
                        ...
                        if isinstance(q_content, dict):
                            ...
                            print(f"   Is initial pictograph (start==end): {q_start == q_end}")
                    ...
                    for i, option in enumerate(question.answer_options):
                        if isinstance(option, dict):
                            ...
                            print(f"     Option {i+1}: start={opt_start}, end={opt_end} {'(CORRECT)' if is_correct else ''}")
                        else:
                    ...
                    if len(question.answer_options) >= 4:
                        ...
                        print(f"❌ PROBLEM: Only {len(question.answer_options)} answer options (need 4)!")
