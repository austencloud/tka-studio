...
import { showSuccess } from "../../../components/shared/ToastManager.svelte.js";
 ...
 */
export async function checkForSequenceInUrl(
  sequenceContainer: any
): Promise<boolean> {
  ...
  try {
    ...
    showSuccess(
      `Loaded sequence with ${moveCount} move${moveCount !== 1 ? "s" : ""}`
    );
