...
import { getImageExportSettings } from "../state/image-export-settings.js";
import { isMobileDevice } from "./fileSystemUtils.js";
...
export async function downloadSequenceImage(
  ...
  try {
    ...
    try {
      ...
      console.log(
        "DownloadHandler: Using settings from getImageExportSettings()",
        {
          ...settings,
        }
      );
      ...
      console.error(
        "DownloadHandler: Error getting export settings from function",
        error
      );
        ...
        if (savedSettings) {
          try {
            ...
            if (parsed && typeof parsed === "object") {
              settings = parsed;
              console.log(
                "DownloadHandler: Using settings from localStorage",
                settings
              );
            }
          } catch (parseError) {
    ...
    try {
      console.log(
        "DownloadHandler: Starting download with downloadImage function"
      );
      ...
      if (result.success) {
        ...
        if (isMobile) {
          ...
          showSuccess("Image saved to your device's Downloads folder", {
            duration: 5000,
          });
          ...
          showSuccess("Image saved successfully to Downloads folder", {
            duration: 5000,
          });
        }
        ...
        console.warn(
          "DownloadHandler: Download function returned false without throwing an error"
        );
