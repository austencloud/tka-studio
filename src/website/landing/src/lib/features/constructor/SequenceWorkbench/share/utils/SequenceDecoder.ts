...
import { decompressString } from "../../../utils/lzstring.js";
 ...
 */
export async function decodeSequenceCompact(
  encoded: string
): Promise<BeatData[]> {
  if (!encoded) return [];
  let decompressed;
  ...
  if (version === "3") {
    ...
    const beatData = parts.slice(2).map((beatStr, index) => {
      ...
      const startPos =
        index === 0
          ? startPosData.position
          : inferPosition(blueMotion, redMotion);
      const endPos = inferEndPosition(blueMotion, redMotion);

      // Create the beat
      return createBeatFromMotions(
        blueMotion,
        redMotion,
        startPos,
        endPos,
        index + 1
      ); // +1 to skip the start position
    });
...
export function decodeStartPosition(encoded: string): any {
  ...
  if (parts.length > 4) {
    ...
    startPosData.position = inferPositionFromLocations(
      startPosData.bluePos,
      startPosData.redPos
    );
  }
 ...
 */
export function inferPositionFromLocations(
  blueLoc: string,
  redLoc: string
): string {
...
export function decodeMotion(
  ...
  const { startOri, endOri } = inferOrientations(
    startLoc,
    endLoc,
    motionType,
    defaultOri
 ...
 */
export function inferMotionType(
  startLoc: string,
  endLoc: string,
  rotDir: string
): string {
