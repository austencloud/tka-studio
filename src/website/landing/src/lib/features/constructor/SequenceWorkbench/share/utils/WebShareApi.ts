...
export async function shareSequenceWithImage(
  ...
  console.log("shareSequenceWithImage called with:", {
    sequenceName,
    shareUrl,
  });
    ...
    if (fileShareSupported && imageResult && imageResult.dataUrl) {
      try {
        ...
        if (navigator.canShare && navigator.canShare(shareData)) {
          console.log(
            "shareUtils: Device can share with files, calling navigator.share"
          );
          ...
          console.log(
            "shareUtils: Device cannot share with files, falling back to text+URL only"
          );
