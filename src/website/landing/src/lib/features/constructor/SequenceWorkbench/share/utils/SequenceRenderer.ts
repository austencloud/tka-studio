...
export async function renderSequence(
  ...
  if (!browser || !element) {
    console.error(
      "Cannot render: not in browser environment or no beat frame element"
    );
    ...
    if (alternativeElement instanceof HTMLElement) {
      console.log(
        "SequenceRenderer: Found alternative element for rendering:",
        alternativeElement
      );
  ...
  try {
    ...
    console.log(
      "SequenceRenderer: Sequence beats count:",
      sequenceBeats.length
    );
    ...
    const result = await exportSequenceImage(element, {
      ...
      addUserInfo:
        settings.addUserInfo === undefined ? true : !!settings.addUserInfo,
      addDifficultyLevel:
        settings.addDifficultyLevel === undefined
          ? true
          : !!settings.addDifficultyLevel,
      addBeatNumbers:
        settings.addBeatNumbers === undefined
          ? true
          : !!settings.addBeatNumbers,
      addReversalSymbols:
        settings.addReversalSymbols === undefined
          ? true
          : !!settings.addReversalSymbols,
      title: sequenceName,
