// src/lib/constructor/stores/sequence/selectionStore.ts
...
function createSelectedStartPosStore() {
  ...
  const {
    ...
    set: originalSet,
    update,
  } = writable<PictographData | null>(initialValue);
  ...
  return {
    ...
    set,
    update: (
      updater: (value: PictographData | null) => PictographData | null
    ) => {
      update((current) => {
        ...
        if (browser) {
          try {
            ...
            console.error(
              "Error saving start position to localStorage:",
              error
            );
          }
        }
