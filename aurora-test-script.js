...
(function () {
  console.clear();
  console.log(
    "%c🎨 Aurora Contrast System Validation",
  ...
  if (currentTheme === "aurora") {
    test(
      ...
      currentVars.panelBg.includes("20, 10, 40") &&
        currentVars.cardBg.includes("25, 15, 45"),
      "Aurora-specific dark purple values",
      `Panel: ${currentVars.panelBg}, Card: ${currentVars.cardBg}`
    );
  } else {
    console.log(
      `   ℹ️  Skipped Aurora match test (current theme is ${currentTheme})`
    );
  }
  ...
  if (collectionCard) {
    ...
    warn(
      "No collection cards found in DOM (may need to navigate to Collections)"
    );
  }
  ...
  console.log("");
  console.log(
    `%c✅ Passed: ${results.passed}`,
    "color: #22c55e; font-weight: bold;"
  );
  console.log(
    `%c❌ Failed: ${results.failed}`,
    results.failed > 0
      ? "color: #ef4444; font-weight: bold;"
      : "color: #6b7280;"
  );
  console.log(
    `%c⚠️  Warnings: ${results.warnings}`,
    results.warnings > 0
      ? "color: #f59e0b; font-weight: bold;"
      : "color: #6b7280;"
  );
  ...
  if (results.failed === 0 && results.warnings === 0) {
    console.log(
      "%c🎉 All tests passed! Aurora contrast system is working correctly.",
      "font-size: 14px; color: #22c55e; font-weight: bold; padding: 8px; background: #dcfce7; border-radius: 4px;"
    );
  } else if (results.failed === 0) {
    console.log(
      "%c✓ All tests passed with some warnings.",
      "font-size: 14px; color: #f59e0b; font-weight: bold;"
    );
  } else {
    console.log(
      "%c❌ Some tests failed. Review the output above for details.",
      "font-size: 14px; color: #ef4444; font-weight: bold; padding: 8px; background: #fee2e2; border-radius: 4px;"
    );
  }
  ...
  console.log(
    "   • Navigate to Explore > Collections to test collection cards"
  );
  console.log("   • Navigate to Explore > Search to test search input");
  console.log(
    "   • Change background in Settings > Background to test theme switching"
  );
  console.log("");
