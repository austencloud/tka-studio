...
async function testAdminPersistence() {
  ...
  page.on('console', (msg) => {
    ...
    if (text.includes('[module-state]') || text.includes('[authStore]') || text.includes('Admin')) {
      console.log('📋', text);
    }
  });
  ...
  try {
    ...
    const visibleButtons = await page.evaluate(() => {
      const buttons = Array.from(document.querySelectorAll('button, .nav-button, .nav-item'));
      return buttons
    ...
    console.log('\n🔍 Visible buttons on page:', JSON.stringify(visibleButtons, null, 2));
    ...
    try {
      ...
      console.log('⚠️ Could not find menu button, trying alternative selectors');
    ...
    try {
      ...
      await page.click('button.module-item:has-text("Admin")', { timeout: 5000 });
      ...
      await page.evaluate(() => {
        ...
        const adminItem = moduleItems.find(item =>
    ...
    console.log('\n🎯 TEST RESULT:');
    if (onAdminAfter) {
      console.log('✅ SUCCESS: Admin module was properly restored after refresh!');
    } else {
