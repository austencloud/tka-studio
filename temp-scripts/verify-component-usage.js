...
function searchForUsage(componentName, modulePath) {
    ...
    if (svelteImports) {
      results.usages.push({
        ...
        matches: svelteImports.split('\n').filter(line =>
          !line.includes(`${modulePath}/${componentName}.svelte:`) // Exclude self-reference
        )
      });
    }
  ...
  results.isUsed = results.usages.length > 0 && results.usages.some(u => u.matches.length > 0);
  results.totalMatches = results.usages.reduce((acc, u) =>
    acc + (Array.isArray(u.matches) ? u.matches.length : 0), 0
  );
...
modulesToCheck.forEach(module => {
    ...
    if (result.isUsed) {
      ...
      result.usages.forEach(usage => {
        if (usage.matches && usage.matches.length > 0) {
          ...
          if (firstMatch) {
            const preview = firstMatch.length > 100
  ...
  console.log('');
  console.log(`Summary: ${moduleResults.summary.used} used / ${moduleResults.summary.unused} unused`);
...
allResults.forEach(module => {
  const percentage = (module.summary.used / module.summary.total * 100).toFixed(1);
  console.log(`${module.module}:`);
  console.log(`  ${module.summary.used}/${module.summary.total} components used (${percentage}%)`);
...
// Save results
fs.writeFileSync('component-usage-verification.json', JSON.stringify(allResults, null, 2));
...
if (totalUnused === 0) {
  ...
  console.log(`⚠️  Found ${totalUnused}/${totalComponents} components with no direct usages.`);
  console.log('');
