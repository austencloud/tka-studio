...
const testLines = reportContent.split('\n').filter(line => line.includes('\\tests\\'));
...
const generatedLines = reportContent.split('\n').filter(line => line.includes('\\.svelte-kit\\'));
...
console.log('-'.repeat(80));
const sortedShared = Object.entries(sharedBreakdown)
  .sort((a, b) => b[1].length - a[1].length);
...
const fileGroups = {};
[...categories.modules, ...categories.shared, ...categories.other].forEach(entry => {
...
sortedModules.forEach(([moduleName, items]) => {
  console.log(`MODULE: ${moduleName.toUpperCase()} (${items.length} unused exports)`);
  console.log('-'.repeat(80));
...
const jsonReport = {
  ...
  moduleBreakdown: Object.fromEntries(
    Object.entries(moduleBreakdown).map(([name, items]) => [
      ...
      {
        count: items.length,
        files: Object.keys(items.reduce((acc, item) => {
          acc[item.file] = true;
          return acc;
        }, {}))
      }
...
fs.writeFileSync('dead-code-analysis.json', JSON.stringify(jsonReport, null, 2));
...
if (highUnusedModules.length > 0) {
  ...
  highUnusedModules.forEach(([moduleName, items]) => {
    const trulyUnused = items.filter(item => !item.isUsedInModule);
    console.log(`   - src/lib/modules/${moduleName}: ${trulyUnused.length} unused exports`);
  });
  console.log('');
}
...
console.log('   c) Search for dynamic imports or runtime usage not detected by ts-prune');
