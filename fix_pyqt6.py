...
def diagnose_pyqt6():
    ...
    try:
        result = subprocess.run([sys.executable, "-c", "import PyQt6; print(f'PyQt6 version: {PyQt6.QtCore.qVersion()}'); print(f'Location: {PyQt6.__file__}')"], 
...
def fix_pyqt6():
    ...
    qt_packages = ["PyQt6", "PyQt6-Qt6", "PyQt6-sip", "PySide6", "PyQt5", "PyQt5-Qt5", "PyQt5-sip"]
    ...
    # Step 3: Update pip
    print("\nStep 3: Updating pip...")
    run_command([sys.executable, "-m", "pip", "install", "--upgrade", "pip"], "Updating pip")
    
    # Step 4: Install fresh PyQt6
    print("\nStep 4: Installing fresh PyQt6...")
    success = run_command([sys.executable, "-m", "pip", "install", "PyQt6", "--no-cache-dir"], 
                         "Installing PyQt6")
    ...
    if not success:
        ...
        success = run_command([sys.executable, "-m", "pip", "install", "PyQt6", "--force-reinstall", "--no-cache-dir"], 
                             "Force reinstalling PyQt6")
        
        if not success:
            # Try specific version
            success = run_command([sys.executable, "-m", "pip", "install", "PyQt6==6.6.1", "--no-cache-dir"], 
