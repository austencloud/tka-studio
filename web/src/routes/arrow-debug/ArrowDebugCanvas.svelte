<script lang="ts">
  ...
  function drawHandPoints() {
    ...
    const handPoints =
      state.currentDebugData.coordinateSystemDebugInfo.handPoints;
  ...
  function drawLayer2Points() {
    ...
    const layer2Points =
      state.currentDebugData.coordinateSystemDebugInfo.layer2Points;
  ...
  function drawAdjustmentVector(
    ...
    if (Math.abs(adjX) > 1 || Math.abs(adjY) > 1) {
      ...
      ctx.fillText(
        `(${adjustment.x.toFixed(1)}, ${adjustment.y.toFixed(1)})`,
        labelX,
        labelY + 10
      );
    }
  ...
  function handleCanvasClick(event: MouseEvent) {
    ...
    console.log(
      `Clicked at scene coordinates: (${sceneX.toFixed(1)}, ${sceneY.toFixed(1)})`
    );
  }
</script>
    ...
    <div class="legend-grid">
      ...
      {#if state.showAdjustmentVectors}
        <div class="legend-item">
          <div
            class="legend-color"
            style="background: {COLORS.vector.adjustment}"
          ></div>
    ...
    <div class="coordinates-display">
      ...
      <div class="coord-item">
        ...
        <span class="coord-value">
          ({state.currentDebugData.finalPosition.x.toFixed(2)}, {state.currentDebugData.finalPosition.y.toFixed(
            2
          )})
        </span>
      </div>
      <div class="coord-item">
        ...
        <span class="coord-value">
          ({(state.currentDebugData.finalPosition.x * SCALE).toFixed(2)}, {(
            state.currentDebugData.finalPosition.y * SCALE
          ).toFixed(2)})
        </span>
      </div>
      <div class="coord-item">
        ...
        <span class="coord-value"
          >{state.currentDebugData.finalRotation.toFixed(2)}°</span
        >
      </div>
    </div>
  {/if}
