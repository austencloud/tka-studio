...
<script lang="ts">
	...
	let { selectedOrientation, onOrientationChange, onClose, color, triggerElement }: Props = $props();
	...
	function renderModalContent() {
		...
		content.querySelectorAll('.orientation-option').forEach(button => {
			button.addEventListener('click', () => {
				const orientation = button.getAttribute('data-orientation') as Orientation;
	...
	const orientations: { id: Orientation; label: string; icon: string; description: string }[] = [
		{ id: 'in' as Orientation, label: 'In', icon: '↑', description: 'Inward orientation' },
		{ id: 'out' as Orientation, label: 'Out', icon: '↓', description: 'Outward orientation' },
		{ id: 'clock' as Orientation, label: 'Clock', icon: '↻', description: 'Clockwise orientation' },
		{ id: 'counter' as Orientation, label: 'Counter', icon: '↺', description: 'Counter-clockwise orientation' }
	];
