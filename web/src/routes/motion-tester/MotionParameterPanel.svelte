...
<div class="motion-params-panel">
  ...
  <div class="panels-container">
    ...
    <div class="prop-section">
      ...
      <PropPanel
        ...
        onStartOrientationChange={(orientation) =>
          state.updateBlueMotionParam("startOri", orientation)}
        onEndOrientationChange={(orientation) =>
          state.updateBlueMotionParam("endOri", orientation)}
        onTurnsChange={(turns) => state.updateBlueMotionParam("turns", turns)}
        onMotionTypeChange={(motionType) =>
          state.updateBlueMotionParam("motionType", motionType)}
      />
    </div>
    ...
    <div class="prop-section">
      ...
      <PropPanel
        ...
        onStartOrientationChange={(orientation) =>
          state.updateRedMotionParam("startOri", orientation)}
        onEndOrientationChange={(orientation) =>
          state.updateRedMotionParam("endOri", orientation)}
        onTurnsChange={(turns) => state.updateRedMotionParam("turns", turns)}
        onMotionTypeChange={(motionType) =>
          state.updateRedMotionParam("motionType", motionType)}
      />
    </div>
  </div>
  ...
  <div class="quick-tests-section">
    ...
    <div class="quick-tests">
      <button class="quick-test-btn" onclick={() => setQuickTest("pro")}
        >N→E PRO</button
      >
      <button class="quick-test-btn" onclick={() => setQuickTest("dash")}
        >E→W DASH</button
      >
      <button class="quick-test-btn" onclick={() => setQuickTest("anti")}
        >S→W ANTI</button
      >
      <button class="quick-test-btn" onclick={() => setQuickTest("float")}
        >N→S FLOAT</button
      >
    </div>
  </div>
