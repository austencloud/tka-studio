...
import type { IPictographValidatorService } from "$services";
...
export class PictographValidatorService implements IPictographValidatorService {
  ...
  private validateMotionData(
    ...
    } else if (!Object.values(Location).includes(motionData.startLocation)) {
      errors.push(
    ...
    } else if (!Object.values(Location).includes(motionData.endLocation)) {
      errors.push(
