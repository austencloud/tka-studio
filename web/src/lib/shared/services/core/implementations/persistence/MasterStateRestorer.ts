...
import type { IBrowseStatePersister } from "$services";
import type { IMainTabState } from "../../../state";
...
export class MasterStateRestorer {
  private isRestoring = false;
  private restorationSteps: Map<string, RestorationStep> = new Map();
  private scrollRestorationQueue: ScrollRestorationData[] = [];

  constructor(
    private tabStateService: IMainTabState,
    private browseStatePersistence: IBrowseStatePersister
  ...
  private async restoreActiveTabState(): Promise<Record<string, unknown>> {
    ...
    switch (activeTab) {
      ...
      case "sequence_card":
        return this.restoreWordCardTabState();
  ...
  private async restoreWordCardTabState(): Promise<Record<string, unknown>> {
    // Implement word card tab restoration
    return { message: "Word card tab restoration not yet implemented" };
  }
...
let masterRestorationService: MasterStateRestorer | null = null;
...
export function getMasterRestorationService(dependencies?: {
  tabStateService: IMainTabState;
  browseStatePersistence: IBrowseStatePersister;
}): MasterStateRestorer {
  if (!masterRestorationService && dependencies) {
    masterRestorationService = new MasterStateRestorer(
