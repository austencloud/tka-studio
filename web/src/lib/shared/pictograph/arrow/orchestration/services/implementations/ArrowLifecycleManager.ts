...
import { type ArrowAssets, type ArrowLifecycleResult, type ArrowPosition, type ArrowState, createArrowAssets, createArrowLifecycleResult, createArrowPosition, createArrowState } from "../../domain";
...
export class ArrowLifecycleManager implements IArrowLifecycleManager {
  constructor(
    ...
    @inject(TYPES.IArrowPositioningOrchestrator) private positioningOrchestrator: IArrowPositioningOrchestrator
  ) {}
  ...
  async calculateArrowPosition(
    ...
    try {
      const [x, y, rotation] = await this.positioningOrchestrator.calculateArrowPoint(
        pictographData,
        motionData
      );

      return createArrowPosition({ x, y, rotation });
    } catch (error) {
  ...
  async coordinateArrowLifecycle(
    ...
    const motionPromises = Object.entries(pictographData.motions).map(
      async ([color, motionData]) => {
        try {
          const arrowState = await this.getArrowState(motionData, pictographData);
    ...
    const allReady = Object.keys(errors).length === 0 && 
