...
export function createPictographState(
  ...
  async function initializeServices() {
    try {
      dataTransformationService = await resolveAsync<IDataTransformationService>(
        TYPES.IDataTransformationService
      );
      componentManagementService = await resolveAsync<IComponentManagementService>(
  ...
  async function calculateArrowPositions(): Promise<void> {
    ...
    if (!currentData?.motions || !servicesInitialized || !arrowLifecycleManager) {
      arrowPositions = {};
      arrowMirroring = {};
      arrowAssets = {};
      showArrows = true;
