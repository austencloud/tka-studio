...
import { createArrowLifecycleResult, TYPES } from '$shared';
import { inject, injectable } from 'inversify';
import type { IArrowLifecycleManager } from '../../../arrow/orchestration/services/contracts';
import type { IPictographCoordinator, PictographRenderingState } from '../contracts/IPictographCoordinator';

@injectable()
export class PictographCoordinator implements IPictographCoordinator {
  constructor(
    @inject(TYPES.IArrowLifecycleManager) private arrowLifecycleManager: IArrowLifecycleManager
  ) {}
  ...
  async coordinatePictographLifecycle(
    ...
    try {
      ...
      const arrowLifecycleResult = await this.arrowLifecycleManager.coordinateArrowLifecycle(
        pictographData
      );
      ...
      const errorMessage = error instanceof Error ? error.message : 'Unknown coordination error';
