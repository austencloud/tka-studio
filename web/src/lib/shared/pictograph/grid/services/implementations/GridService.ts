...
import type { IGridService } from "../contracts/IGridService";
...
export class GridService implements IGridService {
  ...
  private getGridImage(gridMode: GridMode): HTMLImageElement {
    ...
    const svgString = this.createGridSVG(gridMode, 950); // Using the viewBox size from embedded SVG
    const img = new Image();
