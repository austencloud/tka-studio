...
import type { ResponsiveSettings } from "$services/interfaces/device-interfaces";
  ...
  const isDesktop = $derived(() => {
    ...
    function initializeDevice(deviceService: {
      ...
      try {
        ...
        console.log("🔍 Device capabilities:", $state.snapshot(deviceCapabilities));
        console.log("📱 Responsive settings:", $state.snapshot(responsiveSettings));
