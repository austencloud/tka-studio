...
export function createImageExportState(
  ...
  let validationErrors = $derived(() => {
    ...
    const validation = exportService.validateExport(currentSequence, exportOptions);
   ...
   */
  async function updateOptions(newOptions: Partial<TKAImageExportOptions>): Promise<void> {
    try {
      clearErrors();
  ...
  async function generatePreview(sequence?: SequenceData): Promise<void> {
    ...
    try {
      const dataUrl = await exportService.generatePreview(sequence, exportOptions);
   ...
   */
  async function exportSequence(sequence: SequenceData, filename?: string): Promise<void> {
    if (!sequence) {
    ...
    try {
      ...
      const displayFilename = filename || `${sequence.word || 'sequence'}.${exportOptions.format.toLowerCase()}`;
  ...
  return {
    ...
    get exportOptions() { return exportOptions; },
    get previewImageUrl() { return previewImageUrl; },
    ...
    get lastExportedFile() { return lastExportedFile; },
...
export function createPersistentImageExportState(
  ...
  const persistentUpdateOptions = async (newOptions: Partial<TKAImageExportOptions>) => {
