...
@injectable()
export class DeleteService implements IWorkbenchDeleteService {
  async prepareDeleteConfirmation(
    ...
    return {
      ...
      hasVariations,
      variationCount: relatedSequences.length,
      affectedSequences: relatedSequences.map(seq => seq.id),
      warnings: willFixVariationNumbers ? ["Variation numbers will be updated"] : [],
    };
  }
    ...
    try {
      ...
      if (!canDelete) {
        return {
          ...
          deletedSequenceId: sequenceId,
          affectedSequences: [],
          errors: ["Sequence cannot be deleted"],
        };
      }
      ...
      return {
        ...
        affectedSequences: affectedSequences.map(seq => seq.id),
      };
    } catch (error) {
      return {
        success: false,
  ...
  async formatDeleteConfirmationMessage(
    ...
    let message = `Are you sure you want to delete "${sequenceName}"?`;

    if (hasVariations) {
      message += `\n\nThis sequence has ${variationCount} related variation(s).`;
