...
// Remove this interface - use the one from domain instead
// Domain model is imported from $domain
  ...
  implements IMotionLetterIdentificationService
{
  ...
  identifyLetter(
    ...
    for (const [letter, mapping] of Object.entries(this.letterMappings)) {
      ...
      if (matchResult.confidence > 0.5 && bestMatch.confidence < 1.0) {
        if (
    ...
    if (bestMatch.letter) {
      console.log(
        `🔍 [LETTER ID] Best partial match: ${bestMatch.letter} (confidence: ${bestMatch.confidence})`
      );
    } else {
  ...
  private compareMotionSignature(
    ...
    for (const param of coreParams) {
      ...
      if (mappingValue && signatureValue) {
        const mappingStr = String(mappingValue).toLowerCase();
        const signatureStr = String(signatureValue).toLowerCase();
        if (mappingStr === signatureStr) {
    ...
    // Determine confidence level (0-1 scale for domain model)
    let confidenceScore = 0;

    if (missingParameters.length === 0 && matchedParameters.length >= 4) {
      confidenceScore = 1.0; // exact match
    } else if (matchedParameters.length >= 2) {
      confidenceScore = matchedParameters.length / 4; // partial match based on parameter count
    }

    return {
      letter: confidenceScore > 0 ? letter : "",
      confidence: confidenceScore,
      isValid: confidenceScore > 0,
    };
  }
