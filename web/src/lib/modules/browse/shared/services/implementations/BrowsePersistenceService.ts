...
import type { BeatData, PictographData, SequenceData } from "$shared/domain";
import { SequenceDataSchema } from "$shared/domain";
import { safeParseOrNull } from "$shared/utils";
import { injectable } from "inversify";
import type { IPersistenceService } from "../../../../build/workbench";
...
@injectable()
export class BrowsePersistenceService implements IPersistenceService {
  ...
  private normalizeBeats(beats: unknown[]): BeatData[] {
    return beats.map((beat: unknown, index: number) => {
      ...
      return {
        ...
        beatNumber: (beatData.beatNumber as number) ?? index + 1, // ✅ CRITICAL: Ensure beatNumber is always present
        ...
        pictographData: (beatData.pictographData as PictographData) || null,
      };
    });
  }
