  ...
  {#if questionData}
    ...
    <div class="question-section">
      <div class="question-prompt">
        <h3>
          {(questionData as any).questionPrompt || "Choose the correct answer:"}
        </h3>
      </div>
      ...
      <div class="question-content">
        {#if (questionData as any).questionType === "pictograph"}
          <PictographRenderer
            pictographData={(questionData as any)
              .questionContent as PictographData}
          />
    ...
    <div class="answer-section">
      <div
        class="answer-grid"
        class:button-grid={questionData.answerType === QuizAnswerFormat.BUTTON}
      >
        {#each questionData.answerOptions as option (option.id)}
          <div class="answer-option {getAnswerClass(option)}">
            {#if (questionData as any).answerType === QuizAnswerFormat.BUTTON}
              <AnswerButton
              ...
              />
            {:else if (questionData as any).answerType === QuizAnswerFormat.PICTOGRAPH}
              <AnswerPictograph
