...
<script lang="ts">
  ...
  // Services
  const persistenceService = resolve<IPersistenceService>(TYPES.IPersistenceService);
  ...
  let question = $derived(questionData?.question || "What letter does this pictograph represent?");
  ...
  async function loadPictographData() {
    ...
    try {
      ...
      availableLetters = [...new Set(pictographs.map(p => p.letter).filter((letter): letter is Letter => letter != null))].sort();
      
      console.log(`✅ Loaded ${pictographs.length} pictographs with ${availableLetters.length} unique letters`);
    } catch (error) {
  ...
  async function generateRealQuestion() {
    ...
    try {
      ...
      const letterPictographs = await persistenceService.getPictographsByLetter(randomLetter);
      ...
      currentPictograph = letterPictographs[Math.floor(Math.random() * letterPictographs.length)];
  ...
  <div class="pictograph-quiz-container">
    ...
    <div class="quiz-header">
      ...
      <div class="quiz-stats">
        <span class="letter-count">{availableLetters.length} letters available</span>
        <span class="pictograph-count">{pictographs.length} pictographs loaded</span>
      </div>
    </div>
      ...
      <div class="quiz-feedback">
        {#if selectedAnswerId && answerOptions.find(o => o.id === selectedAnswerId)?.isCorrect}
          <div class="feedback correct-feedback">
            ...
            <span class="feedback-text">Correct! This pictograph represents the letter "{correctAnswer}"</span>
          </div>
        {:else}
          <div class="feedback incorrect-feedback">
            <span class="feedback-icon">❌</span>
            <span class="feedback-text">Incorrect. This pictograph represents the letter "{correctAnswer}"</span>
          </div>
        {/if}
