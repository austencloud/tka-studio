/**
 * Word Card Metadata Overlay Service
 *
 * Handles adding metadata overlays to word card SVGs.
...
export class WordCardMetadataOverlayService {
   ...
   */
  generateTitleOverlay(title: string, dimensions: WordCardDimensions): string {
  ...
  private generateTimestampOverlay(dimensions: WordCardDimensions): string {
    const now = new Date();
