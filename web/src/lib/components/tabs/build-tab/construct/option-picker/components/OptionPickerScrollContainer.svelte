...
<script lang="ts">
  ...
  import { createOptionPickerScrollState } from "$lib/state/construct/option-picker/scroll-state.svelte";
...
<div
  class="option-picker-scroll glass-scrollbar-secondary {Array.isArray(
    scrollState.cssClasses
  )
    ? scrollState.cssClasses.join(' ')
    : scrollState.cssClasses || ''}"
