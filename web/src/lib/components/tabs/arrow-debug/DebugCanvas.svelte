<script lang="ts">
  ...
  function renderLayer2Points(): string {
    if (
      !showLayer2Points ||
      !debugData.coordinateSystemDebugInfo?.layer2Points
    ) {
      return "";
    }
  ...
  function renderCoordinateGrid(): string {
    ...
    for (let x = step; x < CANVAS_SIZE; x += step) {
      gridLines.push(
        `<line x1="${x}" y1="0" x2="${x}" y2="${CANVAS_SIZE}" stroke="${COLORS.grid}" stroke-width="0.5" opacity="0.3"/>`
      );
    }
    ...
    for (let y = step; y < CANVAS_SIZE; y += step) {
      gridLines.push(
        `<line x1="0" y1="${y}" x2="${CANVAS_SIZE}" y2="${y}" stroke="${COLORS.grid}" stroke-width="0.5" opacity="0.3"/>`
      );
    }

    // Center lines
    gridLines.push(
      `<line x1="${CENTER}" y1="0" x2="${CENTER}" y2="${CANVAS_SIZE}" stroke="${COLORS.grid}" stroke-width="1" opacity="0.6"/>`
    );
    gridLines.push(
      `<line x1="0" y1="${CENTER}" x2="${CANVAS_SIZE}" y2="${CENTER}" stroke="${COLORS.grid}" stroke-width="1" opacity="0.6"/>`
    );
  ...
  <div class="canvas-info">
    {#if debugData.finalPosition}
      <div class="position-info">
        ...
        ({debugData.finalPosition.x.toFixed(2)}, {debugData.finalPosition.y.toFixed(
          2
        )})
      </div>
    {/if}
