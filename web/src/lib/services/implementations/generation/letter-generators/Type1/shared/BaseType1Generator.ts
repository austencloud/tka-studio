...
export abstract class BaseType1Generator extends BaseLetterGenerator {
   ...
   */
  protected abstract getLetterConfiguration(): Record<
    string,
    Type1LetterConfig
  >;
  ...
  protected createPatterns(): PictographData[] {
    const config = this.getLetterConfiguration()[this.letter];
    if (!config) {
      throw new Error(
  ...
  getSupportedLetters(): string[] {
    return getType1SupportedLetters(this.getLetterConfiguration());
  }
  ...
  supportsLetter(letter: string): boolean {
    return isType1LetterSupported(letter, this.getLetterConfiguration());
  }
