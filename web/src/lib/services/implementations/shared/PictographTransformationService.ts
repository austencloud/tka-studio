...
export interface IPictographTransformationService {
  ...
  validateCSVRow(row: Record<string, string>): { isValid: boolean; errors: string[] };
}

export class PictographTransformationService implements IPictographTransformationService {
  ...
  convertCsvRowToPictographData(
    ...
    try {
      return this.createPictographFromCSVRow(row, gridMode);
    } catch (error) {
      console.warn(`⚠️ Failed to convert CSV row ${index || 'unknown'} to pictograph:`, error);
      return null;
    }
  }
  ...
  createPictographFromCSVRow(
    ...
    try {
      ...
      const blueMotion = createMotionData({
        ...
        rotationDirection: this.enumMappingService.mapRotationDirection(row.blueRotationDirection),
        startLocation: this.enumMappingService.mapLocation(row.blueStartLocation || row.blueStartLoc),
        endLocation: this.enumMappingService.mapLocation(row.blueEndLocation || row.blueendLocation),
        turns: 0,
        isVisible: true,
      });
      ...
      const redMotion = createMotionData({
        ...
        rotationDirection: this.enumMappingService.mapRotationDirection(row.redRotationDirection),
        startLocation: this.enumMappingService.mapLocation(row.redStartLocation || row.redStartLoc),
        endLocation: this.enumMappingService.mapLocation(row.redEndLocation || row.redendLocation),
        turns: 0,
        isVisible: true,
      });
      ...
      return createPictographData({
        ...
        startPosition: this.enumMappingService.convertToGridPosition(row.startPosition),
        endPosition: this.enumMappingService.convertToGridPosition(row.endPosition),
        gridMode: gridMode,
        isBlank: false,
        metadata: {
  ...
  convertMultipleRows(
    ...
    failed: Array<{ index: number; row: Record<string, string>; error: string }>;
  } {
    const successful: PictographData[] = [];
    const failed: Array<{ index: number; row: Record<string, string>; error: string }> = [];

    rows.forEach((row, index) => {
      try {
        const pictograph = this.convertCsvRowToPictographData(row, gridMode, index);
        if (pictograph) {
   ...
   */
  getTransformationStats(rows: Record<string, string>[], gridMode: string): {
    totalRows: number;
    validRows: number;
    invalidRows: number;
