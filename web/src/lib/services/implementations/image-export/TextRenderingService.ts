...
export class TextRenderingService implements ITextRenderingService {
  ...
  renderWordText(
    ...
    let fontSize = Math.floor(TextRenderingService.WORD_BASE_FONT_SIZE * options.beatScale);
    const kerning = Math.floor(TextRenderingService.WORD_KERNING_BASE * options.beatScale);
  ...
  renderUserInfo(
    ...
    const { fontSize } = this.adjustFontAndMargin(baseFontSize, margin, 3, options.beatScale);
  ...
  private adjustFontAndMargin(
    ...
    const fontSize = Math.max(1, Math.floor(baseFontSize * fontScale * beatScale));
    const margin = Math.max(1, Math.floor(baseMargin * marginScale * beatScale));
