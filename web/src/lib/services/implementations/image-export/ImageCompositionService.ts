...
export class ImageCompositionService implements IImageCompositionService {
  ...
  async composeSequenceImage(
    ...
    try {
      ...
      await this.composeFromCanvases(
        ...
        startPositionCanvas,
        layoutData,
        { ...options, layout: [layoutData.columns, layoutData.rows] }
      );
  ...
  getCompositionStats(
    ...
    const beatCount = sequence.beats.length;
    const layout = this.layoutService.calculateLayout(beatCount, options.includeStartPosition);
    const layoutData = this.calculateLayoutData(sequence, options);
    ...
    if (options.addDifficultyLevel && sequence.level) textElements.push('difficulty');
