...
export class BeatRenderingService implements IBeatRenderingService {
  ...
  async renderBeatToCanvas(
    ...
    try {
      ...
      throw new Error(`Failed to render beat to canvas: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  async renderStartPositionToCanvas(
    ...
    try {
      ...
      throw new Error(`Failed to render start position: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  async renderBeatsToCanvases(
    ...
    try {
      ...
      throw new Error(`Batch rendering failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  private async renderPictographToCanvas(
    ...
    try {
      ...
      const svgElement = await this.createSVGFromPictographData(beatData, size, options);
      if (svgElement) {
  ...
  private async createSVGFromPictographData(
    ...
    try {
      ...
      try {
        ...
        const { default: Pictograph } = await import('$lib/components/pictograph/Pictograph.svelte');
   ...
   */
  private drawGrid(ctx: CanvasRenderingContext2D, gridMode: string, size: number): void {
   ...
   */
  private drawProp(ctx: CanvasRenderingContext2D, propData: any, color: string, size: number): void {
    const centerX = size / 2;
    const centerY = size / 2;
   ...
   */
  private drawArrow(ctx: CanvasRenderingContext2D, arrowData: any, color: string, size: number): void {
    const centerX = size / 2;
    const centerY = size / 2;
   ...
   * Draw beat number
   */
  private drawBeatNumber(ctx: CanvasRenderingContext2D, beatNumber: number, size: number): void {
   ...
   */
  private renderStartPositionLabel(ctx: CanvasRenderingContext2D, size: number): void {
   ...
   */
  private applyCombinedGrids(ctx: CanvasRenderingContext2D, size: number): void {
  ...
  private drawReversalSymbols(
    ...
    if (beatData.red_reversal) {
      ...
      ctx.fillRect(size - symbolSize - 5, symbolSize + 10, symbolSize, symbolSize);
    }
  }
