...
describe("SequenceStateService", () => {
  ...
  describe("Sequence Transformations", () => {
    ...
    beforeEach(() => {
      testSequence = service.createNewSequence("Test", 4);
      // Add some data to make clearing meaningful - create new beat objects
      testSequence = {
        ...testSequence,
        beats: [
          createBeatData({
          ...
          createBeatData({ ...testSequence.beats[1], redReversal: true }),
          testSequence.beats[2],
          testSequence.beats[3],
        ],
      };
    });
  ...
  describe("Utilities", () => {
    ...
    beforeEach(() => {
      testSequence = service.createNewSequence("Test", 4);
      // Create beats with different durations
      testSequence = {
        ...testSequence,
        beats: [
          createBeatData({ ...testSequence.beats[0], duration: 2.0 }),
          createBeatData({ ...testSequence.beats[1], duration: 1.5 }),
          createBeatData({ ...testSequence.beats[2], duration: 1.0 }),
          createBeatData({ ...testSequence.beats[3], duration: 0.5 }),
        ],
      };
    });
    ...
    it("should generate sequence statistics", () => {
      // Create test sequence with specific properties
      const testSequenceWithStats = {
        ...testSequence,
        beats: [
          createBeatData({
            ...testSequence.beats[0],
      ...
      const stats = service.getSequenceStatistics(testSequenceWithStats);
    ...
    it("should generate sequence word from pictograph letters", () => {
      // Create test sequence with pictograph data
      const testSequenceWithLetters = {
        ...testSequence,
        beats: [
          createBeatData({
            ...testSequence.beats[0],
          ...
          createBeatData({
            ...testSequence.beats[1],
            pictographData: createPictographData({ letter: Letter.B }),
          }),
          createBeatData({
            ...testSequence.beats[2],
            pictographData: createPictographData({ letter: Letter.C }),
          }),
          testSequence.beats[3],
        ],
      };

      const word = service.generateSequenceWord(testSequenceWithLetters);
