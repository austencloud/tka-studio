...
def test_position_matcher():
    ...
    try:
        ...
        total_pictographs = sum(
            len(group) for group in matcher.pictograph_dataset.values()
        )
        ...
        if alpha1_options:
            ...
            for i, option in enumerate(alpha1_options[:5]):  # Show first 5
                print(
                    f"  {i + 1}. Letter: {option.letter}, Start: {option.start_position}, End: {option.end_position}"
                )
        else:
                ...
                for group_key, group in matcher.pictograph_dataset.items():
                    for item in group:
                        if item.get("start_pos") == "alpha1":
                            ...
                            if count <= 3:
                                print(
                                    f"  Example {count}: Letter={item.get('letter')}, Start={item.get('start_pos')}, End={item.get('end_pos')}"
                                )
