...
class TKADeadCodeCleaner:
    ...
    def remove_unused_variables(self):
        ...
        specific_fixes = [
            (
                "src/desktop/legacy/src/letter_determination/services/motion_comparator.py",
                46,
                "other_copy",
            ),
            (
                "src/desktop/legacy/src/main_window/main_widget/browse_tab/sequence_picker/sequence_picker_sorter.py",
                173,
                "skip_image",
            ),
        ]
                ...
                if var_name in line:
                    ...
                    if new_line.strip() and new_line != line:
                        ...
                        self.changes_made.append(
                            f"Removed unused variable '{var_name}' from {file_path}:{line_num}"
                        )
                        print(
                            f"✅ Removed unused variable '{var_name}' from {file_path}"
                        )
    ...
    def fix_syntax_error(self):
        ...
        file_path = (
            self.root_path / "src/desktop/modern/tests/fixtures/domain_fixtures.py"
        )
        if file_path.exists():
            try:
                with open(file_path, encoding="utf-8") as f:
                ...
                content = re.sub(
                    r"domain\.models\.pictograph_models,\s*$",
                ...
                print("✅ Fixed syntax error in domain_fixtures.py")
    ...
    def remove_unreachable_code(self):
        ...
        file_path = (
            self.root_path
            / "src/desktop/legacy/src/main_window/main_widget/generate_tab/circular/CAP_executors/strict_rotated_CAP_executor.py"
        )
        if file_path.exists():
            try:
                with open(file_path, encoding="utf-8") as f:
                ...
                if len(lines) >= 373:
                    ...
                    if "return" in line:
                        ...
                        self.changes_made.append(
                            f"Removed unreachable code from {file_path}"
                        )
                        print("✅ Removed unreachable code from CAP executor")
    ...
    def run_cleanup(self):
        ...
        print(
            "  3. Review interface files manually (many 'unused' variables are actually API contracts)"
        )
