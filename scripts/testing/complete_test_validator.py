...
class CompleteTestValidator:
    ...
    def execute_test_category(self, category: str, path: str) -> List[TestResult]:
        ...
        try:
            ...
            category_results = self._parse_category_results(
                test_items, result.stdout + result.stderr, execution_time / len(test_items)
            )
        ...
        for i, test_item in enumerate(test_items, 1):
            ...
            try:
                ...
                test_result = self._parse_individual_test_result(
                    test_item, result.stdout + result.stderr, execution_time, result.returncode
                )
    ...
    def _parse_category_results(self, test_items: List[str], output: str, avg_time: float) -> List[TestResult]:
    ...
    def _parse_individual_test_result(self, test_item: str, output: str, execution_time: float, return_code: int) -> TestResult:
    ...
    def _generate_summary(self, total_time: float) -> Dict:
        ...
        collection_errors = len([r for r in self.results if r.status == "COLLECTION_ERROR"])
