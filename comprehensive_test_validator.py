...
class ComprehensiveTestValidator:
    ...
    def _analyze_test_result(self, test_item: str, file_path: str, test_name: str, 
                           result: subprocess.CompletedProcess, execution_time: float) -> TestResult:
    ...
    def generate_detailed_report(self) -> str:
        ...
        report.append(f"- **Passed**: {self.summary['passed']} ({self.summary['passed']/self.summary['total_tests']*100:.1f}%)")
        report.append(f"- **Import Errors**: {self.summary['failed_import']} ({self.summary['failed_import']/self.summary['total_tests']*100:.1f}%)")
        report.append(f"- **Logic Errors**: {self.summary['failed_logic']} ({self.summary['failed_logic']/self.summary['total_tests']*100:.1f}%)")
        report.append(f"- **Collection Errors**: {self.summary['collection_errors']} ({self.summary['collection_errors']/self.summary['total_tests']*100:.1f}%)")
        report.append(f"- **Skipped**: {self.summary['skipped']} ({self.summary['skipped']/self.summary['total_tests']*100:.1f}%)")
        report.append(f"- **Total Execution Time**: {self.summary['total_execution_time']:.2f} seconds")
        report.append("")
...
def main():
    ...
    print(f"❌ Failed: {summary['failed_import'] + summary['failed_logic'] + summary['collection_errors']} ({(summary['failed_import'] + summary['failed_logic'] + summary['collection_errors'])/summary['total_tests']*100:.1f}%)")
    print(f"⏱️ Total Time: {summary['total_execution_time']:.2f} seconds")
    print(f"📄 Reports saved: comprehensive_test_validation_report.md, comprehensive_test_results.json")
