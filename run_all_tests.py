...
def run_test_category(category_name: str, test_path: str) -> Dict:
    ...
    env = {
        "PYTHONPATH": ":".join([
            str(Path.cwd()),
            str(Path.cwd() / "src"),
            str(Path.cwd() / "src" / "desktop" / "modern" / "src"),
        ])
    }
    ...
    try:
        ...
        status = "SUCCESS" if errors == 0 and failed == 0 and total > 0 else "PARTIAL" if total > 0 else "FAILED"
        
        print(f"   📊 Results: {passed}/{total} passed ({passed/total*100:.1f}% success)" if total > 0 else "   ⚪ No tests found")
...
def run_all_tests():
    ...
    for result in results:
        if result["status"] == "SUCCESS":
            print(f"   {result['category']}: {result['passed']}/{result['total']} tests")
    
    # Show problematic categories
    print(f"\n❌ CATEGORIES WITH ISSUES:")
    for result in results:
        if result["status"] not in ["SUCCESS"]:
            print(f"   {result['category']}: {result['status']} - {result['errors']} errors, {result['failed']} failures")
